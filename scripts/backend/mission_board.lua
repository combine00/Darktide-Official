...
function MissionBoard:has_character_been_asked_to_skip_campaign(account_id, character_id)
	if not account_id or not character_id then
		return nil
	end
...
function MissionBoard:is_character_eligible_to_skip_campaign(account_id, character_id)
	if not account_id or not character_id then
		return nil
	end
...
function MissionBoard:skip_and_unlock_campaign(account_id, character_id)
	if not account_id or not character_id then
		return nil
	end
...
function MissionBoard:set_character_has_been_shown_skip_campaign_popup(account_id, character_id)
	local body = {
		player_journey_option_popup = "true"
	}
...
function MissionBoard:reset_cached_highest_difficulty(character_id)
...
function MissionBoard:fetch_player_journey_data(account_id, character_id, force_refresh)
...
function MissionBoard:fetch_character_campaign_skip_data(account_id, character_id)
	if not account_id or not character_id then
...
function MissionBoard:_clear_cached_progression_data()
	self._cached_progression_data = false
	self._difficulty_progression_data = {}
	self._progression_unlock_data = {}
...
function MissionBoard:get_difficulty_progression_data()
	return self._difficulty_progression_data
end

function MissionBoard:get_new_difficulty_unlocked(character_id)
