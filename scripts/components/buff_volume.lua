...
function BuffVolume:init(unit, is_server, nav_world)
...
function BuffVolume:destroy()
	return
end

function BuffVolume:enable(unit)
	return
end

function BuffVolume:disable(unit)
	return
end
...
function BuffVolume:update(unit, dt, t)
...
function BuffVolume:_update_buffs(unit, dt, t, num_results)
...
function BuffVolume:_update_inverse_buffs(unit, dt, t)
	local buff_affected_units = self._buff_affected_units
	local ALIVE = ALIVE
...
function BuffVolume:_add_buff(unit, t)
...
function BuffVolume:_remove_buff(unit, buff_indices, t)
...
function BuffVolume:_calculate_broadphase_size()
...
function BuffVolume:enable_buffs()
	self._buffs_enabled = true
end

function BuffVolume:disable_buffs()
...
function BuffVolume:editor_init(unit)
...
function BuffVolume:editor_validate(unit)
	return true, ""
end

function BuffVolume:editor_destroy(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function BuffVolume:editor_update(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function BuffVolume:editor_property_changed(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
end
