...
function ToxicGasCorals:init(unit, is_server, nav_world)
	self._unit = unit
...
function ToxicGasCorals:destroy(unit)
	self:disable(unit)
end

function ToxicGasCorals:enable(unit)
...
function ToxicGasCorals:disable(unit)
...
function ToxicGasCorals.events:visibility_enable(unit)
	return
end

function ToxicGasCorals.events:visibility_disable(unit)
	return
end

function ToxicGasCorals.events:create_low_gas()
...
function ToxicGasCorals.events:despawn_low_gas()
...
function ToxicGasCorals.events:create_trigger_gas()
	Unit.flow_event(self._unit, "create_trigger_gas")
end

function ToxicGasCorals:hot_join_sync(joining_client, joining_channel)
...
function ToxicGasCorals:editor_init(unit)
...
function ToxicGasCorals:editor_toggle_visibility_state(visible)
	return
end

function ToxicGasCorals:editor_validate(unit)
	local success = true
	local error_message = ""
...
function ToxicGasCorals:editor_destroy(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
end
...
function ToxicGasCorals:editor_update(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function ToxicGasCorals:editor_property_changed(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
end

function ToxicGasCorals:_editor_debug_draw(unit)
	return
end
