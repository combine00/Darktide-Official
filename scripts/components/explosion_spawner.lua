...
function ExplosionSpawner:init(unit, is_server)
	self:enable(unit)
...
function ExplosionSpawner.events:unit_died()
	self:explosive_trigger()
end

function ExplosionSpawner:editor_init(unit)
	self:enable(unit)
end

function ExplosionSpawner:editor_validate(unit)
	return true, ""
end

function ExplosionSpawner:enable(unit)
	return
end

function ExplosionSpawner:disable(unit)
	return
end

function ExplosionSpawner:destroy(unit)
	return
end

function ExplosionSpawner:create_explosion()
	...
	local item_or_nil, origin_slot_or_nil, optional_hit_units_table, optional_attacking_unit_owner_unit, optional_apply_owner_buffs = nil
