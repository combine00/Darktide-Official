...
function ToxicGasFog:init(unit, is_server, nav_world)
	local run_update = true

	return run_update
end

function ToxicGasFog:destroy(unit)
	self:disable(unit)
end

function ToxicGasFog:set_volume_enabled(enabled)
...
function ToxicGasFog:enable(unit)
...
function ToxicGasFog:update(unit, dt, t)
...
function ToxicGasFog:_update_volume_percentage(unit, percentage)
...
function ToxicGasFog:disable(unit)
...
function ToxicGasFog.events:visibility_enable(unit)
	self:set_volume_enabled(true)
end

function ToxicGasFog.events:visibility_disable(unit)
	self:set_volume_enabled(false)
end

function ToxicGasFog.events:create_low_gas()
...
function ToxicGasFog.events:despawn_low_gas()
...
function ToxicGasFog.events:start_fading()
	self._fade_time = FADE_TIME
	self._start_fade_time = FADE_TIME
end

function ToxicGasFog.events:create_trigger_gas()
	Unit.flow_event(self.unit, "create_trigger_gas")
end

function ToxicGasFog:hot_join_sync(joining_client, joining_channel)
...
function ToxicGasFog:editor_init(unit)
...
function ToxicGasFog:editor_toggle_visibility_state(visible)
...
function ToxicGasFog:editor_validate(unit)
	local success = true
	local error_message = ""
...
function ToxicGasFog:editor_destroy(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function ToxicGasFog:editor_update(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function ToxicGasFog:_generate_liquid_positions(unit)
...
function ToxicGasFog:editor_on_mission_changed(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function ToxicGasFog:editor_world_transform_modified(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function ToxicGasFog:editor_property_changed(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function ToxicGasFog:_editor_debug_draw(unit)
	local drawer = self._drawer

	drawer:reset()

	local fail_liquid_position = self._fail_liquid_position
