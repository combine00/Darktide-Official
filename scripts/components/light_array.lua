...
function LightArray:init(unit)
	...
	local light_data = {
		type = self:get_data(unit, "light_type"),
		ies = self:get_data(unit, "light_ies"),
		color_filter = self:get_data(unit, "light_color_filter"),
		intensity = self:get_data(unit, "light_intensity"),
		temperature = self:get_data(unit, "light_temperature"),
		volumetric_intensity = self:get_data(unit, "light_volumetric_intensity"),
		cast_shadows = self:get_data(unit, "light_cast_shadows"),
		dynamic = self:get_data(unit, "light_dynamic"),
		force_static = self:get_data(unit, "light_force_static"),
		max_shadow_resolution = self:get_data(unit, "light_max_shadow_resolution"),
		lens_flare = self:get_data(unit, "light_lens_flare"),
		reflector = self:get_data(unit, "light_reflector"),
		falloff_start = self:get_data(unit, "light_falloff_start"),
		falloff_end = self:get_data(unit, "light_falloff_end"),
		angle_start = self:get_data(unit, "light_angle_start"),
		angle_end = self:get_data(unit, "light_angle_end")
	}
...
function LightArray:editor_init(unit)
...
function LightArray:editor_apply_enabled(light_group_enabled)
...
function LightArray:enable(unit)
	return
end

function LightArray:disable(unit)
	return
end

function LightArray:destroy(unit)
...
function LightArray:editor_destroy(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function LightArray:editor_validate(unit)
	return true, ""
end

function LightArray:update(unit, dt, t)
	if self._effect_active then
		if self._effect_time_between == 0 then
			if self._effect_time_on < self._effect_time then
				self:_effect()

				self._effect_time = 0
			end
		elseif self._effect_time_off < self._effect_time then
...
function LightArray:_effect_cycle(skip_on)
	if self._size < self._effect_index then
		self._effect_index = 1
	end
...
function LightArray:_effect_cycle_inverse(skip_on)
...
function LightArray:_effect_ping_pong(skip_on)
...
function LightArray:_set_light_data(spawned_unit, data)
...
function LightArray:start(unit)
