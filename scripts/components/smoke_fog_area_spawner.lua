...
function SmokeFogAreaSpawner:init(unit, is_server, nav_world)
	self._unit = unit
...
function SmokeFogAreaSpawner:destroy(unit)
	return
end

function SmokeFogAreaSpawner:enable(unit)
	return
end

function SmokeFogAreaSpawner:disable(unit)
	return
end

function SmokeFogAreaSpawner:spawn_smoke_fog()
...
function SmokeFogAreaSpawner:despawn_smoke_fog()
	if not self._is_server then
		return
	end
...
function SmokeFogAreaSpawner:hot_join_sync(joining_client, joining_channel)
	return
end

function SmokeFogAreaSpawner:editor_init(unit)
...
function SmokeFogAreaSpawner:editor_validate(unit)
	local success = true
	local error_message = ""
...
function SmokeFogAreaSpawner:editor_destroy(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function SmokeFogAreaSpawner:editor_update(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function SmokeFogAreaSpawner:editor_property_changed(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function SmokeFogAreaSpawner:_editor_debug_draw(unit)
