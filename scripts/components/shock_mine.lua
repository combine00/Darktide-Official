...
function ShockMine:editor_validate(unit)
	local success = true
	local error_message = ""
...
function ShockMine:enable(unit)
	return
end

function ShockMine:disable(unit)
	return
end
...
function ShockMine:update(unit, dt, t)
...
function ShockMine:_update_target_effects(dt, t)
	...
	local num_results = broadphase:query(POSITION_LOOKUP[unit], HARD_CODED_RADIUS, _broadphase_results, enemy_side_names)
...
function ShockMine:_play_target_vfx(world, unit, target_unit)
...
function ShockMine:_play_target_sfx(wwise_world, target_unit)
...
function ShockMine:_stop_looping_sound(source_id, playing_id, event_name)
	local wwise_world = self._wwise_world
...
function ShockMine:_update_targets_sfx_loop(source_id, num_targets)
...
function ShockMine:_start_idle_vfx_loop()
	local world = self._world
	local unit = self._unit
...
function ShockMine:_stop_idle_vfx_loop()
	local world = self._world
	local idle_particle_ids = self._idle_particle_ids
...
function ShockMine:_update_target_cooldowns(dt, t)
	local cooldowns = self._cooldowns
...
function ShockMine.events:shock_mine_start_arming()
	self:_start_arming()
end

function ShockMine.events:shock_mine_deploy()
	self:_deploy()
end
