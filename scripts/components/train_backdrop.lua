...
function TrainBackdrop:_update_effects(chunk)
	if chunk._chunk_type == "tunnel" then
		if self._wagons_chunk_location_start <= chunk.location_index and chunk.location_index < self._wagons_chunk_location_end then
		...
		if self._wagons_chunk_location_start <= chunk_end and chunk_end < self._wagons_chunk_location_end then
