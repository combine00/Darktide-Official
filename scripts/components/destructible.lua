...
function Destructible:init(unit, is_server)
	...
	local has_damage_material = self._damage_material_slot_name and not not self._damage_amount_variable_name
...
local function get_current_stage(destructible_stages, current_health_percent)
	...
	for i = stage_count, 1, -1 do
		...
		local above_next_threshold = not next_stage or next_stage.health_threshold < current_health_percent
...
function Destructible:_hotjoin_stage_update(unit, dt, t)
...
function Destructible:editor_validate(unit)
	local success = true
	local error_message = ""
...
function Destructible.events:add_damage(damage, hit_actor, attack_direction, attacking_unit)
	local destructible_extension = self._destructible_extension
