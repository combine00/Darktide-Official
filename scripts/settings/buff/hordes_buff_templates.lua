...
local _compute_fire_pulse, _give_passive_grenade_replenishment_buff, _pull_enemies_towards_position = nil
...
templates.hordes_buff_zealot_fire_pulse_while_aiming_lunge = {
	...
	update_func = function (template_data, template_context, dt, t)
		if template_context.is_server and template_data.is_active and template_data.next_pulse_t <= t then
...
templates.hordes_buff_psyker_brain_burst_hits_nearby_enemies = {
	...
	proc_func = function (params, template_data, template_context, t)
		...
		local num_hits = broadphase:query(enemy_position, range, BROADPHASE_RESULTS, enemy_side_names)
		template_data.next_target_index = 1
		template_data.hit_targets_left = 0
...
local function _spread_fire_to_nearby_units(broadphase, query_side_name, query_results, player_unit, origin_unit, origin_position, range, max_units_to_spread, fire_stacks, warp_fire_stacks)
	...
	local t = FixedFrame.get_latest_fixed_time()
	local num_hits = broadphase:query(origin_position, range, query_results, query_side_name)
...
templates.hordes_buff_weakspot_ranged_hit_gives_infinite_ammo = {
	...
	check_proc_func = function (params, template_data, template_context, t)
		...
		return num_weakspot_hit_streak_needed_for_infinite_ammo <= template_data.hits_in_a_row
	end,
	proc_func = function (params, template_data, template_context, t)
		template_data.hits_in_a_row = 0
	end
...
templates.hordes_buff_bonus_crit_chance_on_ammo = {
	...
	conditional_stat_buffs_func = function (template_data, template_context)
		...
		return current_animation_percentage >= ammunition_percentage
	end
}
...
templates.hordes_buff_extra_toughness_near_burning_shocked_enemies = {
	...
	lerp_t_func = function (t, start_time, duration, template_data, template_context)
		...
		if next_check_t < t then
			...
			local enemy_side_names = template_data.enemy_side_names
			local num_stacks = 0
			local num_hits = broadphase:query(player_position, template_data.range, BROADPHASE_RESULTS, enemy_side_names)
			...
			for i = 1, num_hits do
				...
				if buff_extension then
					...
					if target_is_burning_or_electrocuted then
						...
						if max_toughness_stacks_gained_per_burning_shocked_enemy <= num_stacks then
							break
						end
					end
				end
...
templates.hordes_buff_coherency_shock_on_grenade_impact_effect = {
	...
	proc_func = function (params, template_data, template_context, t)
		...
		local broadphase = template_data.broadphase
		local enemy_side_names = template_data.enemy_side_names
		local num_hits = broadphase:query(shock_area_position, range_melee, BROADPHASE_RESULTS, enemy_side_names)
...
templates.hordes_buff_toughness_regen_out_of_melee_range = {
	...
	update_func = function (template_data, template_context, dt, t, template)
		...
		if next_regen_t < t then
			...
			local broadphase = template_data.broadphase
			local enemy_side_names = template_data.enemy_side_names
			local num_hits = broadphase:query(player_position, range_melee, BROADPHASE_RESULTS, enemy_side_names)
...
templates.hordes_buff_toughness_regen_in_melee_range = {
	...
	update_func = function (template_data, template_context, dt, t, template)
		...
		local broadphase = template_data.broadphase
		local enemy_side_names = template_data.enemy_side_names
		local num_hits = broadphase:query(player_position, detection_range_for_tougness_regen_close_to_enemies, BROADPHASE_RESULTS, enemy_side_names)
		local was_active = template_data.is_active
		local is_active = template_data.min_num_enemies <= num_hits
...
templates.hordes_buff_toughness_damage_taken_above_threshold = {
	...
	conditional_stat_buffs_func = function (template_data, template_context)
		...
		return percent_toughness_threshold_for_damage_reduction <= current_toughness_percent
	end
}
...
templates.hordes_buff_burning_damage_per_burning_enemy = {
	...
	lerp_t_func = function (t, start_time, duration, template_data, template_context)
		...
		if next_check_t < t then
			...
			local enemy_side_names = template_data.enemy_side_names
			local num_stacks = 0
			local num_hits = broadphase:query(player_position, range_close, BROADPHASE_RESULTS, enemy_side_names)
...
function _compute_fire_pulse(is_server, player_unit, broadphase, enemy_side_names, t, optional_stacks, optional_skip_effects)
	...
	local player_position = POSITION_LOOKUP[player_unit]
	local num_hits = broadphase:query(player_position, 12, BROADPHASE_RESULTS, enemy_side_names)
...
local function _trigger_aoe_shock_at_position(target_position, owner_unit, broadphase, target_side_names, range, t)
	local num_hits = broadphase:query(target_position, range, BROADPHASE_RESULTS, target_side_names)
...
local function _get_random_nearby_alive_enemy_from_position(target_position, broadphase, target_side_names, range)
	...
	for i = 1, num_hits do
		...
		if HEALTH_ALIVE[target_unit] then
			...
			local picked = math.random() <= pick_chance
...
templates.hordes_buff_shock_pulse_on_toughness_broken = {
	...
	proc_func = function (params, template_data, template_context, t)
		...
		local broadphase = template_data.broadphase
		local enemy_side_names = template_data.enemy_side_names
		local num_hits = broadphase:query(player_position, shock_on_toughness_broken_range, BROADPHASE_RESULTS, enemy_side_names)
...
templates.hordes_buff_grenade_heals_on_explosion = {
	...
	proc_func = function (params, template_data, template_context)
		...
		local position = params.position:unbox()
		local broadphase = template_data.broadphase
		local num_hits = broadphase:query(position, range_close, BROADPHASE_RESULTS, side_name)
...
templates.hordes_buff_ogryn_fire_trail_on_lunge = {
	...
	update_func = function (template_data, template_context, dt, t)
		if template_context.is_server and template_data.is_lunging and template_data.next_pulse_t <= t then
...
templates.hordes_buff_ogryn_rock_charge_while_wield = {
	...
	update_func = function (template_data, template_context, dt, t)
		...
		local requires_help = PlayerUnitStatus.requires_help(character_state_component)
		local is_at_max_stacks = template_data.max_stacks <= template_data.damage_increase_stacks
		...
		if template_data.build_up_enabled and template_data.next_damage_increase_t <= t then
			template_data.damage_increase_stacks = template_data.damage_increase_stacks + 1
			template_data.next_damage_increase_t = t + template_data.damage_increase_interval

			if template_data.max_stacks <= template_data.damage_increase_stacks then
				if template_data.wwise_source_id then
...
templates.hordes_buff_adamant_grenade_multi = {
	...
	proc_func = function (params, template_data, template_context, t)
		...
		if not template_data.cluster_reached_minimum and adamant_target_num_enemies_killed_from_grenade <= template_data.num_enemies_killed_in_cluster then
			local player_unit = template_context.unit
			local ability_extension = template_data.ability_extension
...
templates.hordes_buff_adamant_random_bash = {
	...
	proc_func = function (params, template_data, template_context, t)
		...
		if template_data.hit_units_wipe_t and template_data.hit_units_wipe_t <= t then
