...
templates.adamant_grenade_cluster_kills_tracking_buff = {
	...
	proc_func = function (params, template_data, template_context, t)
		...
		if not template_data.recorded_cluster and target_num_enemies_killed_from_grenade <= template_data.num_enemies_killed_in_cluster then
...
templates.adamant_hunt_stance = {
	...
	proc_func = function (params, template_data, template_context, t)
		if CheckProcFunctions.on_ranged_hit(params, template_data, template_context) and template_data.ammo_talent and template_data.next_ammo_t <= t then
...
function _adamant_mark_enemies_select_unit(template_data, template_context, last_unit_pos, t)
	...
	local num_results = broadphase:query(player_position, boradphase_distance, broadphase_results, enemy_side_names)
	...
	for i = 1, num_results do
		...
		if HEALTH_ALIVE[enemy_unit] and not template_data.targets[enemy_unit] then
			...
			if instant_type or valid_breed and template_data.next_t < t then
...
templates.adamant_forceful_stacks = {
	...
	update_func = function (template_data, template_context, dt, t)
		local should_be_active = talent_settings.forceful.stacks <= template_context.stack_count
...
templates.adamant_forceful_stagger = {
	...
	update_func = function (template_data, template_context, dt, t)
		...
		if template_data.stacks > 0 and stacks <= 0 then
			if template_data.next_low_t <= t then
				_forceful_explosion(template_data, template_context)

				template_data.next_low_t = t + talent_settings.forceful.internal_cd
			end
		elseif template_data.stacks < talent_settings.forceful.stacks and talent_settings.forceful.stacks <= stacks and template_data.next_high_t <= t then
			_forceful_explosion(template_data, template_context)
...
templates.adamant_terminus_warrant = {
	...
	specific_proc_func = {
		...
		on_wield_melee = function (params, template_data, template_context, t)
			...
			if template_data.has_upgrade_talent and talent_settings.terminus_warrant.swap_stacks <= current_stacks then
				template_context.buff_extension:add_internally_controlled_buff("adamant_terminus_warrant_melee_temporary", t)
			end

			if template_data.has_talent and talent_settings.terminus_warrant.swap_stacks_talent <= current_stacks then
...
templates.adamant_weapon_handling = {
	...
	update_func = function (template_data, template_context, dt, t)
		...
		if is_active and template_data.next_t < t then
...
templates.adamant_shield_plates = {
	...
	specific_proc_func = {
		on_perfect_block = function (params, template_data, template_context, t)
			if not template_data.perfect_t or template_data.perfect_t < t then
...
templates.adamant_stamina_spent_replenish_toughness = {
	...
	update_func = function (template_data, template_context, dt, t)
		...
		if talent_settings.stamina_spent_replenish_toughness.stamina <= template_data.spent_stamina then
