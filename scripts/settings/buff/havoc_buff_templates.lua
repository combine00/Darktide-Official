...
local function _bolstering_stop_function(template_context, template_data)
	...
	local num_results = broadphase:query(position, BOLSTERING_RADIUS, DPLUS_RESULTS_1, target_side_names)
...
local function _corruption_stop_function(template_context, template_data)
	...
	local broadphase = broadphase_system.broadphase
	local position = POSITION_LOOKUP[unit]
	local num_results = broadphase:query(position, CORRUPTION_RADIUS, DPLUS_RESULTS_2, target_side_names)
	local current_time = FixedFrame.get_latest_fixed_time()
...
local function _apply_poxburster_bile(template_data, template_context)
	...
	local broadphase = broadphase_system.broadphase
	local num_results = broadphase:query(unit_position, RADIUS, TEMP_BROADPHASE_RESULTS, enemy_side_names)
...
local function _healed_by_the_garden(template_data, template_context)
	...
	local broadphase = broadphase_system.broadphase
	local position = POSITION_LOOKUP[unit]
	local allied_sied_num_results = broadphase:query(position, GARDEN_BUFF_RADIUS, GARDEN_HAVOC_RESULTS_ENEMIES, target_side_names)
	local current_time = FixedFrame.get_latest_fixed_time()
	local t = Managers.time:time("gameplay")
...
local function _close_allies_nurgle_blessed(template_data, template_context)
	...
	local broadphase = broadphase_system.broadphase
	local position = POSITION_LOOKUP[unit]
	local num_results = broadphase:query(position, BUFF_RADIUS, HAVOC_RESULT_1, target_side_names)
	local current_time = FixedFrame.get_latest_fixed_time()
	local t = Managers.time:time("gameplay")
