...
local function _update_target(template_data, template_context)
	...
	if vfx and vfx_data then
		...
		local additional_height = Vector3(0, 0, 2)
		target_position = target_position + additional_height
...
local effect_template = {
	name = "chaos_mutator_ritualist_chanting",
	resources = resources,
	...
	update = function (template_data, template_context, dt, t)
		...
		local target_unit = target_unit_id ~= NetworkConstants.invalid_level_unit_id and Managers.state.unit_spawner:unit(target_unit_id, false)
