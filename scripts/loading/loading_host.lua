...
function LoadingHost:init(network_delegate, loaders, connection_class_name)
	...
	function self._done_loading_level_func(spawn_group_id, peer_id)
	...
	for _, spawn_group in ipairs(spawn_groups) do
		if spawn_group.state == "wait_for_sync" then
			...
			if profiles_synced and packages_synced then
				...
				if LoadingHost.MIN_TIME_BEFORE_SPAWN_GROUP_HAS_SYNC_ISSUE < spawn_group.sync_issue_timer then
	...
	if has_sync_issues then
		...
		for peer_id, non_synced_peer_ids in pairs(spawning_peers_with_sync_issues) do
			for sync_peer_id, _ in pairs(non_synced_peer_ids) do
				...
				elseif LoadingHost.NUM_SYNC_ISSUES_BEFORE_ALLOWING_SYNC_PEER_TO_BE_KICKED <= num_sync_issues then
...
function LoadingHost:_determine_spawn_group_sync_issues(spawn_group, sync_peers_filter, spawning_peers_with_sync_issues)
	local package_sync_host = self._package_synchronizer_host
	local profile_sync_host = Managers.profile_synchronization:synchronizer_host()
	local sync_issue_history = self._sync_issue_history
	local has_sync_issues = false
...
function LoadingHost:generate_mission_seed()
	local override = nil
...
function LoadingHost:load_mission(loading_context)
...
function LoadingHost:_finish_spawn_group(spawn_group)
