...
function RemoteLoadState:init(state_machine, shared_state)
...
function RemoteLoadState:destroy()
...
function RemoteLoadState:update(dt)
	if self._ready_to_spawn and (not self._preload_cinematic or self._cinematic_loaded) then
		return "load_done"
	end
end

function RemoteLoadState:spawn_group_ready(spawn_group)
...
function RemoteLoadState:rpc_finished_loading_level(channel_id, spawn_group)
	local shared_state = self._shared_state
...
function RemoteLoadState:_on_cutscene_loaded(preload, id)
