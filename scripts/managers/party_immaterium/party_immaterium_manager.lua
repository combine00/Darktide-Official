...
function PartyImmateriumManager:is_started()
...
function PartyImmateriumManager:all_members()
...
function PartyImmateriumManager:other_members_by_platform(platform)
...
function PartyImmateriumManager:other_members_account_ids()
	local other_members = Managers.party_immaterium:other_members()
	local account_ids = {}
...
function PartyImmateriumManager:_parse_join_parameter_string(join_parameter, is_reconnect)
...
function PartyImmateriumManager:_can_join_new_party_check(join_parameter, is_reconnect)
...
function PartyImmateriumManager:_handle_advertisement_state_update_event_trigger(event)
	if not self._advertising_state or self._advertising_state.version <= event.version then
		event.party_id = self:party_id()
		self._advertising_state = event
	end
end

function PartyImmateriumManager:_handle_advertisement_request_to_join_list_update_event_trigger(event)
	if event and self._advertisement_join_requests_event_version <= event.version then
...
function PartyImmateriumManager:_update_advertisement_request_to_join_list()
...
function PartyImmateriumManager:party_finder_respond_to_join_request(party_id, account_id, accept)
...
function PartyImmateriumManager:is_in_private_session()
	local game_state = self:party_game_state()
...
function PartyImmateriumManager:have_recieved_game_state()
...
function PartyImmateriumManager:wanted_mission_selected(backend_mission_id, private_session, reef)
...
function PartyImmateriumManager:start_party_finder_advertise(config, tags, category)
	self._advertisement_join_requests_by_account_id = {}
	self._advertisement_join_requests_event_version = -1
	self._advertisement_join_requests_presence_version = -1
...
function PartyImmateriumManager:cancel_party_finder_advertise()
...
function PartyImmateriumManager:advertise_state()
	return self._advertising_state
end

function PartyImmateriumManager:is_party_advertisement_active()
...
function PartyImmateriumManager:advertisement_request_to_join_list()
	return self._advertisement_join_requests_by_account_id, self._advertisement_join_requests_presence_version
end

function PartyImmateriumManager:_remove_advertisement_request(account_id)
	local advertisement_join_requests_by_account_id = self._advertisement_join_requests_by_account_id
...
function PartyImmateriumManager:send_request_to_join_party(data, account_id)
	local id = data.id
