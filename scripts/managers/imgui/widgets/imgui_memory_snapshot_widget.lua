...
function Widget.new(display_name, optional_width)
	local widget = {
		filter = "",
		window_width = 0,
...
function Widget.pad_right(str, target_length, pad_str, cache)
	local str_size = #str
...
function Widget.chunk_from_right(str, step_n, sep)
	sep = sep or " "
	local str_len = #str
...
function Widget.select_map(t, selector)
	local new_t = {}
...
function Widget.max_func(t, func)
	local max_key, max_value = next(t)

	for key, value in pairs(t) do
		if func(max_value) < func(value) then
			max_value = value
			max_key = key
		end
	end
...
function Widget.find_references(ref, ref_blob, name_by_ref, references_at_depth, track_size, depth)
...
function Widget.add_to_blob(val, key, ref_blob, name_by_ref, references_at_depth, track_size, depth)
...
function Widget.grab_lua_memory_tree_snapshot(tbl, root_name)
	Deadlock.pause()
...
function Widget.recursive_header(widget, tbl, children_by_ref, name_by_ref, size_by_ref, one_layer_size, name_length, depth)
...
function Widget.get_tbl(filter)
	local root = _G
