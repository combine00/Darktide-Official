...
function PacingManager:init(world, nav_world, level_seed, pacing_control)
	self._tension = 0
...
function PacingManager:update(dt, t)
	...
	local target_side_id = self._target_side_id
...
function PacingManager:override_allowed_spawn_types(override_settings)
...
function PacingManager:is_enabled()
...
function PacingManager:player_died(player_unit)
...
function PacingManager:waiting_for_ramp_clear()
...
function PacingManager:_update_ramp_up_frequency(dt, t, target_side_id)
...
function PacingManager:get_ramp_up_frequency_modifier(spawn_type)
	return self._ramp_up_frequency_modifiers[spawn_type] or 1
end

function PacingManager:get_max_ramp_up_frequency_modifier(spawn_type)
	local ramp_up_frequency_settings = self._ramp_up_frequency_settings
...
function PacingManager:_get_challenge_rating(unit)
...
function PacingManager:aggroed_minions()
	return self._aggroed_minions
end

function PacingManager:num_aggroed_monsters()
...
function PacingManager:set_horde_rate_modifier(horde_rate_modifier)
...
function PacingManager:current_faction(optional_side_name)
...
function PacingManager:set_roamer_tag_limit_bonus(tag_limit_bonus)
...
function PacingManager:activate_hard_mode()
	self._hard_mode = true

	Managers.telemetry_events:hard_mode_activated()
end

function PacingManager:has_hard_mode()
	return self._hard_mode
end

function PacingManager:is_auric()
	return self._is_auric
end

function PacingManager:set_minion_listening_for_player_deaths(unit, statistics_component, set)
	if set then
...
function PacingManager:get_minions_listening_for_player_deaths()
	return self._minions_listening_for_player_deaths
end

function PacingManager:set_specials_timing_multiplier(modifier)
	return self._specials_pacing:set_timer_multiplier(modifier)
end

function PacingManager:set_specials_force_move_timer(should_force_move_timer)
	return self._specials_pacing:set_force_move_timer(should_force_move_timer)
end

function PacingManager:set_ramp_up_enabled(is_enabled)
	self._ramp_up_enabled = is_enabled
end

function PacingManager:force_coordinated_special_strike(timer, num_breeds)
	self._specials_pacing:force_coordinated_strike(timer, num_breeds)
end

function PacingManager:state_duration()
...
function PacingManager:low_state_duration()
...
function PacingManager:set_specials_pacing_spawner_groups(spawner_groups)
	self._specials_pacing:set_spawner_groups(spawner_groups)
end

function PacingManager:reset_specials_pacing_spawner_groups()
	self._specials_pacing:set_spawner_groups(nil)
end

function PacingManager:remove_monsters_behind_pos(pos)
	self._monster_pacing:remove_monsters_behind_pos(pos)
end

function PacingManager:reset_traveled_this_frame()
