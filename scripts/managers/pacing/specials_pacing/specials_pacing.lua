...
function SpecialsPacing:_setup_specials_slot(specials_slots, specials_slot, template, timer_modifier, optional_breed_name, optional_spawn_timer, optional_injected, optional_coordinated_strike, optional_prefered_spawn_direction, optional_target_unit, optional_spawner_group, optional_is_prevention)
...
function SpecialsPacing:set_timer_multiplier(multiplier)
...
function SpecialsPacing:_travel_distance_spawning()
...
function SpecialsPacing:set_travel_distance_spawning_override(travel_distance_spawning)
...
function SpecialsPacing:_get_breed_name(breed_name)
...
function SpecialsPacing:_find_spawn_position(side_id, breed_name, target_side_id, check_ahead, optional_mainpath_offset, optional_randomize)
	local template = self._template
...
function SpecialsPacing:_filter_too_close_spawners(target_side_id, optional_spawners)
	local main_path_manager = Managers.state.main_path
	local target_unit = main_path_manager:ahead_unit(target_side_id)
...
function SpecialsPacing:_check_monster_override(template)
...
function SpecialsPacing:_update_loner_prevention(target_side_id, template, t)
...
function SpecialsPacing:set_chance_of_coordinated_strike(coordinated_strike_chance)
	self._coordinated_strike_chance_override = coordinated_strike_chance
end

function SpecialsPacing:set_coordinated_strike_num_breeds_override(coordinated_strike_num_breeds_override)
	self._coordinated_strike_num_breeds_override = coordinated_strike_num_breeds_override
end

function SpecialsPacing:set_max_of_same_override(max_of_same_override)
	self._optional_max_of_same_override = max_of_same_override
end

function SpecialsPacing:set_required_challenge_rating(required_challenge_rating)
...
function SpecialsPacing:set_monster_spawn_config(monster_spawn_config)
	self._monster_spawn_config = monster_spawn_config
end

function SpecialsPacing:set_always_move_timer(always_move)
...
function SpecialsPacing:set_force_move_timer(should_force_move_timer)
	self._force_move_timer = should_force_move_timer
end

function SpecialsPacing:force_coordinated_strike(timer, override_num_breeds)
...
function SpecialsPacing:set_spawner_groups(spawner_groups)
	self._spawner_groups = spawner_groups
end

function SpecialsPacing:set_move_timer_when_challenge_rating_above(challenge_rating)
	self._override_move_timer_when_challenge_rating_above = challenge_rating
end

function SpecialsPacing:reset_traveled_this_frame()
