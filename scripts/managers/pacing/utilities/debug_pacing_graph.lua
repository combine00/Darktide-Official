...
function DebugPacingGraph:init(world, name, axis_names, position_x, position_y, graph_extents, width, height, frequency, num_value_segments)
	self._world = world
...
function DebugPacingGraph:destroy()
...
function DebugPacingGraph:update(t, x_value, y_value, optional_annotation)
...
function DebugPacingGraph:draw(dt, t)
	local width = self._width
	local height = self._height
	local gui = self._gui
	local origin_position = self._position:unbox()
