...
function RoamerPacing:_limit_roamer_breeds(breed_name, limit_settings, faction, tag_limits)
		...
		for i = 1, num_updates do
			...
			if not roamer.disabled then
				...
				for j = 1, #ACTIVE_TARGET_POSITIONS do
					if Vector3.distance(roamer_position, ACTIVE_TARGET_POSITIONS[j]) < spawn_distance then
...
function RoamerPacing:override_all_roamer_packs(packs_override)
	self._all_pack_override = packs_override
end

function RoamerPacing:override_roamer_packs(packs_override)
	self._packs_override = packs_override
end

function RoamerPacing:override_num_roamer_range(num_roamer_range_override)
...
function RoamerPacing:set_tag_limit_bonus(tag_limit_bonus)
