...
function HordePacing:_update_horde_pacing(t, dt, side_id, target_side_id)
	...
	local high_challenge_rating = CHALLENGE_RATING_FOR_NO_MOVE_TIMER_OVERRIDE < total_challenge_rating
	local move_timer_override = not high_challenge_rating and Managers.state.difficulty:get_table_entry_by_resistance(TIME_SINCE_FORWARD_TRAVEL_CHANGE_MOVE_TIMER_OVERRIDE) <= time_since_forward_travel_changed
	...
	elseif self._next_horde_at <= self._horde_timer then
...
function HordePacing:traveled_this_frame()
	return self._traveled_this_frame
end

function HordePacing:_init_coordinated_horde_strikes(template)
...
function HordePacing:_evaluate_coordinated_horde_strike(target_side_id)
	local template = self._template
	local coordinated_horde_strike_settings = template.coordinated_horde_strike_settings
...
function HordePacing:_start_coordinated_horde_strike(setting, target_side_id)
...
function HordePacing:_update_coordinated_horde_strike(t, dt, side_id, target_side_id, template)
		...
		repeat
			...
			if strike.time_to_next_wave > 0 then
				...
				if success then
					...
					if strike.current_wave < num_waves then
...
function HordePacing:_start_horde(t, dt, side_id, target_side_id, template)
...
function HordePacing:_spawn_horde(horde_type, horde_template, composition, side_id, target_side_id, optional_main_path_offset, optional_num_tries, optional_disallowed_positions, optional_spawn_max_health_modifier, optional_prefered_direction, optional_target_unit, optional_skip_spawners)
...
function HordePacing:set_rate_modifier(modifier)
...
function HordePacing:override_trickle_horde_compositions(compositions)
	self._override_trickle_horde_compositions = compositions
end

function HordePacing:_trigger_pre_stinger(template, side_id, optional_stinger_event_name)
...
function HordePacing:_spawn_trickle_horde_wave(side_id, target_side_id, optional_main_path_offset, optional_num_tries, template, trickle_horde, t)
	local trickle_horde_template = HordeTemplates.trickle_horde
	local trickle_horde_compositions = self._override_trickle_horde_compositions or template.horde_compositions.trickle_horde
	local current_faction = Managers.state.pacing:current_faction()
	local current_density_type = Managers.state.pacing:current_density_type()
...
function HordePacing:_can_spawn(horde_type, horde_template, composition, side_id, target_side_id, optional_main_path_offset, optional_num_tries)
	local horde_manager = Managers.state.horde
	local towards_combat_vector = true
...
function HordePacing:_set_horde_wave_group_sounds(template, group_id)
	local group_system = Managers.state.extension:system("group_system")
	local group = group_system:group_from_id(group_id)
...
function HordePacing:_first_horde_wave_spawn(template, horde_position, target_unit, optional_stinger_override, optional_horde_type_override)
...
function HordePacing:reset_traveled_this_frame()
