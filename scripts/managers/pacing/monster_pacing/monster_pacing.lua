...
function MonsterPacing:_setup_timer_based_monster_pacing(dt, t, side_id, target_side_id)
...
function MonsterPacing:fill_boss_patrols_by_travel_distance(per_travel_distance)
...
function MonsterPacing:_fill_spawns_by_timer(dt, t, side_id, target_side_id)
	local template = self._template
...
function MonsterPacing:set_health_modifier(modifier)
...
function MonsterPacing:_update_timer(dt, t, side_id, target_side_id)
...
function MonsterPacing:_check_alive()
	for i = 1, #names do
		local name = names[i]
...
function MonsterPacing:_update_allowance(dt, t, side_id, target_side_id)
	local template = self._template
	...
	if monsters_allowed then
		...
		elseif pacing_type == "timer_based" then
			...
			if allowed then
				...
				if self._next_monster_at_t <= self._monster_timer then
...
function MonsterPacing:num_aggroed_monsters()
	local aggroed_monster_units = self._aggroed_monster_units

	return #aggroed_monster_units
end

function MonsterPacing:set_num_monsters_override(override)
	self._num_monsters_override = override
end

function MonsterPacing:set_num_witches_override(override)
	self._num_witches_override = override
end

function MonsterPacing:set_num_boss_patrol_override(override)
...
function MonsterPacing:_spawn_boss_patrol(boss_patrol, ahead_travel_distance, side_id)
	...
	local minion_spawn_manager = Managers.state.minion_spawn
	local num_positions, walk_position = nil
	local below = 2
	local above = 2
	local pacing_type = self._pacing_type
...
function MonsterPacing:remove_monsters_behind_pos(position)
	local main_path_manager = Managers.state.main_path
