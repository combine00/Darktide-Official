...
function ApproveChannelDelegate:init()
	self._registered_lobbies = {}
end

function ApproveChannelDelegate:destroy()
	local success = true
	local error_str = ""
...
function ApproveChannelDelegate:register(lobby_id, message, object)
...
function ApproveChannelDelegate:unregister(lobby_id, message)
	local key = self.make_key(lobby_id, message)
	self._registered_lobbies[key] = nil
end

function ApproveChannelDelegate:approve_channel(channel_id, peer_id, lobby_id, message)
	local key = self.make_key(lobby_id, message)
...
function ApproveChannelDelegate.make_key(lobby_id, message)
	if message then
