...
local function lua_within_extents(unit, extents)
	...
	if extents[1] < math.abs(position[1]) or extents[2] < math.abs(position[2]) or extents[3] < math.abs(position[3]) then
		return false
	end
...
function OutOfBoundsManager:init(world, hard_cap_extents, soft_cap_extents)
...
function OutOfBoundsManager:pre_update(shared_state)
...
function OutOfBoundsManager:update(shared_state)
	local soft_cap_out_of_bounds_units = shared_state.soft_cap_out_of_bounds_units
...
function OutOfBoundsManager:register_soft_oob_unit(unit, object, callback_name)
	local registered_units = self._registered_units
...
function OutOfBoundsManager:unregister_soft_oob_unit(unit, object)
	local registered_units = self._registered_units
	local callbacks = registered_units[unit]
...
function OutOfBoundsManager:hard_cap_extents()
	return self._hard_cap_extents:unbox()
end

function OutOfBoundsManager:soft_cap_extents()
	return self._soft_cap_extents:unbox()
end

function OutOfBoundsManager:limit_line_end_position_to_soft_cap_extents(start_position, end_position)
