...
function EmoteManager:init(is_host, network_event_delegate)
...
function EmoteManager:_get_index(unit_id)
	return table.index_of(self._tracking_id, unit_id)
end

function EmoteManager:running_emote(unit_id)
	return self:_get_index(unit_id) ~= -1
end

function EmoteManager:_trigger_face_event(unit, event_name)
...
function EmoteManager:_stop_emote(index)
...
function EmoteManager:_start_emote(unit, slot_id, rpc)
...
function EmoteManager:trigger_emote(unit, slot_id)
...
function EmoteManager:rpc_trigger_emote(_, unit_id, slot_id_lookup)
	local unit = Managers.state.unit_spawner:unit(unit_id)
...
function EmoteManager:check_emote_input(input_extension)
...
function EmoteManager:stop_emote(unit_id)
	local index = self:_get_index(unit_id)
...
function EmoteManager:rpc_stop_emote(channel_id, unit_id)
	local index = self:_get_index(unit_id)
