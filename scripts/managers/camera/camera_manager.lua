...
function CameraManager:_current_transition(camera_nodes)
	return camera_nodes[#camera_nodes].transition
end

function CameraManager:add_camera_effect_sequence_event(event, start_time)
...
function CameraManager:add_camera_effect_shake_event(event_name, optional_source_unit_data)
	if not self._camera_shake_enabled then
		return
	end
...
function CameraManager:use_ui_listener(use_ui_listener)
	self._use_ui_listener = use_ui_listener
end

function CameraManager:_update_camera_properties(camera, shadow_cull_camera, camera_nodes, camera_data, viewport_name)
