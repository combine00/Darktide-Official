...
function PathTypeOpen:init(world, nav_world, num_sides, is_server, use_nav_point_time_slice)
	self._world = world
...
function PathTypeOpen:destroy()
	return
end

function PathTypeOpen:ahead_unit(side_id)
...
function PathTypeOpen:behind_unit(side_id)
	local side_progress_on_path = self._side_progress_on_path
	local progress_on_path = side_progress_on_path[side_id]
...
function PathTypeOpen:furthest_travel_distance(side_id)
	local side_progress_on_path = self._side_progress_on_path
	local progress_on_path = side_progress_on_path[side_id]
...
function PathTypeOpen:furthest_travel_percentage(side_id)
...
function PathTypeOpen:time_since_forward_travel_changed(side_id)
	local side_progress_on_path = self._side_progress_on_path
	local progress_on_path = side_progress_on_path[side_id]
...
function PathTypeOpen:time_since_behind_travel_changed(side_id)
	local side_progress_on_path = self._side_progress_on_path
	local progress_on_path = side_progress_on_path[side_id]
...
function PathTypeOpen:segment_index_by_unit(unit)
	return self._segment_index_by_unit[unit]
end

function PathTypeOpen:update_progress_on_path(t)
