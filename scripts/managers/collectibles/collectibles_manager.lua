...
function CollectiblesManager:init(is_server, network_event_delegate)
...
function CollectiblesManager:on_gameplay_post_init(level_seed)
...
function CollectiblesManager:destroy()
	if not self._is_server then
		self._network_event_delegate:unregister_events(unpack(CLIENT_RPCS))
	end
end

function CollectiblesManager:_random(...)
...
function CollectiblesManager:register_destructible(data)
...
function CollectiblesManager:register_collectible(interactor_unit, collectible_type)
...
function CollectiblesManager:collectible_destroyed(data, attacking_unit)
...
function CollectiblesManager:rpc_player_helped_collecting_collectible(channel_id, peer_id, collectible_type_lookup, assisted_players)
	local type_lookup = NetworkLookup.collectible_type_lookup[collectible_type_lookup]

	self:_show_collectible_notification(peer_id, type_lookup, NOTIFICATION_TYPES.helped_collect, assisted_players)
end

function CollectiblesManager:_show_collectible_notification(peer_id, collectible_type, notification_type, assisted_players)
	local player_manager = Managers.player
	local local_player_id = 1
	local player = player_manager:player(peer_id, local_player_id)
	local player_name = player and player:name()
