...
function ChatManager:peer_id_from_session_handle(session_handle)
	return self._channel_host_peer_id[session_handle]
end

function ChatManager:tag_from_session_handle(session_handle)
...
function ChatManager:is_initialized()
	return self._initialized
end

function ChatManager:connection_state()
	return self._connection_state
end

function ChatManager:is_connected()
...
function ChatManager:login_state()
	return self._login_state
end

function ChatManager:is_logged_in()
	return self:login_state() == ChatManagerConstants.LoginState.LOGGED_IN and self._account_handle ~= nil
end

function ChatManager:login(peer_id, account_id, vivox_token)
...
function ChatManager:join_chat_channel(channel, host_peer_id, voice, text, tag, vivox_token)
	if not self:is_logged_in() then
		return
	end
...
function ChatManager:leave_channel(channel_handle)
	if self:is_logged_in() then
...
function ChatManager:sessions()
	return self._sessions
end

function ChatManager:connected_chat_channels()
	local channels = {}
...
function ChatManager:connected_voip_channels()
	local channels = {}
...
function ChatManager:mic_volume_changed()
...
function ChatManager:send_channel_message(channel_handle, message_body)
	Managers.telemetry_events:chat_message_sent(message_body)
	Vivox.send_session_message(channel_handle, message_body)
end

function ChatManager:send_loc_channel_message(channel_handle, key, context)
...
function ChatManager:update(dt, t)
	self._t = t
...
function ChatManager:channel_text_mute_participant(channel_handle, participant_uri, mute)
...
function ChatManager:channel_voip_mute_participant(channel_handle, participant_uri, mute)
...
function ChatManager:player_mute_status_changed(account_id)
	for session_handle, channel in pairs(self._sessions) do
...
function ChatManager:get_capture_devices()
	return self._capture_devices
end

function ChatManager:set_capture_device(device_id)
...
function ChatManager:_poll_party()
...
function ChatManager:_handle_event(message)
...
function ChatManager:_handle_response(message)
...
function ChatManager:_handle_response_error(message)
...
function ChatManager:_validate_participants()
...
function ChatManager:_update_transmitting_channel_priority()
	local priority = {
