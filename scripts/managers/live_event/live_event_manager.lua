...
function LiveEventManager:init(is_host, event_delegate)
	self._events = {}
...
function LiveEventManager:destroy()
...
function LiveEventManager:add_player(id, account_id, is_local)
	self._players[id] = {
		id = id,
...
function LiveEventManager:has_player(id)
	return self._players[id] ~= nil
end

function LiveEventManager:remove_player(id)
...
function LiveEventManager:remove_all()
	local promises = {}
...
function LiveEventManager:_on_tier_claimed_success(id, event_id, completed_tier, result)
...
function LiveEventManager:_on_tier_claimed_fail(id, event_id, completed_tier, error)
...
function LiveEventManager:_claim_next_tier(id, event_id)
	local event_data = self._events[event_id]
	local player_data = self._players[id]
...
function LiveEventManager:_on_track_state_fail(id, event_id, error)
...
function LiveEventManager:_update_player_xp(id, event_id)
	local is_host = self._is_host
...
function LiveEventManager:_update_player(dt, id)
	local player_data = self._players[id]
	local account_id = player_data.account_id
	...
	for event_id, _ in pairs(self._events) do
		...
		local can_write = not event_data.promise and (not has_timer or STATE_FAIL_DELAY <= event_data.update_timer)
...
function LiveEventManager:refresh_progress()
...
function LiveEventManager:_needs_update(dt, t)
	return REFRESH_TIMER_SUCCESS < self._time_since_update
end

function LiveEventManager:_stop_current_event()
	self._active_event_id = nil
...
function LiveEventManager:_start_event(event_id)
	local event_data = self._events[event_id]
...
function LiveEventManager:_update_active_event(dt, t)
	...
	for event_id, event_data in pairs(self._events) do
		local starts_at = event_data.starts_at
		local ends_at = event_data.ends_at
...
function LiveEventManager:update(dt, t)
...
function LiveEventManager:_add_event(backend_data)
...
function LiveEventManager:_on_refresh_success(backend_events)
...
function LiveEventManager:_on_refresh_fail(error)
	self._refresh_promise = nil
...
function LiveEventManager:refresh()
...
function LiveEventManager:active_progress(optional_id)
...
function LiveEventManager:active_time_left(optional_t)
...
function LiveEventManager:_show_combat_feed_message(amount)
	local event_data = self:_active_event()
...
function LiveEventManager:rpc_live_event_trigger_combat_feed(channel_id, amount)
	self:_show_combat_feed_message(amount)
end

function LiveEventManager:_trigger_combat_feed(event_id, listener_id, stat_id, amount)
