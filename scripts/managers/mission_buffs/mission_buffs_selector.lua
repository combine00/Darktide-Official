...
function MissionBuffsSelector:_register_events()
	return
end

function MissionBuffsSelector:_unregister_events()
	return
end

function MissionBuffsSelector:_register_rpcs(network_event_delegate)
	network_event_delegate:register_session_events(self, unpack(SERVER_RPCS))
end

function MissionBuffsSelector:_unregister_rpcs(network_event_delegate)
	network_event_delegate:unregister_events(unpack(SERVER_RPCS))
end

function MissionBuffsSelector:init(mission_buffs_manager, mission_buffs_handler, network_event_delegate, is_server)
...
function MissionBuffsSelector:destroy()
...
function MissionBuffsSelector:give_random_buff_to_all(buffs_pool)
	local random_buff = self._get_random_buff_from_pool(buffs_pool)

	self._mission_buffs_handler:give_buff_to_all(random_buff)
end

function MissionBuffsSelector:give_random_buff_to_player(player, buffs_pool)
	local random_buff = self._get_random_buff_from_pool(buffs_pool)

	self._mission_buffs_handler:give_buff_to_player(player, random_buff)
end

function MissionBuffsSelector:give_family_buff_to_all(skip_notification)
...
function MissionBuffsSelector:give_family_buff_to_player(player, skip_notification)
...
function MissionBuffsSelector:_get_random_family_buff_for_player(player)
...
function MissionBuffsSelector:_get_family_buffs_pool(family_name)
	return MissionBuffsAllowedBuffs.buff_families[family_name].buffs
end

function MissionBuffsSelector:init_legendary_buffs_pool_for_player(player, buffs_to_exclude)
...
function MissionBuffsSelector:_get_valid_legendary_buffs_for_player_setup(player)
...
function MissionBuffsSelector:_pop_legendary_buff_from_players_pool(player, wave_num)
...
function MissionBuffsSelector._get_random_buff_from_pool(buffs_pool)
...
function MissionBuffsSelector._get_random_buff_and_remove_from_pool(buffs_pool)
	local available_buffs = buffs_pool
	local num_buffs = #available_buffs
...
function MissionBuffsSelector._fill_with_random_items_from_pool(items_pool, num_items, results)
...
function MissionBuffsSelector:create_buff_family_choice_for_all(num_choices)
	local players = Managers.player:human_players()
...
function MissionBuffsSelector:create_buff_family_choice_for_player(player, num_choices)
	local is_human = player:is_human_controlled()
...
function MissionBuffsSelector:create_legendary_buff_choice_for_player(player, wave_num, num_choices)
	local is_human = player:is_human_controlled()
...
function MissionBuffsSelector:create_legendary_buff_choice_for_all(wave_num, num_choices)
	local players = Managers.player:human_players()
...
function MissionBuffsSelector:player_selected_buff_choice(player, choice_index)
...
function MissionBuffsSelector:set_buff_family_for_player(player, buff_family_name, from_choice)
...
function MissionBuffsSelector:try_start_new_buff_choice_for_player(player)
...
function MissionBuffsSelector:rpc_server_mission_buffs_player_buff_choice(channel_id, peer_id, local_player_id, choice_index)
