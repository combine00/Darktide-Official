...
function MissionBuffsHandler:init(mission_buffs_manager, game_mode, is_server)
	self._game_mode = game_mode
	self._mission_buffs_manager = mission_buffs_manager
	self._persistent_data = MissionBuffsPersistentData:new()
end

function MissionBuffsHandler:destroy()
	self._game_mode = nil
...
function MissionBuffsHandler:does_player_have_existing_data(player)
	return self._persistent_data:does_player_have_existing_data(player)
end

function MissionBuffsHandler:does_player_have_legendary_buffs_pool(player)
	return self._persistent_data:does_player_have_legendary_buffs_pool(player)
end

function MissionBuffsHandler:check_player_buff_family_state(player)
	return self._persistent_data:check_player_buff_family_state(player)
end

function MissionBuffsHandler:get_buff_family_selected_by_player(player)
	return self._persistent_data:get_buff_family_selected_by_player(player)
end

function MissionBuffsHandler:get_num_buffs_given_to_player(player)
	return self._persistent_data:get_num_buffs_given_to_player(player)
end

function MissionBuffsHandler:get_num_legendary_buff_choices_pending_for_player(player)
	return self._persistent_data:get_num_legendary_buff_choices_pending_for_player(player)
end

function MissionBuffsHandler:does_player_have_buff_saved(player, buff_name)
	return self._persistent_data:does_player_have_buff_saved(player, buff_name)
end

function MissionBuffsHandler:save_buff_family_choice_for_player(player, family_name_choices)
	self._persistent_data:add_choice_for_player(player, family_name_choices, true)
end

function MissionBuffsHandler:save_buff_choice_for_player(player, buff_name_choices)
	self._persistent_data:add_choice_for_player(player, buff_name_choices, false)
end

function MissionBuffsHandler:restore_unselected_legendary_buffs_to_player_pool(player, buff_options, chosen_option_index)
	self._persistent_data:restore_unselected_legendary_buffs_to_player_pool(player, buff_options, chosen_option_index)
end

function MissionBuffsHandler:try_resolve_current_choice_for_player(player, choice_index)
	return self._persistent_data:try_resolve_current_choice_for_player(player, choice_index)
end

function MissionBuffsHandler:try_start_new_choice_for_player(player)
	return self._persistent_data:try_start_new_choice_for_player(player)
end

function MissionBuffsHandler:get_current_choice_for_player(player)
	return self._persistent_data:get_current_choice_for_player(player)
end

function MissionBuffsHandler:restore_buffs_given_to_player(player)
...
function MissionBuffsHandler:give_buff_to_all(buff_name)
...
function MissionBuffsHandler:give_buff_to_player(player, buff_name, skip_notification, skip_saving_buff_in_player_data)
...
function MissionBuffsHandler:set_legendary_buffs_available_for_player(player, legendary_buffs)
	self._persistent_data:set_legendary_buffs_available_for_player(player, legendary_buffs)
end

function MissionBuffsHandler:get_legendary_buffs_available_for_player(player)
	return self._persistent_data:get_legendary_buffs_available_for_player(player)
end

function MissionBuffsHandler:buff_family_choice_initiated()
	self._persistent_data:buff_family_choice_initiated()
end

function MissionBuffsHandler:set_buff_family_for_player(player, family_name, priotity_family_buffs, family_buffs, from_choice)
...
function MissionBuffsHandler:check_if_all_players_chosen_family()
...
function MissionBuffsHandler:does_player_have_family_selected(player)
	return self._persistent_data:does_player_have_family_selected(player)
end

function MissionBuffsHandler:get_family_buffs_available_for_player(player)
	...
	if #priority_buffs_available <= 0 and #family_buffs_available <= 0 then
...
function MissionBuffsHandler:log_player_data(player)
