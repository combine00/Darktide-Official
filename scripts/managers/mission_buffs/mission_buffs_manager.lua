...
function MissionBuffsManager:_register_rpcs(network_event_delegate)
	if not self._is_server then
		network_event_delegate:register_session_events(self, unpack(CLIENT_RPCS))
	end
end

function MissionBuffsManager:_unregister_rpcs(network_event_delegate)
	if not self._is_server then
		network_event_delegate:unregister_events(unpack(CLIENT_RPCS))
	end
end

function MissionBuffsManager:_register_events()
	for _, event in ipairs(EVENTS) do
		Managers.event:register(self, event[1], event[2])
	end
end

function MissionBuffsManager:_unregister_events()
	for _, event in ipairs(EVENTS) do
		Managers.event:unregister(self, event[1])
	end
end

function MissionBuffsManager:init(is_server, game_mode, game_mode_name, network_event_delegate)
...
function MissionBuffsManager:destroy()
...
function MissionBuffsManager:check_if_all_players_chosen_family()
	self._mission_buffs_handler:check_if_all_players_chosen_family()
end

function MissionBuffsManager:does_player_have_buff_saved(player, buff_name)
	return self._mission_buffs_handler:does_player_have_buff_saved(player, buff_name)
end

function MissionBuffsManager:try_show_new_ui_notification()
	self._mission_buffs_ui_manager:try_show_new_ui_notification()
end

function MissionBuffsManager:get_buffs_to_exclude()
	return self._backend_buffs_to_exclude
end

function MissionBuffsManager:get_buff_family_selected_by_player(player)
	return self._mission_buffs_handler:get_buff_family_selected_by_player(player)
end

function MissionBuffsManager:cb_backend_buffs_to_exclude_received(buffs_to_exclude)
...
function MissionBuffsManager:cb_backend_buffs_to_exclude_failed(err)
...
function MissionBuffsManager:give_player_silent_buff_not_saved_to_player_data(player, buff_name)
	self._mission_buffs_handler:give_buff_to_player(player, buff_name, true, true)
end

function MissionBuffsManager:check_catchup_for_new_player(player, override_waves_completed_num)
...
function MissionBuffsManager:_get_catchup_buff_counts_for_player(player, waves_completed)
...
function MissionBuffsManager._get_num_buffs_for_waves_completed(waves_completed, waves_list)
	local num_legendary_buffs = 0

	for i = 1, #waves_list do
		if waves_list[i] <= waves_completed then
...
function MissionBuffsManager:_manage_delayed_data_initialization_for_players()
...
function MissionBuffsManager:_manage_player_spawn(player, is_respawn)
...
function MissionBuffsManager:_request_specific_buff(buff_name, target_self)
	if target_self then
...
function MissionBuffsManager:_request_buff_family_choice(num_choices)
...
function MissionBuffsManager:_request_family_buff_for_all()
	local is_server_or_host = self:_is_server_or_host()
...
function MissionBuffsManager:_request_legendary_buff_choice(wave_num, num_choices)
	local is_server_or_host = self:_is_server_or_host()
...
function MissionBuffsManager:rpc_client_mission_buffs_buff_choices_received(channel_id, choices_network_id_array, is_buff_family_choice, wave_num)
...
function MissionBuffsManager:rpc_client_mission_buffs_buff_received(channel_id, buff_template_id, wave_num)
	...
	local buff_family_source_id = nil
...
function MissionBuffsManager:rpc_client_mission_buffs_family_received(channel_id, buff_family_id)
...
function MissionBuffsManager:_request_buff_for_all(buff_name)
	if self:_is_hosting_client() then
		self._mission_buffs_handler:give_buff_to_all(buff_name)
	else
		local buff_template_id = self:_is_client() and NetworkLookup.buff_templates[buff_name]
	end
end

function MissionBuffsManager:_request_buff_for_self(buff_name)
	if self:_is_hosting_client() then
		local local_player = Managers.player:local_player(1)
...
function MissionBuffsManager:notify_buff_given_to_player(player, buff_name)
	local buff_template_id = NetworkLookup.buff_templates[buff_name]
...
function MissionBuffsManager:_notify_buff_family_given_to_player(player, buff_family_name)
...
function MissionBuffsManager:_add_externally_controlled_buff_to_player(player, buff_name)
...
function MissionBuffsManager:send_choice_options_to_player(player, current_choice)
	local is_player_hosting_client = self._is_hosting_player(player)
...
function MissionBuffsManager:_notify_server_buff_choice(choice_index, is_random_selection)
...
function MissionBuffsManager._notify_ui(context)
	Managers.event:trigger("event_mission_buffs_update_presentation", context)
end

function MissionBuffsManager:_is_server_or_host()
	return self._is_server
end

function MissionBuffsManager:_is_hosting_client()
	return not DEDICATED_SERVER and self._is_server
end

function MissionBuffsManager._is_hosting_player(player)
...
function MissionBuffsManager:_is_client()
	return not self._is_server
end

function MissionBuffsManager._get_local_player_peer_and_local_id()
	local local_player = Managers.player:local_player(1)
...
function MissionBuffsManager._convert_buff_name_to_template_id(buff_name)
	return NetworkLookup.buff_templates[buff_name]
end

function MissionBuffsManager._convert_buff_template_id_to_name(buff_template_id)
	return NetworkLookup.buff_templates[buff_template_id]
end

function MissionBuffsManager._convert_options_names_array_to_network_ids(options_names_array, is_buff_family_choice)
...
function MissionBuffsManager._convert_options_network_id_array_to_names(buff_template_ids_array, is_buff_family_choice)
	local network_lookup = is_buff_family_choice and NetworkLookup.hordes_build_families or NetworkLookup.buff_templates
