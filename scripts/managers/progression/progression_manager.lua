...
function ProgressionManager:_parse_report(eor, account_wallets)
	local player = Managers.player:player(Network.peer_id(), 1)
	...
	local is_owner, rank_played = nil
...
function ProgressionManager:_get_mastery_rewards(account_data)
...
function ProgressionManager:_has_won_mission(account_data)
...
function ProgressionManager:_get_havoc_order_rewards(account_data, havoc_data, orders, havoc_settings, is_owner, rank_played)
	...
	if not received_new_order then
		...
		elseif not round_won and not is_owner and rank_played < havoc_session.current.rank then
			havoc_session.previous.charges = math.clamp(havoc_session.current.charges, min_charges, max_charges)
		elseif not round_won and min_charges <= havoc_session.current.charges and havoc_session.current.charges < max_charges then
...
ProgressionManager:_get_havoc_order_rewards ended up in a state unaccounted for:
havoc_info: %s
havoc_session: %s
round_won: %s
reward_cards: %s
...
function ProgressionManager:_get_havoc_highest_rank(account_data)
	local round_won = self:_has_won_mission(account_data)
...
function ProgressionManager:_get_havoc_week_rank(account_data)
	local round_won = self:_has_won_mission(account_data)
...
function ProgressionManager:_parse_reward_cards(account_data, item_rewards)
	local reward_cards = account_data.rewardCards
...
function ProgressionManager:_check_level_up(stats, item_rewards)
	if self._session_report_is_dummy then
...
function ProgressionManager:_add_unlocked_weapons_to_card(reward_card, archetype_name, target_level)
...
function ProgressionManager:_level_up(stats, target_level, item_rewards)
...
function ProgressionManager:_parse_level_up_rewards(reward_card, type, data, item_rewards)
...
function ProgressionManager:check_level_up_pending()
...
function ProgressionManager:_have_level_up_pending(stats)
	local needed_xp_for_next_level = stats.neededXpForNextLevel
...
function ProgressionManager:_do_level_up_pending(stats)
	local current_level = stats.currentLevel
...
function ProgressionManager:_refresh_profiles()
