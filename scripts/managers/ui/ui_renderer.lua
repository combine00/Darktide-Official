...
function UIRenderer.create_resource_renderer(world, gui, gui_retained, reference_name, material_name, optional_width, optional_height, ignore_back_buffer)
...
function UIRenderer:video_player(reference_name)
	local video_players = self.video_players
	local video_player = video_players[reference_name]
...
function UIRenderer:text_size(text, font_type, font_size, optional_size, options, use_max_extents)
	local font_data = UIFonts.data_by_type(font_type)
	local font = font_data.path
	local flags = font_data.render_flags or 0
	optional_size = optional_size and Vector2(optional_size[1], optional_size[2])
...
function UIRenderer:crop_text_width(text, font_type, font_size, max_width, crop_from_left, options, use_max_extents)
	local scale = self.scale or 1
