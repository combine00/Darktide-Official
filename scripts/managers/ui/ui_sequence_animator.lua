...
function UISequenceAnimator:init(ui_scenegraph, scenegraph_definition, animation_definitions)
...
function UISequenceAnimator:start_animation(parent, animation_sequence_name, widgets, params, speed, callback, delay)
	delay = delay or 0
...
function UISequenceAnimator:update(dt, t)
...
function UISequenceAnimator:complete_animation(animation_id)
	local animation_definitions = self._animation_definitions
	local animation = self._active_animations[animation_id]
	animation.force_complete = true
end

function UISequenceAnimator:is_animation_active(animation_id)
	return self._active_animations[animation_id] ~= nil
end

function UISequenceAnimator:is_animation_completed(animation_id)
	return self._active_animations[animation_id] == nil
end

function UISequenceAnimator:stop_animation(animation_id)
