...
function UIResolution.width_fragments()
	return NUM_SCREEN_FRAGMENTS_W
end

function UIResolution.height_fragments()
	return NUM_SCREEN_FRAGMENTS_H
end

function UIResolution.save(width, height)
	Application.set_user_setting("screen_resolution", 1, width)
	Application.set_user_setting("screen_resolution", 2, height)
	Application.save_user_settings()
end

function UIResolution.scale_vector(position, scale, pixel_snap, use_real_coordinates)
...
function UIResolution.scale_lua_vector2(position, scale, pixel_snap)
	local pos_x = position[1] > 0 and math_max(position[1] * scale, 1) or position[1] * scale
	local pos_y = position[2] > 0 and math_max(position[2] * scale, 1) or position[2] * scale
...
function UIResolution.scale_vector3(position, scale, pixel_snap)
...
function UIResolution.inverse_scale_vector(position, inverse_scale, pixel_snap)
	if pixel_snap then
