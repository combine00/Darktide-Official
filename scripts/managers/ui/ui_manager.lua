...
function UIManager:update_client_loadout_waiting_state(state)
	self._client_waiting_loadout = state
end

function UIManager:get_client_loadout_waiting_state()
...
function UIManager:_setup_icon_renderers()
	...
	local back_buffer_render_handlers = {
		...
		companion = self:create_single_icon_renderer("weapon", "companion", companion_render_settings),
		icon = self:create_single_icon_renderer("icon", "icon")
	}
	self._back_buffer_render_handlers = back_buffer_render_handlers
end

function UIManager:create_single_icon_renderer(render_type, id, settings)
...
function UIManager:destroy_single_icon_renderer(id)
	local single_icon_renderers = self._single_icon_renderers
...
function UIManager:_handle_single_icon_renderer_destruction(dt)
...
function UIManager:create_renderer(name, world, create_resource_target, gui, gui_retained, material_name, optional_width, optional_height, ignore_back_buffer)
...
function UIManager:_on_ui_element_package_loaded(reference_key, reference_name, package_id)
...
function UIManager:input_service(optional_service_name)
...
function UIManager:start_tracking_input_hold(action_name, time_completed, cb_completed)
	return self._input_hold_tracker:start_tracking(action_name, time_completed, cb_completed)
end

function UIManager:stop_tracking_input_hold(tracking_id)
...
function UIManager:active_top_view()
	return self._view_handler:active_top_view()
end

function UIManager:view_active_data(view_name)
...
function UIManager:create_viewport(world, viewport_name, viewport_type, viewport_layer, shading_environment_name, shading_callback, optional_render_targets)
...
function UIManager:using_input(ignore_hud, ignore_views, ignore_constant_elements)
...
function UIManager:emote_wheel_active()
	local hud = self._hud

	return hud and hud:emote_wheel_active()
end

function UIManager:emote_wheel_wants_camera_control()
	local hud = self._hud
...
function UIManager:event_pause_popup_input(popup_id, paused)
...
function UIManager:event_remove_ui_popups_by_priority(priority)
	local active_popups = self._active_popups
...
function UIManager:load_profile_portrait(profile, cb, render_context, unload_cb)
...
function UIManager:event_portrait_render_change(value)
	local instance = self._back_buffer_render_handlers.portraits
...
function UIManager:load_item_icon(real_item, cb, render_context, dummy_profile, prioritize, unload_cb)
...
function UIManager:update_item_icon_priority(id)
	local weapons_instance = self._back_buffer_render_handlers.weapons
	local weapon_skin_instance = self._back_buffer_render_handlers.weapon_skin
	local companion_instance = self._back_buffer_render_handlers.companion
	local cosmetics_instance = self._back_buffer_render_handlers.cosmetics
...
function UIManager:increment_item_icon_load_by_existing_id(id)
	local weapons_instance = self._back_buffer_render_handlers.weapons
	local weapon_skin_instance = self._back_buffer_render_handlers.weapon_skin
	local companion_instance = self._back_buffer_render_handlers.companion
	local cosmetics_instance = self._back_buffer_render_handlers.cosmetics
...
function UIManager:event_cinematic_skip_state(show_skip, can_skip)
...
function UIManager:cinematic_skip_state()
...
function UIManager:release_packages()
