...
function SocialService:on_profile_signed_in()
...
function SocialService:is_in_training_grounds()
...
function SocialService:is_in_singleplay()
	local host_type = Managers.connection:host_type()
...
function SocialService:fetch_friend_invites(force_update)
...
function SocialService:fetch_blocked_accounts(force_update, skip_platform)
...
function SocialService:fetch_blocked_players(force_update)
...
function SocialService:local_player_is_joinable()
	local activity_id = Managers.presence:presence()
	local presence_settings = PresenceSettings.settings[activity_id]
...
function SocialService:_check_split_party()
...
function SocialService:_check_cross_play_invite(other_player_info)
...
function SocialService:_check_cross_play_join(other_player_info)
	local platform_myself = self:platform()
	local platform_other = other_player_info:platform()
	local presence_myself = Managers.presence:presence_entry_myself()
...
function SocialService:_get_kick_voting_template(player_info)
...
function SocialService:_fetch_fatshark_friends(force_update)
...
function SocialService:_update_blocked_players(blocked_accounts, platform_blocked_accounts)
...
function SocialService:get_player_info_by_account_id(account_id)
	local players_by_account_id = self._players_by_account_id
	local player_info = players_by_account_id[account_id]
...
function SocialService:_event_friend_invite(data)
	self._friend_invites_has_changed = true
end

function SocialService:_event_friend_invite_accepted(data)
	self._friend_invites_has_changed = true
	self._friends_list_has_changed = true
end

function SocialService:_event_friend_removed(data)
	self._friends_list_has_changed = true
end
...
function SocialService:_event_on_social_blocklist_update()
	self:fetch_blocked_players()
end

function SocialService:_update_platform_players(platform_accounts, account_type)
	local players_by_platform_user_id = self._players_by_platform_user_id
...
function SocialService:_update_player_info_platform_information(player_info)
	local players_by_platform_user_id = self._players_by_platform_user_id
...
function SocialService:get_fatshark_id()
	return self._backend_interfaces:get_fatshark_id()
end

function SocialService:get_player_info_by_fatshark_id(fatshark_id)
...
function SocialService:_remove_friend_request_if_friends_or_blocked(player_info)
...
function SocialService:platform_unfriend_reason_for_disabled()
	local platform = self._platform
