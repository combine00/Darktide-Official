...
function Teams:init(team_settings)
	local teams = {}
	self._teams = teams
...
function Teams:has_space_in_any_team_for_peers(peer_ids)
...
function Teams:add_peers_to_any_team(peer_ids)
...
function Teams:remove_peer_from_team(peer_id)
	local team = self._peer_to_team_lookup[peer_id]

	self:_remove_peer_from_team(peer_id, team)
end

function Teams:team_name_from_peer_id(peer_id)
	local team = self._peer_to_team_lookup[peer_id]

	return team:name()
end

function Teams:team_name_from_team_id(team_id)
	local team = self._teams[team_id]

	return team:name()
end

function Teams:_add_peer_to_team(peer_id, team)
	self._peer_to_team_lookup[peer_id] = team

	team:add_peer(peer_id)
end

function Teams:_remove_peer_from_team(peer_id, team)
