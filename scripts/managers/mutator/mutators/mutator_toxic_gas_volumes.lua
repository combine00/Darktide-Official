...
function MutatorToxicGasVolumes:init(is_server, network_event_delegate, mutator_template, nav_world, level_seed)
...
function MutatorToxicGasVolumes:_activate_corals(cloud, coral_event_type)
...
function MutatorToxicGasVolumes:_deactivate_corals(cloud, coral_event_type)
	if not cloud.corals then
		return
	end
...
function MutatorToxicGasVolumes:_start_fading_cloud(cloud, fade_time)
...
function MutatorToxicGasVolumes:_activate_cloud(cloud)
...
function MutatorToxicGasVolumes:_deactivate_cloud(cloud)
...
function MutatorToxicGasVolumes:_activate_alternating_cloud(cloud, optional_activating_timer, timer)
	cloud.active = true
...
function MutatorToxicGasVolumes:_update_activating_cloud(dt, cloud, timer)
...
function MutatorToxicGasVolumes:_activate_activating_cloud(cloud, timer)
	cloud.fog_component:set_volume_enabled(true)
...
function MutatorToxicGasVolumes:on_spawn_points_generated(level, themes)
...
function MutatorToxicGasVolumes:_get_clouds(gas_clouds)
...
function MutatorToxicGasVolumes:_get_corals(corals)
	local component_system = Managers.state.extension:system("component_system")
...
function MutatorToxicGasVolumes:_setup_static_clouds()
...
function MutatorToxicGasVolumes:_setup_alternating_clouds()
	local gas_clouds = self._gas_clouds
	local num_sections = self:_get_clouds(gas_clouds)
...
function MutatorToxicGasVolumes:_update_static_clouds(dt, t)
	local main_path_manager = Managers.state.main_path
...
function MutatorToxicGasVolumes:_update_alternating_clouds(dt, t)
	local main_path_manager = Managers.state.main_path
	local _, ahead_travel_distance = main_path_manager:ahead_unit(TARGET_SIDE_ID)
...
function MutatorToxicGasVolumes:_random(...)
...
function MutatorToxicGasVolumes:update(dt, t)
	if not self._is_server then
		return
	end
