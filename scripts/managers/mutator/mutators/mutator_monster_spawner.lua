...
local _calculate_positions = nil
local TARGET_SIDE_ID = 1

function MutatorMonsterSpawner:init(is_server, network_event_delegate, mutator_template, nav_world)
...
function MutatorMonsterSpawner:destroy()
...
function MutatorMonsterSpawner:on_spawn_points_generated(level, themes)
	...
	if self._valid_spawn_points < self._num_to_spawn then
...
function MutatorMonsterSpawner:update(dt, t)
	if not self._is_server then
		return
	end
...
function MutatorMonsterSpawner:_calculate_probabillity(optional_probabillity_reroll, remove_chance)
	if not self._chance_initialized then
		...
		local weights = self._section_probabillity
		local add_to_other_probabillites = nil
...
function MutatorMonsterSpawner:_add_clean_spawn()
	local monsters = {}
	local spawn_point_sections = self._spawn_point_sections
...
function MutatorMonsterSpawner:_spawn_monster(monster, ahead_target_unit, side_id)
