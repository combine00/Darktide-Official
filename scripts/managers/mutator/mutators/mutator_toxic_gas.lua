...
function MutatorToxicGas:init(is_server, network_event_delegate, mutator_template, nav_world)
...
function MutatorToxicGas:_activate_cloud(cloud)
...
function MutatorToxicGas:_deactivate_cloud(cloud)
...
function MutatorToxicGas:update(dt, t)
	if not self._is_server then
		return
	end
