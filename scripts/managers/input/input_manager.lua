...
function InputManager:load_input_layout(layout_name)
	local devices = {
		{
			"ps4_controller"
		},
...
function InputManager:_update_wwise_rumble()
...
function InputManager:start_suppress_wwise_rumble()
	if DEDICATED_SERVER then
		return
	end
...
function InputManager:stop_suppress_wwise_rumble()
	if DEDICATED_SERVER then
		return
	end
