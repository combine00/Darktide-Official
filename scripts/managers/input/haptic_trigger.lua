...
function HapticTrigger:init(trigger_side)
...
function HapticTrigger:update(dt, t)
	self:_update_vibration(dt, t)
end

function HapticTrigger:_update_vibration(dt, t)
...
function HapticTrigger:set_ammo_count_percentage(ammo_percentage)
	self._ammo_count_percentage = ammo_percentage
end

function HapticTrigger:trigger_vibration(frequency)
...
function HapticTrigger:stop_vibration()
	if not self._haptics_enabled then
		return
	end
...
function HapticTrigger:set_haptic_trigger_template(haptic_trigger_template)
	self._haptic_trigger_template = haptic_trigger_template

	self:_apply_current_template()
end

function HapticTrigger:_apply_current_template()
	local haptic_trigger_template = self._haptic_trigger_template
...
function HapticTrigger:unwield()
	self._ammo_count_percentage = nil

	self:_set_no_effect()
end

function HapticTrigger:destroy()
	self._ammo_count_percentage = nil

	self:_set_no_effect()
end

function HapticTrigger:suppress_haptics(suppressed)
...
function HapticTrigger:_set_feedback_effect(settings_type, params)
	local resistance_multiplier = self._resistance_multiplier[settings_type] or DEFAULT_RESISTANCE_MULTIPLIER
...
function HapticTrigger:_set_multi_position_feedback_effect(settings_type, params)
	local resistance_multiplier = self._resistance_multiplier[settings_type] or DEFAULT_RESISTANCE_MULTIPLIER
...
function HapticTrigger:_set_slope_feedback_effect(settings_type, params)
	local resistance_multiplier = self._resistance_multiplier[settings_type] or DEFAULT_RESISTANCE_MULTIPLIER
...
function HapticTrigger:_set_vibration_effect(settings_type, position, frequency, amplitude)
...
function HapticTrigger:_set_multi_position_vibration_effect(settings_type, frequency, amplitude)
	local vibration_multiplier = self._vibration_multiplier[settings_type] or DEFAULT_VIBRATION_MULTIPLIER
