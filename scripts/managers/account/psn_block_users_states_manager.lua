...
function PsnBlockUsersStatesManager:init()
...
function PsnBlockUsersStatesManager:update(dt, t)
	...
	if MAX_TIME_INTERVAL <= self._current_time then
		self._current_calls = 0
		self._current_time = 0
		self._current_waiting_time = 0
	end
...
function PsnBlockUsersStatesManager:set_account_id(account_id)
	self._account_id = account_id
end

function PsnBlockUsersStatesManager:set_wait_to_collect_accounts(new_value)
	self._wait_to_collect_accounts = new_value
end

function PsnBlockUsersStatesManager:is_blocking_me(account_id)
	return self._blocked_by[account_id] or false
end

function PsnBlockUsersStatesManager:add_requested_account(account_id)
...
function PsnBlockUsersStatesManager:is_platform_id_already_requested(platform_id)
	return self._already_requested_account[platform_id] or false
end

function PsnBlockUsersStatesManager:fetch_block_users_states(account_ids, result_promise, result_data_table)
...
function PsnBlockUsersStatesManager:_calculate_waiting_time()
...
function PsnBlockUsersStatesManager:_get_platform_token()
...
function PsnBlockUsersStatesManager:_print_update_debug_information()
...
function PsnBlockUsersStatesManager:_print_waiting_time_debug_information(remaining_time, remaining_calls, max_delay, waiting_time)
	if DevParameters.debug_print_ps5_block_users_states then
