...
function PlayerSessionPSN.join_session(web_api, user_id, session_id)
	return Managers.party_immaterium:get_your_standing_invite_code():next(function (party_id_with_invite_code)
		local push_context_id = WebApi.create_push_context(user_id)
		local invite_code_base64 = string.encode_base64(party_id_with_invite_code)
		local content = cjson.encode({
...
function PlayerSessionPSN.leave_session(web_api, user_id, session_id)
	local api_group = "sessionManager"
...
function PlayerSessionPSN.is_session_leader(web_api, user_id, session_id)
	local api_group = "sessionManager"
...
function PlayerSessionPSN.update_session_max_players(web_api, user_id, session_id, max_players)
	local api_group = "sessionManager"
...
function PlayerSessionPSN.update_session_join_disabled(web_api, user_id, session_id, disabled)
	local api_group = "sessionManager"
	local path = string.format("/v1/playerSessions/%s", session_id)
	local method = WebApi.PATCH
	local content = cjson.encode({
...
function PlayerSessionPSN.update_session_privacy(web_api, user_id, session_id, is_private)
	local api_group = "sessionManager"
	local path = string.format("/v1/playerSessions/%s", session_id)
	local method = WebApi.PATCH
...
function PlayerSessionPSN.get_session_invite_code(web_api, user_id, session_id)
	local api_group = "sessionManager"
...
function PlayerSessionPSN.send_session_invite(web_api, user_id, session_id, invitee_account_id)
	local content = cjson.encode({
		invitations = {
			{
				to = {
