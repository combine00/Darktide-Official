...
function GameModeSurvival:init(game_mode_context, game_mode_name, network_event_delegate)
...
function GameModeSurvival:hot_join_sync(sender, channel)
...
function GameModeSurvival:client_update(dt, t)
	GameModeSurvival.super.client_update(self, dt, t)
end

function GameModeSurvival:server_update(dt, t)
	GameModeSurvival.super.server_update(self, dt, t)
end

function GameModeSurvival:destroy()
...
function GameModeSurvival:_game_mode_state_changed(new_state, old_state)
...
function GameModeSurvival:get_game_progression_percentage()
...
function GameModeSurvival:evaluate_end_conditions()
...
function GameModeSurvival:_gamemode_complete(result, reason)
...
function GameModeSurvival:rpc_fetch_session_report()
	self:_fetch_session_report()
end

function GameModeSurvival:_fetch_session_report()
...
function GameModeSurvival:_all_players_disabled(num_alive_players, alive_players, include_bots)
...
function GameModeSurvival:_all_players_dead(num_alive_players, alive_players, include_bots)
	for i = 1, num_alive_players do
		local player = alive_players[i]
		local player_unit = player.player_unit
		local unit_data_extension = ScriptUnit.has_extension(player_unit, "unit_data_system")
...
function GameModeSurvival:_failure_conditions_met()
...
function GameModeSurvival:in_safe_volume(volume_unit)
...
function GameModeSurvival:_restrict_spawning(enabled)
...
function GameModeSurvival:_set_pacing(enabled)
	self._pacing_enabled = enabled

	Managers.state.pacing:set_enabled(self._pacing_enabled)
end

function GameModeSurvival:complete(reason)
	self._completed = true
	self._end_reason = reason
end

function GameModeSurvival:fail(reason)
	self._failed = true
	self._end_reason = reason
end

function GameModeSurvival:_init_buff_system(game_mode_name, network_event_delegate)
	self._mission_buffs_manager = MissionBuffsManager:new(self._is_server, self, game_mode_name, network_event_delegate)
end

function GameModeSurvival:_destroy_buff_system(game_mode_name, network_event_delegate)
	self._mission_buffs_manager:destroy()

	self._mission_buffs_manager = nil
end

function GameModeSurvival:on_island_entered(island_name)
	Log.info("GameModeSurvival", string.format("Players entered island %s", island_name))

	self._current_island = island_name
end

function GameModeSurvival:on_island_completed()
...
function GameModeSurvival:on_mcguffin_picked_up()
	if self._is_server then
...
function GameModeSurvival:on_mcguffin_returned()
	if self._is_server then
		Managers.stats:record_team("hook_game_mode_mcguffins_returned")
	end
end

function GameModeSurvival:on_wave_started(wave_num)
...
function GameModeSurvival:on_objective_completed(wave_num)
...
function GameModeSurvival:_tag_remaining_enemies()
	if self._is_server then
	...
	if not DEDICATED_SERVER then
		...
		if outline_system then
			...
			local _ = side:relation_sides("allied")
			local target_units = side:alive_units_by_tag("allied", "minion")
...
function GameModeSurvival:on_wave_completed(wave_num)
	Log.info("GameModeSurvival", string.format("Wave %d completed event received", wave_num))

	local show_progression_notification = nil
...
function GameModeSurvival:_handle_horde_game_start()
...
function GameModeSurvival:_horde_game_completed_server()
...
function GameModeSurvival:_handle_game_end_server(game_won)
...
function GameModeSurvival:_give_game_end_damage_immunity_buff()
	local players = Managers.player:human_players()
...
function GameModeSurvival:_on_minion_unit_spawned(unit)
...
function GameModeSurvival:_handle_giving_buffs_for_wave(wave_num)
...
function GameModeSurvival:_update_wave_progression(wave_num, started_wave)
...
function GameModeSurvival:_handle_progression_notification(wave_num, started_wave, show_wave_progression_notification)
...
function GameModeSurvival:_is_wave_progress_data_old(wave_num, started_wave)
	if started_wave then
...
function GameModeSurvival:_trigger_wave_started_ui_notification()
...
function GameModeSurvival:_trigger_wave_ended_ui_notification()
	self._mission_buffs_manager._mission_buffs_ui_manager:queue_wave_end_notification_ui(self._waves_completed)
end

function GameModeSurvival:get_islands_completed()
	return self._islands_completed
end

function GameModeSurvival:can_start_wave_one()
	local level = Managers.state.mission:mission_level()

	Level.trigger_event(level, "hordes_players_ready_for_start")
end

function GameModeSurvival:wait_for_players_to_choose_family()
	local level = Managers.state.mission:mission_level()

	Level.trigger_event(level, "hordes_wait_for_players_choose_family")
end

function GameModeSurvival:is_objective_completed_for_wave(wave_num)
	local target_objective = string.format("wave_%d_objective_completed", wave_num)

	return not not self._objectives_completed[target_objective]
end

function GameModeSurvival:get_total_waves_completed()
	local total_waves_completed = self._islands_completed * HordesModeSettings.waves_per_island + self._waves_completed

	return total_waves_completed
end

function GameModeSurvival:get_waves_completed()
	local current_waves_completed = self._waves_completed or 3

	return current_waves_completed
end

function GameModeSurvival:get_last_wave_completed()
	return self._waves_completed
end

function GameModeSurvival:get_current_wave()
	return self._current_wave
end

function GameModeSurvival:is_wave_in_progress()
	return self._is_wave_in_progress
end

function GameModeSurvival:get_minion_health_modifier(breed)
...
function GameModeSurvival:on_player_unit_spawn(player, unit, is_respawn)
...
function GameModeSurvival:on_player_unit_despawn(player)
...
function GameModeSurvival:_store_persistent_player_data(player)
...
function GameModeSurvival:_apply_persistent_player_data(player)
...
function GameModeSurvival:should_spawn_dead(player)
	if player:is_human_controlled() then
		local account_id = _player_account_id(player)
...
function GameModeSurvival:player_time_until_spawn(player)
	local unique_id = player:unique_id()

	return self._players_respawn_time[unique_id]
end

function GameModeSurvival:_apply_respawn_penalties(player)
	local respawn_settings = self._settings.respawn
	local player_unit = player.player_unit
...
function GameModeSurvival:can_spawn_player(player)
...
function GameModeSurvival:_set_ready_time_to_spawn(player, time)
	local unique_id = player:unique_id()
...
function GameModeSurvival:rpc_set_player_respawn_time(channel_id, peer_id, local_player_id, time)
...
function GameModeSurvival:rpc_client_hordes_set_progress_data(channel_id, current_wave, waves_completed, wave_in_progress)
...
function GameModeSurvival:rpc_client_hordes_wave_completed(channel_id, wave_num)
	Log.info("GameModeSurvival", "Server called wave completed. Waves Completed: %d", wave_num)

	if self._waves_completed < wave_num then
		self:on_wave_completed(wave_num)
	end
end

function GameModeSurvival:rpc_client_hordes_show_wave_completed_notification(channel_id, wave_num)
	Log.info("GameModeSurvival", "Server called show wave completed notification. Waves Completed: %d", wave_num)
	self:_trigger_wave_ended_ui_notification()
end

function GameModeSurvival:rpc_client_hordes_tag_remaining_enemies(channel_id)
	self:_tag_remaining_enemies()
end

function GameModeSurvival:get_additional_pickups()
