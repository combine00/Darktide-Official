...
function AchievementsManager:init(is_client, event_delegate, use_batched_saving, broadcast_unlocks)
...
function AchievementsManager:_setup_achievements(backend_data)
...
function AchievementsManager:achievement_definition(achievement_id)
	return self._definitions[achievement_id]
end

function AchievementsManager:achievement_definitions()
	return self._definitions
end

function AchievementsManager:_achievement_completed(player_id, achievement_id)
...
function AchievementsManager:achievement_completed(player, achievement_id)
...
function AchievementsManager:_track_achievements(player_id)
	local player_data = self._players[player_id]
...
function AchievementsManager:_untrack_achievement(player_id, achievement_id)
...
function AchievementsManager:_untrack_achievements(player_id)
	local player_data = self._players[player_id]
...
function AchievementsManager:_setup_platform()
	local definitions = self._definitions
...
function AchievementsManager:add_player(player)
	local player_id = player.remote and player.stat_id or player:local_player_id()
...
function AchievementsManager:_get_player_diff(player_id)
	local player_data = self._players[player_id]
...
function AchievementsManager:_save_succeded(player_data, assumed_complete)
...
function AchievementsManager:_save_failed(player_data, ...)
	local player_id = player_data.id
...
function AchievementsManager:_save_player_diff(player_id)
...
function AchievementsManager:_player_is_dirty(player_id)
	local player_data = self._players[player_id]

	return player_data and player_data.dirty
end

function AchievementsManager:_update_dirty_player(player_id)
	local player_data = self._players[player_id]
...
function AchievementsManager:update(dt, t)
	for player_id, _ in pairs(self._players) do
		self:_update_dirty_player(player_id)
	end
end

function AchievementsManager:has_player(player_id)
	return self._players[player_id] ~= nil
end

function AchievementsManager:_clear_platform()
...
function AchievementsManager:remove_player(player_id)
	local player_data = self._players[player_id]
...
function AchievementsManager:remove_all()
	for player_id, _ in pairs(self._players) do
...
function AchievementsManager:_became_host()
	for player_id, _ in pairs(self._players) do
		self:_track_achievenments(player_id)
	end
end

function AchievementsManager:_stopped_being_host()
	for player_id, _ in pairs(self._players) do
		self:_untrack_achievements(player_id)
	end
end

function AchievementsManager:set_host(is_host)
...
function AchievementsManager:_fetch_unclaimed_penance_rewards()
...
function AchievementsManager:_fetch_penance_track_account_state()
	local backend_interface = Managers.backend.interfaces
	...
	return Promise.all(unpack(promises)):next(function (responses)
		...
		if current_rewardable_tier > rewarded_tiers then
			return true
		end
...
function AchievementsManager:_show_item_rewards(rewards)
...
function AchievementsManager:_advertise_unlocked_achievement(player_id, achievement_id)
	local player_data = self._players[player_id]
...
function AchievementsManager:_unlock_achievement(player_id, achievement_id, never_tracked, remote_unlock)
	local player_data = self._players[player_id]
...
function AchievementsManager:unlock_achievement(player, achievement_id, unlock_previous)
	local player_id = player.remote and player.stat_id or player:local_player_id()
	local player_data = self._players[player_id]
...
function AchievementsManager:rpc_unlock_achievement(_, local_player_id, achievement_index)
...
function AchievementsManager:_show_unlock_in_chat(player, achievement_id)
...
function AchievementsManager:rpc_ally_unlocked_achievement(_, peer_id, local_player_id, achievement_index)
...
function AchievementsManager:_on_achievement_unlock(player_id, achievement_id)
	local player_data = self._players[player_id]
...
function AchievementsManager:_on_stat_trigger(listener_id, stat_name, value)
