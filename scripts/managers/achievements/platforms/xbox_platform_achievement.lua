...
function XboxPlatformAchievement:_get_platform_id(achievement_definition)
...
function XboxPlatformAchievement:_should_show_progress(achievement_definition)
	local xbox_data = achievement_definition.xbox
...
function XboxPlatformAchievement:_add_achievement(lookup, platform_data)
...
function XboxPlatformAchievement:init(definitions)
...
function XboxPlatformAchievement:destroy()
	return Promise.resolved()
end

function XboxPlatformAchievement:_set_progress(achievement_definition, progress)
...
function XboxPlatformAchievement:is_platform_achievement(achievement_definition)
	local achievement_id = achievement_definition.id

	return self._progress[achievement_id] ~= nil
end

function XboxPlatformAchievement:unlock(achievement_definition)
	return self:_set_progress(achievement_definition, 100)
end

function XboxPlatformAchievement:is_unlocked(achievement_definition)
	local achievement_id = achievement_definition.id

	return self._progress[achievement_id] == 100
end

function XboxPlatformAchievement:set_progress(achievement_definition, progress)
...
function XboxPlatformAchievement:get_progress(achievement_definition)
	local achievement_id = achievement_definition.id
