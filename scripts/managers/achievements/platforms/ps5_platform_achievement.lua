...
function PS5PlatformAchievement:_get_platform_id(achievement_definition)
	local psn_data = achievement_definition.psn

	return psn_data and psn_data.id
end

function PS5PlatformAchievement:_should_show_progress(achievement_definition)
	local psn_data = achievement_definition.psn

	return psn_data and psn_data.show_progress
end

function PS5PlatformAchievement:_add_achievement(lookup, data)
...
function PS5PlatformAchievement:init(definitions)
...
function PS5PlatformAchievement:destroy()
	return Promise.resolved()
end

function PS5PlatformAchievement:_set_progress(achievement_definition, progress)
...
function PS5PlatformAchievement:is_platform_achievement(achievement_definition)
	local achievement_id = achievement_definition.id

	return self._progress[achievement_id] ~= nil
end

function PS5PlatformAchievement:unlock(achievement_definition)
	return self:_set_progress(achievement_definition, 100)
end

function PS5PlatformAchievement:is_unlocked(achievement_definition)
	local achievement_id = achievement_definition.id

	return self._progress[achievement_id] == 100
end

function PS5PlatformAchievement:set_progress(achievement_definition, progress)
...
function PS5PlatformAchievement:get_progress(achievement_definition)
	local achievement_id = achievement_definition.id
