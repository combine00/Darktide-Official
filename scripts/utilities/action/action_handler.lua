...
function ActionHandler:fixed_update(dt, t, condition_func_params, frame)
	local registered_components = self._registered_components
...
function ActionHandler:_update_action(handler_data, action, dt, t, frame)
	local component = handler_data.component
...
function ActionHandler:_anim_event(action_settings, action, condition_func_params, is_chain_action, running_action)
	...
	if action_settings.anim_event_func then
		anim_event, anim_event_3p = action_settings:anim_event_func(condition_func_params, is_chain_action, running_action)
...
function ActionHandler:action_settings_from_action_input(id, actions, action_input)
	if not actions then
		return nil
	end
...
function ActionHandler:_validate_chain_action(chain_action, t, current_action_t, time_scale, actions, condition_func_params, used_input, running_action_state)
...
function ActionHandler:_update_stop_input(id, handler_data, t, condition_func_params, action_params)
	local running_action = handler_data.running_action
	local has_running_action = running_action ~= nil
