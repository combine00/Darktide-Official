...
function ChainLightning.jump(t, physics_world, source_node, hit_units, broadphase, enemy_side_names, initial_travel_direction, radius, max_angle, close_max_angle, vertical_max_angle, max_z_diff, on_add_func, add_func_context, jump_validation_func)
...
function ChainLightning.is_valid_target(physics_world, source_unit, target_unit, query_position, travel_direction, max_angle, close_max_angle, max_vertical_angle, max_z_diff, jump_validation_func, min_distance)
	if not HEALTH_ALIVE[target_unit] then
		return false, "health"
	end
...
function ChainLightning.max_targets(time_in_action, chain_settings, depth, use_random)
...
function ChainLightning.targeting_parameters(time_in_action, chain_settings, stat_buffs)
...
function ChainLightning.execute_attack(target_unit, attacking_unit, power_level, charge_level, target_index, target_number, attack_direction_or_nil, damage_profile, damage_type, is_critical_strike, source_item)
