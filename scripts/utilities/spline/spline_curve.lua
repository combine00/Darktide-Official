...
function SplineCurve:init(points, class_name, movement_class, name, subdivisions)
	self._t = 0
	self._name = name
	local splines = {}
...
function SplineCurve:recalc_splines(points, name)
	self._name = name
...
function SplineCurve:get_travel_dist_to_spline_point(point_index)
...
function SplineCurve:get_point_at_distance(dist)
	local spline_points = self._splines
	local spline_class = self._spline_class
	local travel_dist = 0
