...
function Explosion.create_explosion(world, physics_world, source_position, optional_impact_normal, attacking_unit, explosion_template, power_level, charge_level, attack_type, is_critical_strike, ignore_cover, item_or_nil, origin_slot_or_nil, optional_hit_units_table, optional_attacking_unit_owner_unit, optional_apply_owner_buffs, predicted)
	...
	if not USE_PHYSICS_OVERLAP then
		...
		num_hits = broadphase:query(source_position, radius, broadphase_results, broadphase_categories)

		if USE_SEPERATE_QUERY_FOR_DESTRUCTIBLES and close_radius > 0 and target_destructibles then
			destructibles_num_hits = broadphase:query(source_position, close_radius, destructible_broadphase_results, destructibles_only_filter)
		end
	end
...
function Explosion.lerp_values(attacking_unit, explosion_template_name_or_nil, override_action_name_or_nil)
...
function Explosion.radii(explosion_template, charge_level, lerp_values, attack_type, attacker_stat_buffs, attacker_breed_or_nil)
