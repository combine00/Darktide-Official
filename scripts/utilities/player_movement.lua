...
function PlayerMovement.teleport(player, position, rotation)
...
function PlayerMovement.teleport_fixed_update(player_unit, position, rotation)
	PlayerMovement._teleport(player_unit, position, rotation)
end

function PlayerMovement._teleport_callback(player_unit, boxed_position, boxed_rotation)
	if ALIVE[player_unit] then
		PlayerMovement._teleport(player_unit, boxed_position:unbox(), boxed_rotation and boxed_rotation:unbox() or nil)
	end
end

function PlayerMovement._teleport(player_unit, position, rotation)
...
function PlayerMovement.calculate_absolute_rotation_position(parent_unit, relative_rotation, relative_position)
...
function PlayerMovement.calculate_absolute_position(parent_unit, relative_position)
	local parent_pose = Unit.world_pose(parent_unit, 1)
...
function PlayerMovement.calculate_absolute_rotation(parent_unit, relative_rotation)
	local parent_pose = Unit.world_pose(parent_unit, 1)
...
function PlayerMovement.calculate_relative_rotation_position(parent_unit, absolute_rotation, absolute_position)
...
function PlayerMovement.calculate_relative_position(parent_unit, absolute_position)
	local inverted_parent_pose = Matrix4x4.inverse(Unit.world_pose(parent_unit, 1))
...
function PlayerMovement.calculate_relative_rotation(parent_unit, absolute_rotation)
	local inverted_parent_pose = Matrix4x4.inverse(Unit.world_pose(parent_unit, 1))
...
function PlayerMovement.calculate_final_unit_rotation(current_rotation, target_rotation, forward_rotation, t, is_husk)
