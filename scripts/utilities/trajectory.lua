...
function Trajectory.check_trajectory_collisions(physics_world, from_position, target_position, gravity, projectile_speed, angle, sections, collision_filter, time_in_flight, debug_draw, optional_radius, optional_relax_distance)
	local to_target = target_position - from_position
	local to_target_flat = Vector3.normalize(Vector3.flat(to_target))
...
function Trajectory.sphere_sweep_collision_check(physics_world, start_position, flat_direction, x_vel_0, y_vel_0, gravity, radius, collision_filter, start_t, end_t, debug_draw)
