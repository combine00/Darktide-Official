...
local _get_display_data_from_path, _calculate_weapon_statistics, _resolve_damage_template_lerps, _resolve_explosion_template_lerps, _get_weapon_stats = nil

local function _resolve_stat_path(table_tree, path, start_idx, end_idx)
	local resolved_value = table_tree

	for ii = start_idx, end_idx do
		if not resolved_value then
			return nil
		end

		local current_path_segment = path[ii]
		...
		if type(current_path_segment) == "table" then
			...
			for sub_tree_idx = min_range, max_range do
				sum = sum + _resolve_stat_path(resolved_value[sub_tree_idx], path, ii + 1, end_idx)
			end
										...
										for modified_stat_idx = 1, #stat_group do
											...
											if stat_display_data then
												...
												if ui_data then
													...
													for ii = 1, #damage_profile_path do
														local path_segment = damage_profile_path[ii]
...
function WeaponStats:calculate_stats(weapon_template, weapon_tweak_templates, damage_profile_lerp_values)
	...
	local attacker_buff_extension, target_buff_extension = nil
	...
	local reload_start_time = nil
	local reload_loop_ammo_refil = 0
	local reload_loop_time, reload_start_scale, reload_loop_scale, reload_scale = nil
		...
		if weapon_handling then
			...
			if lerped_identifier then
				...
				if action_stats then
					action_time_scale = action_stats.time_scale or action_time_scale
				end
			end
		end
			...
			if damage_profile then
				...
				for index, target in pairs(damage_targets) do
					if index == target_index then
						...
						local target_unit, attacker_breed_or_nil, attacker_instigator_breed_or_nil = nil
...
function WeaponStats:get_compare_stats_limits(weapon_template)
	...
	for name, compare_weapon_template in pairs(templates_to_compare) do
		...
		for ii = 1, #stats_list do
			local stat_name = stats_list[ii]
...
function WeaponStats:get_comparing_stats()
	...
	if item_base_stats then
		for ii = 1, #item_base_stats do
			local stat = item_base_stats[ii]
...
local function _calculate_action_damage(action_power_level, damage_profile, current_hit_lerp_values, target_index, charge_level, dropoff_scalar, attack_settings)
	...
	for ii = 1, attack_settings_size, 3 do
		local armor_type = attack_settings[ii]
		local attack_flags = attack_settings[ii + 1]
		local impact_flags = attack_settings[ii + 2]
...
function _get_display_data_from_path(display_stats, stat_data)
	...
	for ii = 1, #stat_data - 1 do
		local path_name = stat_data[ii]
...
function _resolve_damage_template_lerps(weapon_template, target_name, stat_data, min, max, path_length_offset, optional_damage_profile)
	...
	if action then
		...
		if damage_profile then
			...
			for ii = 1, #stat_data - (path_length_offset or 1) do
				local path_name = stat_data[ii]
				resolved_table = resolved_table[path_name]
	...
	for stat_group_idx = 1, #stats_to_represent do
		...
		if ui_data then
			...
			if template_type == template_types.damage then
				...
				if resolved_table then
					for ii = 1, #stat_data do
						local path = stat_data[ii]
