...
function PlayerUnitPeeking.start(peeking_component, t)
	peeking_component.is_peeking = true
end

function PlayerUnitPeeking.stop(peeking_component, first_person_extension, optional_height_time_to_change, optional_height_change_function)
...
function PlayerUnitPeeking.leaving_peekable_character_state(peeking_component, animation_extension, first_person_extension, optional_height_time_to_change, optional_height_change_function)
...
function PlayerUnitPeeking.on_enter_cover(animation_extension)
	animation_extension:anim_event_1p("to_cover")
	animation_extension:anim_event("to_cover")
end

function PlayerUnitPeeking.on_leave_cover(animation_extension)
	animation_extension:anim_event_1p("from_cover")
	animation_extension:anim_event("from_cover")
end

function PlayerUnitPeeking.fixed_update(peeking_component, ledge_finder_extension, animation_extension, first_person_extension, talent_extension, is_crouching, breed)
...
function PlayerUnitPeeking.best_peekable_ledge(num_ledges, ledges, first_person_extension, breed)
...
function PlayerUnitPeeking.update_first_person_animations(unit_1p, data, dt, t)
	if not data.is_initialized then
		_initialize_1p(unit_1p, data)
	end
end

function PlayerUnitPeeking.update_third_person_animations(unit_3p, data, dt)
	if not data.is_initialized then
