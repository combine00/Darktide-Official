...
local _place_flow_3p_cb_footstep, _trigger_footstep_and_foley = nil
local MaterialFx = {
	flow_cb_3p_footstep = function (world, wwise_world, physics_world, unit, sound_alias, foot, use_cached_material_hit)
	...
	trigger_material_fx = function (unit, world, wwise_world, physics_world, sound_alias, source_id, query_from, query_to, optional_set_speed_parameter, optional_set_first_person_parameter, use_cached_material_hit)
		local fx_extension = ScriptUnit.has_extension(unit, "fx_system")
...
function MaterialFx.update_1p_footsteps(t, footstep_time, right_foot_next, previous_frame_character_state_name, is_in_first_person_mode, context, ...)
	...
	if (character_state_name == "walking" or character_state_name == "sprinting") and (previous_frame_character_state_name == "walking" or previous_frame_character_state_name == "sprinting") then
		if right_foot_next then
			footstep_sound_alias = "footstep_right"
		else
			footstep_sound_alias = "footstep_left"
		end
...
function MaterialFx.update_3p_footsteps(previous_frame_character_state_name, is_in_first_person_mode, context, ...)
