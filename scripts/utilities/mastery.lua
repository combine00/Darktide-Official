...
local cached_pattern_id_to_category_id, cached_category_id_to_pattern_id, cached_cost_per_trait_level, cached_trait_level_unlock_from_mastery_count = nil
local Mastery = {
	get_trait_costs = function ()
	...
	get_dummy_weapon_xp_per_level = function ()
		return dummy_exp_per_level
	end,
	get_weapon_xp_per_level = function (mastery_data)
...
function Mastery.get_max_points(mastery_data)
	local milestones = mastery_data and mastery_data.milestones
	local points = 0
...
function Mastery.get_all_unlocked_points(mastery_data)
	local milestones = mastery_data and mastery_data.milestones
...
function Mastery.get_max_blessing_rarity_unlocked_level(mastery_data)
	local milestones = mastery_data and mastery_data.milestones
	local rarity = 1
...
function Mastery.get_max_blessing_rarity_unlocked_level_by_points_spent(traits)
...
function Mastery.get_max_perk_rarity_unlocked_level(mastery_data)
	local milestones = mastery_data and mastery_data.milestones
	local rarity = 1
...
function Mastery.rewards_by_level(mastery_data)
	local milestones = mastery_data and mastery_data.milestones
...
function Mastery.milestones_by_level(mastery_data)
	local milestones = mastery_data and mastery_data.milestones
	local level_milestones = {}
...
function Mastery.get_mastery_max_level(mastery_data)
	local max_level = 0
	local milestones = mastery_data and mastery_data.milestones
...
function Mastery.get_masteries_rewards_by_id(masteries_data, pattern_id, reward_type_id)
	local rewards = {}
...
function Mastery.get_mastery_rewards_by_id(mastery_data, reward_type_id)
	local rewards = {}
	local milestones = mastery_data and mastery_data.milestones
...
function Mastery.get_milestone_ui_data(milestone)
...
function Mastery.get_reward_ui_data(id, reward)
	local reward_data = {}
...
function Mastery.get_level_by_xp(mastery_data, xp)
...
function Mastery.get_current_expertise_cap(mastery_data)
	...
	for i = #rewards_data, 1, -1 do
		...
		if reward_data.level <= current_level then
...
function Mastery.get_max_expertise_cap(mastery_data)
	if not mastery_data or table.is_empty(mastery_data) then
		return 0
	end
...
function Mastery.get_mark_data(mark_reward)
	local mark_data = {}
...
function Mastery.get_mark_item(mark_reward)
...
function Mastery.get_all_mastery_marks(mastery_data)
...
function Mastery.get_start_and_end_xp(mastery_data)
	local start_exp = 0
	local end_exp = 0
...
function Mastery.get_start_and_end_xp_by_level(mastery_data)
	local start_exp = 0
	local end_exp = 0
...
function Mastery.get_trait_cost(trait_rarity)
	local costs = Mastery.get_trait_costs()
...
function Mastery.get_spent_points(traits)
...
function Mastery.get_max_trait_points(traits)
	local points = 0
	local costs = Mastery.get_trait_costs()
...
function Mastery.get_available_points(mastery_data, traits)
	local spent_points = Mastery.get_spent_points(traits)
...
function Mastery.get_trait_level_unlock_from_mastery_count(rarity)
	local costs = Mastery.get_trait_costs()

	return costs.trait_unlock_threshold[tostring(rarity)] or 0
end

function Mastery.get_max_trait_level_unlock_from_mastery_count()
	local costs = Mastery.get_trait_costs()

	return costs.trait_unlock_threshold[tostring(table.size(costs.trait_unlock_threshold))] or 0
end

function Mastery.get_levels_to_claim(mastery_data, new_xp)
...
function Mastery.has_available_points(masteries_data, masteries_traits)
...
function Mastery.can_claim_any_reward(masteries_data)
	local filtered_masteries_data = Mastery.get_masteries_by_archetype(masteries_data)
...
function Mastery.can_claim_reward(mastery_data)
...
function Mastery.get_masteries_by_archetype(masteries_data)
...
function Mastery.is_mastery_unlocked(mastery_data)
	local player_manager = Managers.player
	local player = player_manager:local_player(1)
	local profile = player:profile()
	local profile_archetype = profile.archetype
...
function Mastery.get_xp_for_next_level(mastery_data)
	local xp = 0
...
function Mastery.get_xp_for_max_level(mastery_data)
	local xp = 0
...
function Mastery.get_weapon_mapping()
...
function Mastery.get_unclaimed_rewards(mastery_data)
	local result = {}
