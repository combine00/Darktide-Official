...
function ConstantElementNotificationFeed:_event_player_authenticated()
	local save_manager = Managers.save
	local assist_notifications_enabled = true
...
function ConstantElementNotificationFeed:_on_player_portrait_loaded(notification, player, grid_index, rows, columns, render_target)
	local widget = notification.widget

	_apply_player_portrait_icon_cb_func(widget, grid_index, rows, columns, render_target)
end

function ConstantElementNotificationFeed:_on_player_frame_loaded(notification, item)
	local widget = notification.widget

	_add_player_frame_cb_func(widget, item)
end

function ConstantElementNotificationFeed:event_update_assist_notification_type(value)
	self._assist_notifications_enabled = value == "notification"
end

function ConstantElementNotificationFeed:event_update_crafting_pickup_notification_type(value)
	self._crafting_pickup_notifications_enabled = value == "notification"
end

function ConstantElementNotificationFeed:event_add_notification_message(message_type, data, callback, sound_event, done_callback, delay)
	if delay then
...
function ConstantElementNotificationFeed:_can_show_currency_of_type(currency_type)
...
function ConstantElementNotificationFeed:_can_show_assist_notification()
	return self._assist_notifications_enabled
	return true
end

function ConstantElementNotificationFeed:_add_notification_message(message_type, data, callback, sound_event, done_callback)
...
function ConstantElementNotificationFeed:_remove_notification(notification_to_remove, ui_renderer)
	local notifications = self._notifications
	...
	for i = 1, #notifications do
		...
		if notification == notification_to_remove then
			...
			if notification.portrait_loaded_info then
				_remove_player_portrait_icon_cb_func(widget, ui_renderer)

				local portrait_loaded_info = notification.portrait_loaded_info
				local icon_load_id = portrait_loaded_info.icon_load_id

				Managers.ui:unload_profile_portrait(icon_load_id)

				notification.portrait_loaded_info = nil
			end

			if notification.frame_loaded_info then
				_remove_player_frame_cb_func(widget, ui_renderer)

				local frame_loaded_info = notification.frame_loaded_info
				local icon_load_id = frame_loaded_info.icon_load_id

				Managers.ui:unload_item_icon(icon_load_id)

				notification.frame_loaded_info = nil
			end
...
function ConstantElementNotificationFeed:_create_notification_entry(notification_data)
	...
	local player = notification_data.player
	local use_player_portrait = notification_data.use_player_portrait

	if player and use_player_portrait then
...
function ConstantElementNotificationFeed:_evaluate_notification_removal(notification, ui_renderer)
	local widget = notification.widget
