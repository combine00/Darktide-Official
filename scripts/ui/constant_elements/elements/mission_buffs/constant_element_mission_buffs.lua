...
function MissionBuffs:init(parent, draw_layer, start_scale)
...
function MissionBuffs:_setup_background_gui()
...
function MissionBuffs:_destroy_background()
...
function MissionBuffs:cb_shading_callback(world, shading_env, viewport, default_shading_environment_name)
...
function MissionBuffs:_present_buffs(ui_renderer)
	...
	if buffs_size and buffs_size > 0 then
		...
		for i = 1, buffs_size do
			...
			local sub_buff_name, title, icon, gradient = nil
...
function MissionBuffs:_generate_buffs_widgets(layout, ui_renderer)
	...
	local left_click_callback_name = nil
...
function MissionBuffs:_destroy_buff_widgets(ui_renderer)
...
function MissionBuffs:_set_background_blur(blur_amount)
...
function MissionBuffs:_cb_on_buff_clicked(widget, element)
	local buff_index = element.buff_index

	self:_handle_choice_resolution(buff_index, false)
end

function MissionBuffs:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function MissionBuffs:on_resolution_modified(scale)
	MissionBuffs.super.on_resolution_modified(self, scale)
end

function MissionBuffs:_cutscene_start()
	self._cutscene_fade = true
end

function MissionBuffs:_cutscene_end()
	self._cutscene_fade = nil
end

function MissionBuffs:_are_timers_running()
	return self._texts_timer or self._buffs_timer or self._blur_timer
end

function MissionBuffs:_are_animations_running()
...
function MissionBuffs:_is_player_alive()
	local player = nil
...
function MissionBuffs:_is_player_in_mission()
	return self._current_game_mode == "survival"
end

function MissionBuffs:_should_force_inactivate()
...
function MissionBuffs:_check_cutscene_state()
...
function MissionBuffs:_update_view_state(dt, ui_renderer)
...
function MissionBuffs:_update_timers_state(dt, ui_renderer)
...
function MissionBuffs:_update_texts_state(dt, ui_renderer)
...
function MissionBuffs:_update_buffs_state(dt, ui_renderer)
...
function MissionBuffs:_update_blur_state(dt, ui_renderer)
...
function MissionBuffs:update(dt, t, ui_renderer, render_settings, input_service)
...
function MissionBuffs:_handle_input(input_service, dt, t)
...
function MissionBuffs:_select_buff_by_index(wanted_index)
...
function MissionBuffs:_on_navigation_input_changed()
...
function MissionBuffs:_presentation_changed(context)
	self:_update_presentation(context)
end

function MissionBuffs:_update_presentation(context)
	if self._context.is_choice and not self._context.buff_chosen then
		self:_force_choice_resolution()
	end

	local buffs_data = nil
	...
	elseif not is_before_first_wave_start and not is_after_last_wave_end and DEFAULT_TIMER < timer or is_catchup then
	...
	if is_catchup then
		if not context.is_buff_family then
			if table.is_empty(self._context) then
				-- Nothing
...
function MissionBuffs:_force_choice_resolution()
	if self._context.is_choice and not self._context.buff_chosen then
...
function MissionBuffs:_play_buff_acquired_sounds()
...
function MissionBuffs:_handle_choice_resolution(choice_index, is_random_timeout_choice)
...
function MissionBuffs:using_input()
	return self._using_input
end

function MissionBuffs:_push_cursor()
...
function MissionBuffs:_pop_cursor()
	if self._cursor_pushed then
		local input_manager = Managers.input
		local name = self.__class_name
...
function MissionBuffs:_reset_view(ui_renderer)
	self:_destroy_background()
...
function MissionBuffs:destroy(ui_renderer)
	self:_reset_view(ui_renderer)
...
function MissionBuffs:_has_text()
	return self._context.title or self._context.sub_title
end

function MissionBuffs:_reset_buff_animations()
...
function MissionBuffs:_reset_text_animations()
...
function MissionBuffs:should_update()
	return true
end

function MissionBuffs:should_draw()
