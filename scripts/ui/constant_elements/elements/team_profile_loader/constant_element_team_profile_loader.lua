...
function player_template:sync_profile_changes(updated_profile)
	local loading_data = self.loading_data
...
function ConstantElementTeamProfileLoader:init(parent, draw_layer, start_scale)
	self._player_instance_templates_by_account_id = {}
	self._event_listeners_initialized = false
	self._delay_timer = nil
end

function ConstantElementTeamProfileLoader:destroy()
	if self._event_listeners_initialized then
		Managers.event:unregister(self, "event_player_set_profile")
	end
end

function ConstantElementTeamProfileLoader:_on_state_changed(new_state_name)
	self._current_state_name = new_state_name
	self._delay_timer = nil
end

function ConstantElementTeamProfileLoader:update(dt, t, ui_renderer, render_settings, input_service)
...
function ConstantElementTeamProfileLoader:_sync_template_instances_by_players(players)
...
function ConstantElementTeamProfileLoader:_sync_player_template_instances()
...
function ConstantElementTeamProfileLoader:_event_player_set_profile(player, profile)
	...
	if instance_character_profile_id == character_id then
		instance:sync_profile_changes(profile)
	else
		instance:on_deactivation()
...
function ConstantElementTeamProfileLoader:set_visible(visible, optional_visibility_parameters)
	return
end

function ConstantElementTeamProfileLoader:should_update()
	return true
end

function ConstantElementTeamProfileLoader:should_draw()
	return false
end
