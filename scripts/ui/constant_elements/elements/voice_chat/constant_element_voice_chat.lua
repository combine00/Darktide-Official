...
function ConstantElementVoiceChat:init(parent, draw_layer, start_scale, data, definition_path, definition_settings)
...
function ConstantElementVoiceChat:_update_voice_chat_visible_setting_update(value)
	self._should_render = value
end

function ConstantElementVoiceChat:update(dt, t, ui_renderer, render_settings, input_service)
...
function ConstantElementVoiceChat:_update_players_speaking(dt, t, ui_renderer)
...
function ConstantElementVoiceChat:_remove_speaking_player(removed_account_id)
...
function ConstantElementVoiceChat:_chat_manager_participant_update(channel_handle, participant)
	if participant then
...
function ConstantElementVoiceChat:_chat_manager_participant_removed(channel_handle, participant_uri, participant)
	local account_id = participant and participant.account_id

	self:_remove_speaking_player(account_id)
end

function ConstantElementVoiceChat:_chat_manager_participant_added(channel_handle, participant)
	local account_id = participant and participant.account_id
...
function ConstantElementVoiceChat:destroy(ui_renderer)
...
function ConstantElementVoiceChat:draw(dt, t, ui_renderer, render_settings, input_service)
...
function ConstantElementVoiceChat:_voip_manager_updated_channel_state(session_handle, state, incoming)
...
function ConstantElementVoiceChat:_remove_player(account_id)
	if account_id then
		self:_remove_speaking_player(account_id)
