...
function ConstantGroupFinderStatus:init(parent, draw_layer, start_scale)
...
function ConstantGroupFinderStatus:_cancel_active_request_anims()
...
function ConstantGroupFinderStatus:start_player_request_anim_enter()
...
function ConstantGroupFinderStatus:start_player_request_anim_exit()
	self:_cancel_active_request_anims()
...
function ConstantGroupFinderStatus:on_player_request_anim_enter_complete()
	return
end

function ConstantGroupFinderStatus:on_player_request_anim_exit_complete()
	self._request_exit_anim_id = nil
end

function ConstantGroupFinderStatus:_sync_votes(slots)
...
function ConstantGroupFinderStatus:_setup_ready_slots(amount)
...
function ConstantGroupFinderStatus:_update_party_size()
...
function ConstantGroupFinderStatus:_set_start_time(time)
...
function ConstantGroupFinderStatus:_set_slot_status_by_index(index, occupied, ready)
	local widget = self._slot_widgets[index]
	widget.content.selected = ready
	widget.content.occupied = occupied
end

function ConstantGroupFinderStatus:destroy()
...
function ConstantGroupFinderStatus:_update_incoming_join_requests()
	local party_immaterium = Managers.party_immaterium
	local join_requests, version = nil
	...
	if join_requests then
		if self._advertisement_join_requests_version <= version then
...
function ConstantGroupFinderStatus:update(dt, t, ui_renderer, render_settings, input_service)
...
function ConstantGroupFinderStatus:_check_element_position()
	local new_view = Managers.ui:active_top_view()
...
function ConstantGroupFinderStatus:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
	ConstantGroupFinderStatus.super._draw_widgets(self, dt, t, input_service, ui_renderer, render_settings)
end

function ConstantGroupFinderStatus:_change_lobby_status_position(x, y)
...
function ConstantGroupFinderStatus:draw(dt, t, ui_renderer, render_settings, input_service)
