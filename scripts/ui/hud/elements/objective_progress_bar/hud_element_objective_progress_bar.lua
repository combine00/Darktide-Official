...
function HudElementObjectiveProgressBar:init(parent, draw_layer, start_scale)
...
function HudElementObjectiveProgressBar:_setup_widget_groups()
...
function HudElementObjectiveProgressBar:destroy(ui_renderer)
...
function HudElementObjectiveProgressBar:draw(dt, t, ui_renderer, render_settings, input_service)
...
function HudElementObjectiveProgressBar:event_objective_progress_bar_open(objective)
	local active_objective_array = self._active_objective_array

	if active_objective_array[objective] or self._max_objectives <= #active_objective_array then
		return
	end
...
function HudElementObjectiveProgressBar:event_objective_progress_bar_close(objective)
	local active_objective_array = self._active_objective_array
...
function HudElementObjectiveProgressBar:update(dt, t, ui_renderer, render_settings, input_service)
	local is_active = self._is_active
...
function HudElementObjectiveProgressBar:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function HudElementObjectiveProgressBar:_set_active(active)
	self._is_active = active
end

function HudElementObjectiveProgressBar:_set_health_bar_alpha(alpha_fraction)
...
function HudElementObjectiveProgressBar:_apply_widget_bar_fractions(objective, widget, bar_fraction, ghost_fraction, max_fraction, t)
