...
function HudBarLogic:init(settings)
...
function HudBarLogic:_verify_settings(settings)
	for i = 1, #settings_list do
		local setting_name = settings_list[i]
	end
end

function HudBarLogic:_sync_bar_progress(bar_progress, bar_max_progress)
...
function HudBarLogic:update(dt, t, bar_progress, bar_max_progress)
...
function HudBarLogic:_set_bar_fraction(name, fraction, current_fraction, total_duration, animation_threshold, force_update)
...
function HudBarLogic:_update_bar_animations(dt, t)
	local bar_animations = self._bar_animations
	local update_order = self._update_order
	local stored_fractions = self._stored_fractions
	local update = false
...
function HudBarLogic:_reset_bar_alpha()
	self:_set_alpha(255)
...
function HudBarLogic:_update_bar_alpha(dt, t)
...
function HudBarLogic:_set_alpha(alpha)
	local alpha_fraction = alpha / 255
	self._alpha_multiplier = alpha_fraction
end

function HudBarLogic:_update_bar_animation(anim_data, dt)
...
function HudBarLogic:alpha_multiplier()
	return self._alpha_multiplier
end

function HudBarLogic:animated_fractions()
	return self._bar_fraction, self._bar_max_fraction
end

function HudBarLogic:_set_animated_fractions(bar_fraction, bar_max_fraction)
