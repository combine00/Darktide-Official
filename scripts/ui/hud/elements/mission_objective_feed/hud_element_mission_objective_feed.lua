...
function HudElementMissionObjectiveFeed:destroy(ui_renderer)
...
function HudElementMissionObjectiveFeed:_update_widgets(dt, t)
...
function HudElementMissionObjectiveFeed:_update_timer_progress(hud_objective, dt, realign)
	local objective_name = hud_objective:objective_name()
	local widget = self._widgets_by_name[objective_name]
	local content = widget.content
	...
	if realign then
		local realignment_text = nil

		if show_hours then
			realignment_text = text
		elseif show_minutes then
			realignment_text = "00:00:00"
		else
			realignment_text = "00:00"
		end
