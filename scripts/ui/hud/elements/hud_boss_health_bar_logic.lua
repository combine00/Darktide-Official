...
function HudBossHealthBarLogic:init(settings)
...
function HudBossHealthBarLogic:_verify_settings(settings)
	for i = 1, #settings_list do
		local setting_name = settings_list[i]
	end
end

function HudBossHealthBarLogic:_sync_player_health(bar_progress, bar_max_progress)
	...
	if bar_progress ~= self._bar_progress or bar_max_progress ~= self._bar_max_progress then
		...
		if not settings.animate_on_health_increase then
		...
		local animate = self:_set_bar_fraction(name, bar_progress, nil, settings.duration_health, health_animation_threshold, force_update)

		self:_set_bar_fraction("health_max", bar_max_progress, current_bar_max_progress, settings.duration_health, health_animation_threshold, animate)
		self:_set_bar_fraction("health_ghost", bar_progress, force_update and previous_fraction, settings.duration_health_ghost, nil, animate)
	end
end

function HudBossHealthBarLogic:update(dt, t, bar_progress, bar_max_progress)
	self._health_fraction = nil
	self._health_ghost_fraction = nil

	self:_sync_player_health(bar_progress, bar_max_progress)
	self:_update_bar_animations(dt, t)
	self:_update_bar_alpha(dt, t)
end

function HudBossHealthBarLogic:_set_bar_fraction(name, fraction, current_fraction, total_duration, animation_threshold, force_update)
...
function HudBossHealthBarLogic:_update_bar_animations(dt, t)
	local bar_animations = self._bar_animations
...
function HudBossHealthBarLogic:_reset_bar_alpha()
	self:_set_alpha(255)
...
function HudBossHealthBarLogic:_update_bar_alpha(dt, t)
...
function HudBossHealthBarLogic:_set_alpha(alpha)
	local alpha_fraction = alpha / 255
	self._alpha_multiplier = alpha_fraction
end

function HudBossHealthBarLogic:_update_bar_animation(anim_data, dt)
...
function HudBossHealthBarLogic:alpha_multiplier()
	return self._alpha_multiplier
end

function HudBossHealthBarLogic:animated_health_fractions()
	return self._health_fraction, self._health_ghost_fraction, self._health_max_fraction
end

function HudBossHealthBarLogic:_set_animated_health_fractions(health_fraction, health_ghost_fraction, health_max_fraction)
