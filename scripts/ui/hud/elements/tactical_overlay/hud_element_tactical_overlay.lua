...
function HudElementTacticalOverlay:init(parent, draw_layer, start_scale, optional_context)
...
function HudElementTacticalOverlay:_create_resource_renderer()
...
function HudElementTacticalOverlay:_add_class_buffs_data(display_buffs, profile)
	local class_loadout = {
		ability = {},
		blitz = {},
		aura = {},
			...
			if settings_by_node_type then
				...
				if modifiers then
					for i = 1, #modifiers do
						...
						elseif talent_type == "blitz" and class_loadout.grenade_ability then
							modifier_icon = class_loadout.grenade_ability.hud_icon or modifier_icon
						end
...
function HudElementTacticalOverlay:_add_items_buffs_data(display_buffs, profile)
...
function HudElementTacticalOverlay:_add_player_buffs()
	...
	for i = 1, #buffs do
		...
		if not buff:is_negative() then
			...
			if not is_generic and not is_weapon and not is_gadget and not is_aura and not is_talent and not is_horde_sub_buff or has_hud then
				...
				local texture, gradient, material_values = nil
				...
				local category_id, sub_category_id, size, offset = nil
				...
				if is_horde then
					...
					if buff_data and buff_data.title and buff_data.title ~= "" then
						title = Localize(buff_data.title) or title
					end

					if buff_data then
						description = MissionBuffsParser.get_formated_buff_description(buff_data, Color.ui_terminal(255, true)) or description
					end
						...
						for talent_name, definition in pairs(archetype_talents) do
							...
							if talent_buff_passive_template_name == buff_name or talent_buff_coherency_template_name == buff_name or talent_name == buff_related_talent then
								if definition.display_name then
									title = Localize(definition.display_name) or title
								end
...
function HudElementTacticalOverlay:_generate_buffs_layout(display_buffs)
...
function HudElementTacticalOverlay:_setup_buffs_presentation(ui_renderer)
	local extensions = self._parent:player_extensions()
	local buff_extension = extensions and extensions.buff
...
function HudElementTacticalOverlay:_update_buff_input_text()
...
function HudElementTacticalOverlay:_remove_buffs_presentation()
	if self._buff_panel_grid then
		local ingame_service_name = "Ingame"
...
function HudElementTacticalOverlay:_create_buff_panel_widgets(configs, ui_renderer)
	local definitions = {}
	local widgets = {}
...
function HudElementTacticalOverlay:_destroy_resource_renderer()
	if self._world then
		local world_name = self.__class_name .. "_ui_tactical_overlay_world"
		local viewport_name = self.__class_name .. "_ui_tactical_overlay_viewport"
		local buffs_renderer_name = self.__class_name .. "tactical_overlay_buffs_renderer"
...
function HudElementTacticalOverlay:_buffs_navigation(dt, t, input_service)
	local scrollbar_widget = self._widgets_by_name.buff_panel_scrollbar
...
function HudElementTacticalOverlay:set_using_input(value)
	self._using_input = value
end

function HudElementTacticalOverlay:using_input()
...
function HudElementTacticalOverlay:_update_left_panel_elements(ui_renderer)
	local margin = 20
...
function HudElementTacticalOverlay:_set_contracts(optional_data)
	self._contract_data = optional_data
	self._contracts_fetched = optional_data ~= nil
	self._contracts_promise = nil
end

function HudElementTacticalOverlay:_fetch_task_list()
	local player_manager = Managers.player
	local player = player_manager:local_player(1)
...
function HudElementTacticalOverlay:_update_right_grid_size()
...
function HudElementTacticalOverlay:_update_right_hint()
...
function HudElementTacticalOverlay:_update_right_timer_text(dt, t, ui_renderer)
...
function HudElementTacticalOverlay:_delete_tab_bar(ui_renderer)
...
function HudElementTacticalOverlay:_get_page(page_key)
	local grid_overrides = self._grid_overrides

	return grid_overrides[page_key] or ElementSettings.right_panel_grids[page_key]
end

function HudElementTacticalOverlay:_update_right_tab_bar(ui_renderer)
	self:_update_right_hint()
	...
	if table.size(entries) > 0 then
		...
		local selected_blueprint, selected_definition, selected_config = nil
		...
		for _, page_key in ipairs(ordered_keys) do
			if entries[page_key] then
				...
				if selected then
					selected_config = table.clone(config)
...
function HudElementTacticalOverlay:_override_right_panel_category(key, data, ui_renderer)
...
function HudElementTacticalOverlay:_delete_right_panel_widgets(page_key, ui_renderer, is_update)
...
function HudElementTacticalOverlay:_create_right_panel_widgets(page_key, configs, ui_renderer)
...
function HudElementTacticalOverlay:_setup_contracts(contracts_data, ui_renderer)
...
function HudElementTacticalOverlay:_on_right_panel_widgets(key, func_name, ...)
...
function HudElementTacticalOverlay:_update_right_panel_widgets(ui_renderer, desired_page)
	return self:_on_right_panel_widgets(desired_page, "update", ui_renderer)
end

function HudElementTacticalOverlay:_setup_live_event(ui_renderer)
...
function HudElementTacticalOverlay:_update_live_event(dt, ui_renderer)
...
function HudElementTacticalOverlay:_setup_achievements(ui_renderer)
...
function HudElementTacticalOverlay:_update_achievements(dt, ui_renderer)
	local save_data = Managers.save:account_data()
	local favorite_achievements = save_data.favorite_achievements
...
function HudElementTacticalOverlay:_switch_right_grid(ui_renderer)
	local current_key = self._right_panel_key
...
function HudElementTacticalOverlay:_set_difficulty_icons()
...
function HudElementTacticalOverlay:_havoc_rank()
	local havoc_rank_info = self._widgets_by_name.havoc_rank_info
...
function HudElementTacticalOverlay:_setup_havoc_mutators()
...
function HudElementTacticalOverlay:_setup_right_panel_widgets()
	local definitions = {
...
function HudElementTacticalOverlay:draw(dt, t, ui_renderer, render_settings, input_service)
...
function HudElementTacticalOverlay:_draw_render_target(ui_renderer, render_settings)
...
function HudElementTacticalOverlay:_total_materials_collected(material_type)
...
function HudElementTacticalOverlay:_update_materials_collected()
...
function HudElementTacticalOverlay:set_visible(visible, ui_renderer, use_retained_mode)
