...
function HudElementPlayerHealth:init(parent, draw_layer, start_scale, definitions)
...
function HudElementPlayerHealth:update(dt, t, ui_renderer, render_settings, input_service)
...
function HudElementPlayerHealth:_set_disabled(disabled)
...
function HudElementPlayerHealth:_is_player_disabled(unit_data_extension)
	local character_state_component = unit_data_extension:read_component("character_state")

	return PlayerUnitStatus.is_disabled(character_state_component) or false
end

function HudElementPlayerHealth:_set_bar_alpha(alpha_fraction)
	local widgets_by_name = self._widgets_by_name
...
function HudElementPlayerHealth:_apply_health_fraction(health_fraction, health_ghost_fraction, health_max_fraction)
...
function HudElementPlayerHealth:_apply_toughness_fraction(health_fraction, health_ghost_fraction, health_max_fraction)
	local definitions = self._definitions
	local scenegraph_definition = definitions.scenegraph_definition
	local widgets_by_name = self._widgets_by_name
	local bar_width = scenegraph_definition.bar.size[1]
