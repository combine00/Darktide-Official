...
local function _current_and_max_thresholds(template, ui_hud)
	...
	for ii = #thresholds, 1, -1 do
		if thresholds[ii].threshold <= num_special_charges then
...
function weapon_counter_template_kill_charges.create_widget_defintion(scenegraph_id)
...
function weapon_counter_template_kill_charges.on_enter(hud_element_weapon_counter, slot_name, widget, template)
...
function weapon_counter_template_kill_charges.update_function(hud_element_weapon_counter, ui_renderer, widget, is_currently_wielded, weapon_counter_settings, template, dt, t)
