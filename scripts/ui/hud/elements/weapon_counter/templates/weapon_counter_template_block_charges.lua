...
weapon_counter_template_block_charges.size = size
weapon_counter_template_block_charges.center_size = center_size
...
local function _current_and_max_thresholds(template, ui_hud)
	...
	local num_activations = weapon_special_tweak_data.num_activations
	local num_charges_to_consume_on_activation = weapon_special_tweak_data.num_charges_to_consume_on_activation

	if not num_charges_to_consume_on_activation then
		return 0, 0
	end

	local current_threshold = 1

	for ii = num_activations, 1, -1 do
		if num_special_charges >= num_charges_to_consume_on_activation * ii then
	...
	return current_threshold, num_activations
end
...
function weapon_counter_template_block_charges.create_widget_defintion(scenegraph_id)
...
function weapon_counter_template_block_charges.on_enter(hud_element_weapon_counter, slot_name, widget, template)
	...
	_set_arc_sizes(widget, ui_hud, template, max_thresholds)
end

function weapon_counter_template_block_charges.update_function(hud_element_weapon_counter, ui_renderer, widget, is_currently_wielded, weapon_counter_settings, template, dt, t)
	local content = widget.content
	local style = widget.style
	local ui_hud = hud_element_weapon_counter._parent
	local current_threshold, max_thresholds = _current_and_max_thresholds(template, ui_hud)

	for ii = 1, max_thresholds do
		local percentage_filled = _segment_threshold_progress(template, ui_hud, ii, current_threshold)
		local segment_style = style[string.format("charge_bar_%d", ii)]
...
return weapon_counter_template_block_charges
