...
function CraftingMechanicusBarterItemsView:on_enter()
...
function CraftingMechanicusBarterItemsView:_setup_background_world()
...
function CraftingMechanicusBarterItemsView:world_spawner()
	return self._world_spawner
end

function CraftingMechanicusBarterItemsView:_setup_button_callbacks()
	self._widgets_by_name.confirm_button.content.hotspot.pressed_callback = callback(self, "_confirm_pressed")
end

function CraftingMechanicusBarterItemsView:_setup_description()
...
function CraftingMechanicusBarterItemsView:cb_on_favorite_pressed()
	local widget = nil
...
function CraftingMechanicusBarterItemsView:_setup_masteries(available_masteries)
	local layout = {}
...
function CraftingMechanicusBarterItemsView:on_resolution_modified(scale)
...
function CraftingMechanicusBarterItemsView:on_back_pressed()
...
function CraftingMechanicusBarterItemsView:update(dt, t, input_service)
	if self._item_grid then
...
function CraftingMechanicusBarterItemsView:_handle_input(input_service)
...
function CraftingMechanicusBarterItemsView:_cb_fetch_inventory_items(items)
	...
	for item_id, item in pairs(items) do
		...
		if not item.no_crafting and slots then
			...
			local is_invalid_item_for_operation = equipped_item.gear_id == item.gear_id or not item.rarity or item.rarity < min_rarity or not has_expertise or expertise_level < min_expertise_level
...
function CraftingMechanicusBarterItemsView:_setup_sort_options()
	self._sort_options = {
		{
...
function CraftingMechanicusBarterItemsView:cb_on_sort_button_pressed(option)
...
function CraftingMechanicusBarterItemsView:_sort_grid_layout(sort_function)
	...
	if self._current_present_grid_layout_callback then
		self:_current_present_grid_layout_callback(filtered_layout)
	end
end

function CraftingMechanicusBarterItemsView:_mark_items_to_sell(items)
...
function CraftingMechanicusBarterItemsView:_unmark_items_to_sell(items)
	if items and #items > 0 then
		local widgets = self._item_grid:widgets()
...
function CraftingMechanicusBarterItemsView:_setup_menu_tabs(content)
...
function CraftingMechanicusBarterItemsView:cb_switch_tab(index)
...
function CraftingMechanicusBarterItemsView:_present_pattern_layout(slot_filter)
...
function CraftingMechanicusBarterItemsView:_present_items_layout(layout, present_callback)
	...
	function self:_current_present_grid_layout_callback(callback_layout)
...
function CraftingMechanicusBarterItemsView:_add_external_layout(layout)
	local spacing_entry = {
		is_external = true,
		widget_type = "spacing_vertical"
	}
...
function CraftingMechanicusBarterItemsView:_present_sacrifice_layout(layout)
	local generate_blueprints_function = require("scripts/ui/view_content_blueprints/item_blueprints")
...
function CraftingMechanicusBarterItemsView:cb_on_grid_entry_left_pressed(widget, element)
...
function CraftingMechanicusBarterItemsView:_reset_master_xp_increase()
	local mastery_id = self._selected_pattern
	...
	local bar_progress, bar_new_progress = nil

	if is_max_level or mastery_end_exp_adjusted == 0 then
		bar_progress = 1
	elseif mastery_current_xp_adjusted == 0 then
		bar_progress = 0
	else
		bar_progress = math.ilerp(mastery_start_exp, mastery_end_exp, mastery_data.current_xp)
	end
...
function CraftingMechanicusBarterItemsView:_update_mastery_xp_increase()
	local added_xp = 0
	local mastery_id = self._selected_pattern
	local mastery_data = self._masteries[mastery_id]
	local mastery_max_level = mastery_data.mastery_max_level
	...
	local mastery_experience_level_color = mastery_data.mastery_level < mastery_level and Color.ui_blue_light(255, true) or info_style.mastery_experience.default_color
	local mastery_level_added_color = mastery_data.mastery_level < mastery_level and Color.ui_blue_light(255, true) or info_style.mastery_level.default_color
	local mastery_end_exp_adjusted = mastery_end_exp - mastery_start_exp
	local mastery_current_xp_adjusted = mastery_current_xp - mastery_start_exp
	local mastery_current_experience_text = string.format("{#color(%d,%d,%d)}%d{#reset()}", mastery_experience_added_color[2], mastery_experience_added_color[3], mastery_experience_added_color[4], mastery_current_xp_adjusted)
	local mastery_next_experience_text = string.format("{#color(%d,%d,%d)}%d{#reset()}", mastery_experience_level_color[2], mastery_experience_level_color[3], mastery_experience_level_color[4], mastery_end_exp_adjusted)
	...
	if is_max_level then
		bar_new_progress = 1
	elseif added_xp == 0 then
		bar_new_progress = 0
	else
		bar_new_progress = math.ilerp(mastery_start_exp, mastery_end_exp, mastery_current_xp)
	end
...
function CraftingMechanicusBarterItemsView:_cb_pattern_on_present()
...
function CraftingMechanicusBarterItemsView:cb_pattern_on_grid_entry_left_pressed(widget, element)
...
function CraftingMechanicusBarterItemsView:equipped_item_in_slot(slot_name)
	local player = self:_player()
	local profile = player:profile()
...
function CraftingMechanicusBarterItemsView:_on_navigation_input_changed()
	if self._using_cursor_navigation then
		if self._ui_state == "select_weapon" then
			self._discard_items_element:disable_input(false)
...
function CraftingMechanicusBarterItemsView:dialogue_system()
	return self._parent:dialogue_system()
end

function CraftingMechanicusBarterItemsView:ui_renderer()
	return self._ui_renderer
end

function CraftingMechanicusBarterItemsView:_present_mastery(mastery_id)
...
function CraftingMechanicusBarterItemsView:_item_hover_update()
	local widget = nil
...
function CraftingMechanicusBarterItemsView:_get_offer_item_layout_by_gear_id(gear_id)
...
function CraftingMechanicusBarterItemsView:_change_state(state_name)
...
function CraftingMechanicusBarterItemsView:_confirm_pressed()
	if self._ui_state == "select_pattern" then
		self:_change_state("select_weapon")
	elseif self._ui_state == "select_weapon" then
...
function CraftingMechanicusBarterItemsView:_complete_purchase()
