...
function MissionBoardViewLogic:init(context)
...
function MissionBoardViewLogic:update(dt, t)
	self:_update_page(t)
...
function MissionBoardViewLogic:destroy()
...
function MissionBoardViewLogic:_update_backend_data(t)
...
function MissionBoardViewLogic:_update_page(t)
...
function MissionBoardViewLogic:_setup_bonus_config()
	self._bonus_data = {}
...
function MissionBoardViewLogic:_setup_regions()
...
function MissionBoardViewLogic:_request_page(index)
...
function MissionBoardViewLogic:get_requested_page_index()
	return self._requested_page_index
end

function MissionBoardViewLogic:get_current_page_index()
	return self._page_index
end

function MissionBoardViewLogic:get_page_settings()
	return self._page_settings
end

function MissionBoardViewLogic:has_synced_missions_data()
	return self._has_synced
end

function MissionBoardViewLogic:get_missions_data()
	return self._mission_data
end

function MissionBoardViewLogic:get_filtered_missions()
	return self._filtered_missions
end

function MissionBoardViewLogic:get_current_page(page_index)
	local page_settings = self._page_settings

	return page_settings and page_settings[page_index or self._page_index]
end

function MissionBoardViewLogic:get_last_unlocked_page(page_settings)
...
function MissionBoardViewLogic:request_prev_page()
...
function MissionBoardViewLogic:request_next_page()
	local current_page = self:get_current_page()
...
function MissionBoardViewLogic:request_page_at(index)
	self:_request_page(index)
end

function MissionBoardViewLogic:get_bonus_data(category)
...
function MissionBoardViewLogic:get_threat_level_progress()
	local return_table = {}
...
function MissionBoardViewLogic:get_bonus_range(category)
	local bonus_data = self._bonus_data[category]
	...
	for _, percentile_value in pairs(bonus_data) do
		if lo == nil or percentile_value < lo then
			lo = percentile_value or lo
		end

		if hi == nil or hi < percentile_value then
			hi = percentile_value or hi
		end
	end
...
function MissionBoardViewLogic:is_mission_locked(mission)
...
function MissionBoardViewLogic:get_mission_unlock_data(mission_key, mission_category)
...
function MissionBoardViewLogic:refresh_filtered_missions()
	local mission_data = self._mission_data
...
function MissionBoardViewLogic:_remove_unwanted_missions(missions)
	...
	local first_locked_index, second_locked_index = nil
...
function MissionBoardViewLogic:get_region_latencies()
	return self._regions_latency
end

function MissionBoardViewLogic:start_mission_matchmaking(party_manager, selected_mission_id)
...
function MissionBoardViewLogic:start_quickplay_matchmaking(party_manager, categories)
	local page_settings = self:get_current_page()
...
function MissionBoardViewLogic:set_private_matchmaking(value)
	self._is_private = value
end

function MissionBoardViewLogic:is_private_match()
	return self._is_private
end

function MissionBoardViewLogic:_annotate_pages()
	local pages = self._page_settings
	local prev_unlocked_index = nil
...
function MissionBoardViewLogic:get_quickplay_unlock_status()
...
function MissionBoardViewLogic:_get_ordered_story_missions()
...
function MissionBoardViewLogic:get_campaign_mission_display_order(mission_key, mission_category)
	if not mission_key or not mission_category then
...
function MissionBoardViewLogic:get_random_mission()
	local filtered_missions = self._filtered_missions
...
function MissionBoardViewLogic:_get_missions_per_category(category)
	return self._backend_unlock_missions[category] or {}
end

function MissionBoardViewLogic:_get_quickplay_categories(categories)
	local categories = {
		"common",
		"story"
	}
...
function MissionBoardViewLogic:page_is_unlocked(page_index, page_settings)
	local page_settings = page_settings or self._page_settings
...
local function level_is_at_least(level)
	return function (parent)
		return level <= parent._player_level
	end
end
...
function MissionBoardViewLogic:_should_show_mission(mission)
...
function MissionBoardViewLogic:_mission_passes_filter(mission, filter)
...
function MissionBoardViewLogic:_is_story_mission_complete(mission)
	if mission.category ~= "story" then
		return false
	end
...
function MissionBoardViewLogic:_mission_passes_filters(mission, filters, target, exact)
	exact = exact == true
	target = target or #filters
	local count = 0
...
function MissionBoardViewLogic:_mission_passes_all_filters(mission, filters)
	return self:_mission_passes_filters(mission, filters, #filters, false)
end

function MissionBoardViewLogic:_on_missions_data_fetch_success(results)
...
function MissionBoardViewLogic:_on_missions_data_fetch_failure()
...
function MissionBoardViewLogic:_config_success(config_results)
	local has_cached_progression_data = Managers.data_service.mission_board:has_cached_progression_data()
	self._has_cached_progression_data = has_cached_progression_data
	self._backend_unlock_missions = Managers.data_service.mission_board:get_filtered_missions_data()
	self._difficulty_progress_data = Managers.data_service.mission_board:get_difficulty_progression_data()
...
function MissionBoardViewLogic:_config_failure(errors)
