...
function MissionBoardViewDefinitions.create_objectives_panel_widget(scenegraph_id, title, sub_title, icon, size)
...
function MissionBoardViewDefinitions.create_reward_widget(scenegraph_id, amount, icon, size)
	return UIWidget.create_definition({
		{
			value = "content/ui/materials/frames/frame_tile_2px",
			style_id = "frame",
...
animations.mission_exit = {
	{
		...
		update = function (parent, ui_scenegraph, scenegraph_definition, widget, progress, mission_board_view)
			local style = widget.style
			local growing = math.ease_in_quad(progress)
			local shrinking = 1 - math.ease_out_quad(progress)
