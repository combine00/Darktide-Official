...
function MissionBoardView:init(settings, context)
...
function MissionBoardView:_update_play_button_game_mode_text()
...
function MissionBoardView:_page_is_unlocked(page_index)
...
function MissionBoardView:_has_active_campaign_missions(filtered_missions)
	local has_active_campaign_mission = false
	local active_mission_id = nil
...
function MissionBoardView:_get_last_unlocked_page()
...
function MissionBoardView:_request_next_page()
	self._mission_board_logic:request_next_page()
end

function MissionBoardView:_request_prev_page()
	self._mission_board_logic:request_prev_page()
end

function MissionBoardView:_request_page_at(index)
	self._mission_board_logic:request_page_at(index)
end

function MissionBoardView:_get_ui_theme(optional_name)
...
function MissionBoardView:_change_ui_theme(new_theme_name)
...
function MissionBoardView:_open_current_page()
...
function MissionBoardView:_claim_slot(slot_group, mission_category)
...
function MissionBoardView:_release_slot(slot)
	local slot_group = slot.group
	local slot_index = slot.index
	local used_slots = self._used_slots
	used_slots[slot_group][slot_index] = false
end

function MissionBoardView:_widget_from_blueprint(widget_id, blueprint_name, mission_data, creation_context, ...)
...
function MissionBoardView:_create_mission_widget_from_mission(mission, blueprint_name, slot)
...
function MissionBoardView:_set_static_mission_widget(id, optional_creation_context, optional_on_pressed_callback)
...
function MissionBoardView:_set_quickplay_widget()
	local bonus_text = nil
...
function MissionBoardView:_add_mission_widget(mission)
	local flags = mission.flags
	local is_flash = not not flags.flash
...
function MissionBoardView:_destroy_widget(widget)
	UIWidget.destroy(self._ui_renderer, widget)
	self:_unregister_widget_name(widget.name)
end

function MissionBoardView:_replace_mission_widget(old_widget, new_mission)
	local mission_widgets = self._mission_widgets
...
function MissionBoardView:_remove_mission_widget(widget)
	local mission_widgets = self._mission_widgets
...
function MissionBoardView:_missions_are_ui_equivalent(mission_a, mission_b)
	if mission_a.map ~= mission_b.map then
		return false
	end

	if mission_b.expiry_game_time < mission_a.start_game_time or mission_a.expiry_game_time < mission_b.start_game_time then
		return false
	end
...
function MissionBoardView:_ui_equivalent_mission(target, t)
	...
	for _, mission in pairs(missions) do
		...
		local is_active = mission.start_game_time <= t and t <= mission.expiry_game_time
...
function MissionBoardView:_update_mission_widgets(t)
...
function MissionBoardView:_has_issue(key)
	return self._issues[key] ~= nil
end

function MissionBoardView:_has_issue_mission_changed(issue_key)
...
function MissionBoardView:_refresh_issue()
	local current_issue_key = self._current_issue_key
	local can_start_mission = nil
...
function MissionBoardView:_set_issue(key, priority, type, message)
	local current_issue_key = self._current_issue_key
	...
	if issues[key] and issues[current_issue_key] and issues[current_issue_key].priority < issues[key].priority then
		current_issue_key = key
	else
		local max_priority = -1
		local max_priority_key = nil

		for curr_key, curr_config in pairs(issues) do
			local curr_priority = curr_config.priority

			if max_priority < curr_priority then
				max_priority_key = curr_key
...
function MissionBoardView:_poll_issue(issue_key, priority, should_add, issue_type, text_function, ...)
...
function MissionBoardView:_poll_issue_localized(issue_key, priority, should_add, loc_key, optional_issue_type, ...)
	return self:_poll_issue(issue_key, priority, should_add, optional_issue_type or "warning", Localize, loc_key, ...)
end

function MissionBoardView:_get_mission_locked_issue_message()
	local current_text = ""
...
function MissionBoardView:_update_info_state(t)
...
function MissionBoardView:_update_page(t)
	if not self._page_settings or table.is_empty(self._page_settings) then
		return
	end
...
function MissionBoardView:_calculate_camera_properties()
...
function MissionBoardView:_update_camera(dt, t)
	...
	if math.abs(target_rotation_speed) < math.abs(rotation_speed) then
	...
	if math.abs(target_zoom_speed) < math.abs(zoom_speed) then
	...
	if rotation_is_different or zoom_is_different then
		...
		self._camera_rotation = current_rotation
		self._zoom_speed = zoom_speed
		self._camera_zoom = current_zoom
...
function MissionBoardView:update(dt, t, input_service)
	...
	if is_loading then
		input_service = input_service:null_service() or input_service
	end
...
function MissionBoardView:_mission(mission_id)
	return self._filtered_missions[mission_id]
end

function MissionBoardView:_set_sidebar_key_active(key_name, active)
...
function MissionBoardView:_has_sidebar_tabs()
...
function MissionBoardView:_set_sidebar_tab(target_tab_name)
	local sidebar_key_is_active = self._sidebar_key_is_active
...
function MissionBoardView:_set_next_sidebar_tab()
...
function MissionBoardView:_update_mission_location(mission, palette_name)
	...
	local title, sub_title, texture, is_locked, banner_text = nil
...
function MissionBoardView:_update_location_circumstance(mission, palette_name)
...
function MissionBoardView:_update_mission_objectives_panel(mission, palette_name)
	if not mission then
	...
	local title, sub_title, icon = nil
...
function MissionBoardView:_update_mission_objectve_info(mission, palette_name)
		...
		if tab_id then
			if tab_id == "main_objective" then
				...
				if circumstance_rewards then
					credits = credits + circumstance_rewards.credits
					xp = xp + circumstance_rewards.xp
				end
...
function MissionBoardView:_mark_info_panel_dirty(value)
	self._info_panel_dirty = value
end

function MissionBoardView:_update_mission_info_panel()
	if self._info_panel_dirty then
		...
		if self._selected_mission_id == "qp_mission_widget" then
			mission = "qp_mission_widget"
		else
			mission = self:_mission(self._selected_mission_id)
		end
...
function MissionBoardView:_set_selected(id, sidebar_function_name, move_gamepad_cursor, force_reload, ...)
...
function MissionBoardView:_set_quickplay_sidebar()
	return true
end

function MissionBoardView:_set_selected_quickplay(move_gamepad_cursor, force_reload)
	return self:_set_selected("qp_mission_widget", "_set_quickplay_sidebar", move_gamepad_cursor, force_reload)
end

function MissionBoardView:_set_mission_sidebar(mission_id)
	local mission = self:_mission(mission_id)
...
function MissionBoardView:_set_selected_mission(mission_id, move_gamepad_cursor, force_reload)
	return self:_set_selected(mission_id, "_set_mission_sidebar", move_gamepad_cursor, force_reload)
end

function MissionBoardView:_clear_selected()
	self:_set_selected(nil)
end

function MissionBoardView:_difficulty_stepper_input(input_service)
...
function MissionBoardView:_play_button_input(input_service)
...
function MissionBoardView:_virtual_cursor_input(input_service, dt, t)
	...
	local move_requested = settings.snap_input_length_threshold < input_len

	if move_requested then
		self._gamepad_cursor_snap_delay = t + settings.snap_delay
	end

	local should_snap = self._gamepad_cursor_snap_delay < t
	...
	local requested_mission_id, requested_callback = nil
	local selected_mission_id = self._selected_mission_id
...
function MissionBoardView:_set_gamepad_cursor(widget)
	...
	local size_x = widget.content.size[1]
	local size_y = widget.content.size[2]
...
function MissionBoardView:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
	MissionBoardView.super._draw_widgets(self, dt, t, input_service, ui_renderer, render_settings)
end

function MissionBoardView:_draw_elements(dt, t, ui_renderer, render_settings, input_service)
...
function MissionBoardView:_callback_toggle_private_matchmaking(value)
	self._mission_board_logic:set_private_matchmaking(value)
	self:_update_play_button_game_mode_text()
end

function MissionBoardView:_callback_open_options()
...
function MissionBoardView:_destroy_options_element()
	self:_remove_element("mission_board_options_element")

	self._mission_board_options = nil
end

function MissionBoardView:_on_back_pressed()
...
function MissionBoardView:_create_viewport(camera_unit)
	local world_spawner_settings = MissionBoardViewSettings.world_spawner_settings
...
function MissionBoardView:event_register_camera_02(camera_unit)
	Managers.event:unregister(self, "event_register_camera_02")

	self._near_camera_unit = camera_unit
end

function MissionBoardView:_get_difficulty_data_by_name(difficulty_name)
...
function MissionBoardView:_update_threat_level_progress(difficulty_index, target_color)
...
function MissionBoardView:_get_input_text(alias_name)
...
function MissionBoardView:_setup_difficulty_selector()
...
function MissionBoardView:_create_difficulty_stepper_indicators()
...
function MissionBoardView:_update_difficulty_stepper(page_index)
	local page_settings = self._page_settings[page_index]
...
function MissionBoardView:_setup_threat_level_tooltip()
...
function MissionBoardView:_update_threat_level_tooltip(dt, t)
...
function MissionBoardView:_callback_hide_threat_level_tooltip()
	self._threat_level_tooltip_visible = false
end

function MissionBoardView:_callback_show_threat_level_tooltip()
	self._threat_level_tooltip_visible = true
end

function MissionBoardView:_set_stepper_gamepad_input()
	local difficulty_selector = self._widgets_by_name.difficulty_stepper
...
function MissionBoardView:_on_group_finder_pressed()
...
function MissionBoardView:_set_hologram_outline(value)
...
function MissionBoardView:_update_hologram_unit(dt, t)
...
function MissionBoardView:_get_hologram_unit(unit_name)
	local world = self._world_spawner and self._world_spawner._world
