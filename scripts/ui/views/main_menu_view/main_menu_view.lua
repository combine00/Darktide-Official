...
function MainMenuView:_create_server_migration_element(data)
...
function MainMenuView:on_server_migration_removed()
	for i = 1, #self._context.migration_data do
		local migration_to_check = self._context.migration_data[i]
...
function MainMenuView:_destroy_migration_element()
	self:_remove_element("server_migration_element")

	self._server_migration_element = nil
end

function MainMenuView:_create_wallet_element()
...
function MainMenuView:_update_counts_refreshes(dt, t)
...
function MainMenuView:_on_character_widget_selected(index, ignore_sound_trigger)
...
function MainMenuView:_draw_elements(dt, t, ui_renderer, render_settings, input_service)
	local elements_array = self._elements_array
...
function MainMenuView:_handle_input(input_service, dt, t)
	if self._character_wait_overlay_active or self._server_migration_element then
		input_service = input_service:null_service()
	end
...
function MainMenuView:_show_character_details(show, profile)
	...
	if show == true then
		...
		self._promise_container:cancel_on_destroy(is_archetype_available_promise):next(function (result)
			...
			function self._widgets_by_name.play_button.content.hotspot.pressed_callback()
...
function MainMenuView:_cb_unset_player_icon(widget)
	local material_values = widget.style.character_portrait.material_values
