...
function PenanceOverviewView:init(settings, context)
...
function PenanceOverviewView:_update_commendation_status()
...
function PenanceOverviewView:on_enter()
...
function PenanceOverviewView:request_achievement_favorite_add(achievement_id)
...
function PenanceOverviewView:request_achievement_favorite_remove(achievement_id)
...
function PenanceOverviewView:is_favorite_achievement(achievement_id)
	return AchievementUIHelper.is_favorite_achievement(achievement_id)
end

function PenanceOverviewView:_setup_background_world(world_params)
...
function PenanceOverviewView:event_register_camera(camera_unit)
...
function PenanceOverviewView:_set_penance_points_presentation(points)
	self._widgets_by_name.penance_points_panel.content.text = string.format("%d ", points)
end

function PenanceOverviewView:_setup_input_legend()
...
function PenanceOverviewView:_handle_back_pressed()
	local view_name = self.view_name

	Managers.ui:close_view(view_name)
end

function PenanceOverviewView:cb_on_close_pressed()
	if self:can_exit() then
		self:_handle_back_pressed()
	end
end

function PenanceOverviewView:cb_on_toggle_penance_appearance()
...
function PenanceOverviewView:_set_title(text)
	local widget = self._widgets_by_name.page_header
	widget.content.text = Localize(text)
end

function PenanceOverviewView:_setup_achievements()
	local player = self:_player()
...
function PenanceOverviewView:_get_carousel_layouts(filter_out_ids, numbers_to_add)
	local player = self:_player()
...
function PenanceOverviewView:_fetch_unclaimed_penance_rewards()
...
function PenanceOverviewView:_fetch_penance_track()
...
function PenanceOverviewView:_fetch_penance_track_account_state()
	local backend_interface = Managers.backend.interfaces
...
function PenanceOverviewView:_claim_track_tier(tier)
	local backend_interface = Managers.backend.interfaces
	local penance_track = backend_interface.tracks
...
function PenanceOverviewView:_cache_sub_categories()
...
function PenanceOverviewView:_get_achievement_card_layout(achievement_id, is_tooltip)
	local player = self:_player()
...
function PenanceOverviewView:_should_display_achievement(player, is_complete, achievement_definition)
...
function PenanceOverviewView:_cache_achievements(player)
	local total_score = 0
...
function PenanceOverviewView:_handle_input(input_service, dt, t)
...
function PenanceOverviewView:draw(dt, t, input_service, layer)
...
function PenanceOverviewView:_get_carousel_entry_settings(index)
...
function PenanceOverviewView:_add_carousel_entry(index)
	local current_ids = {}
...
function PenanceOverviewView:_update_carousel_entries(dt, t, input_service)
...
function PenanceOverviewView:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
	PenanceOverviewView.super._draw_widgets(self, dt, t, input_service, ui_renderer, render_settings)
end

function PenanceOverviewView:_set_carousel_index(index, animate, has_direction_changed)
...
function PenanceOverviewView:_handle_carousel_scroll(input_service, dt)
	if self._result_overlay then
		input_service = input_service:null_service()
	end
...
function PenanceOverviewView:_focus_on_card(index)
	local carousel_entries = self._carousel_entries
...
function PenanceOverviewView:on_resolution_modified(scale)
...
function PenanceOverviewView:_setup_account_state_data(data)
...
function PenanceOverviewView:_setup_track_data(data)
	local rewards = {}
...
function PenanceOverviewView:_add_points(points, animate, force_reward_bar_change)
...
function PenanceOverviewView:_update_single_animations(dt, t)
...
function PenanceOverviewView:_update_animations(dt, t)
...
function PenanceOverviewView:_set_handle_navigation()
...
function PenanceOverviewView:update(dt, t, input_service)
...
function PenanceOverviewView:_verify_favorite_achievements(index)
	local account_data = Managers.save:account_data()
	local favorite_achievements = account_data.favorite_achievements
	local player = self:_player()
...
function PenanceOverviewView:_claim_wintrack_reward(index)
...
function PenanceOverviewView:on_exit()
	local save_manager = Managers.save
...
function PenanceOverviewView:_setup_tooltip_grid(layout)
...
function PenanceOverviewView:_setup_penance_grid(layout, optional_display_name)
...
function PenanceOverviewView:_on_penance_grid_selection_changed(index)
...
function PenanceOverviewView:_claim_penance(reward_bundle)
	local backend_interface = Managers.backend.interfaces
	local player_rewards = backend_interface.player_rewards
...
function PenanceOverviewView:_cb_on_penance_secondary_pressed(widget, config)
	local content = widget.content
	local element = content.element
...
function PenanceOverviewView:_cb_on_penance_pressed(widget, config)
	if widget.content.can_claim then
		...
		function self._penance_claimed_callback()
...
function PenanceOverviewView:_setup_result_overlay(result_data, result_type)
	if self._result_overlay then
...
function PenanceOverviewView:_close_result_overlay()
	if self._result_overlay then
		self._result_overlay = nil
...
function PenanceOverviewView:_is_result_presentation_active()
	if self._result_overlay then
		return true
	end
...
function PenanceOverviewView:_draw_progress_bar_for_achievement(achievement_definition, is_complete)
	local achievement_type_name = achievement_definition.type
...
function PenanceOverviewView:_can_claim_achievement_by_id(achievement_id)
	local can_claim = self._penance_to_reward_bundle_map[achievement_id] ~= nil

	return can_claim
end

function PenanceOverviewView:_get_achievement_bar_progress(achievement_definition)
	local player = self:_player()
	local achievement_type_name = achievement_definition.type
	local achievement_type = AchievementTypes[achievement_type_name]
...
function PenanceOverviewView:_get_penance_layout_entry_by_achievement_id(achievement_id)
	local player = self:_player()
	local achievement = AchievementUIHelper.achievement_definition_by_id(achievement_id)
	local achievement_definition = Managers.achievements:achievement_definition(achievement_id)
...
function PenanceOverviewView:on_category_button_pressed(index, option, force_selection)
...
function PenanceOverviewView:ui_renderer()
	return self._ui_renderer
end

function PenanceOverviewView:_create_carousel_entry(layout)
...
function PenanceOverviewView:_setup_carousel_entries(achievement_layouts, ui_renderer)
	local carousel_entries = self._carousel_entries
...
function PenanceOverviewView:_on_carousel_card_pressed(index, entry)
	local achievement_id = entry.achievement_id
	local reward_bundle = self._penance_to_reward_bundle_map[achievement_id]

	function self._carousel_claimed_callback()
		local grid = entry.grid
...
function PenanceOverviewView:_on_carousel_card_secondary_pressed(index, entry)
	local achievement_id = entry.achievement_id
	local player = self:_player()
	local is_currently_favorited = self:is_favorite_achievement(achievement_id)
	local is_complete = Managers.achievements:achievement_completed(player, achievement_id)
...
function PenanceOverviewView:cb_category_tab_option_update(content, style)
...
function PenanceOverviewView:_carousel_grid_on_resolution_modified(scale)
	local carousel_entries = self._carousel_entries
...
function PenanceOverviewView:_update_categories_tab_bar_position()
...
function PenanceOverviewView:_category_score(category_id)
...
function PenanceOverviewView:_present_achievement_tooltip(achievement_id)
...
function PenanceOverviewView:_setup_top_panel()
...
function PenanceOverviewView:_on_panel_option_pressed(index)
...
function PenanceOverviewView:_force_select_panel_index(index)
	self:_on_panel_option_pressed(index)
	self._top_panel:set_selected_panel_index(index)
end

function PenanceOverviewView:cb_on_switch_focus()
...
function PenanceOverviewView:_cb_favorite_legend_visibility(add_to_favorite)
...
function PenanceOverviewView:_on_favorite_pressed()
...
function PenanceOverviewView:_on_navigation_input_changed()
...
function PenanceOverviewView:_update_vo(dt, t)
...
function PenanceOverviewView:dialogue_system()
	local world_spawner = self._world_spawner
...
function PenanceOverviewView:_cb_on_play_vo(id, event_name)
	self._current_vo_event = event_name
	self._current_vo_id = id
end

function PenanceOverviewView:play_vo_events(events, voice_profile, optional_route_key, optional_delay, is_opinion_vo)
