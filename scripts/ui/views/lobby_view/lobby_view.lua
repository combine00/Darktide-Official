...
function LobbyView:_remove_input_legend()
...
function LobbyView:_reset_loadout_widgets_navigation()
...
function LobbyView:cb_on_trigger_gamepad_tooltip_navigation_pressed()
...
function LobbyView:_update_loadout_widgets_for_navigation()
...
function LobbyView:_next_loadout_widgets_for_navigation()
...
function LobbyView:_prev_loadout_widgets_for_navigation()
	if not self._loadout_widgets_for_navigation then
		return
	end
...
function LobbyView:_assign_player_to_slot(player, slot)
	...
	local companion_state_machine = nil
...
function LobbyView:_cb_unset_player_icon(slot)
	local widget = slot.panel_widget
	local material_values = widget.style.character_portrait.material_values
...
function LobbyView:_setup_talents_widgets(spawn_slot)
	local margin = 10
...
function LobbyView:_setup_weapon_widgets(spawn_slot)
	local profile = spawn_slot.player:profile()
	local size = {
		370,
		0
...
function LobbyView:_set_weapons_visibility()
...
function LobbyView:_setup_tooltip_info(talent_hover_data)
	local widgets_by_name = self._widgets_by_name
...
function LobbyView:_get_text_height(text, text_style, optional_text_size)
	local ui_renderer = self._ui_renderer
...
function LobbyView:_update_talent_tooltip_position()
	local widgets_by_name = self._widgets_by_name
...
function LobbyView:_setup_item_stats(reference_name)
	local layer = 10
...
function LobbyView:_update_item_stats_position(width, height)
...
function LobbyView:_on_tooltip_hover_start(slot, data)
	if self._show_weapons then
		local item = data
...
function LobbyView:_on_tooltip_hover_stop()
