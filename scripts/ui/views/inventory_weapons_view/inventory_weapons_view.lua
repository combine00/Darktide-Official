...
function InventoryWeaponsView:cb_on_discard_button_pressed()
...
function InventoryWeaponsView:_mark_items_to_sell(items)
...
function InventoryWeaponsView:_unmark_items_to_sell(items)
	if items and #items > 0 then
		local widgets = self._item_grid:widgets()
...
function InventoryWeaponsView:_item_hover_update()
...
function InventoryWeaponsView:cb_on_favorite_pressed()
	local widget = nil
...
function InventoryWeaponsView:cb_on_grid_entry_left_pressed(widget, element)
	if self._discard_items_element then
...
function InventoryWeaponsView:is_selected_item_favorited()
	local selected_grid_widget = self:selected_grid_widget()
...
function InventoryWeaponsView:cb_on_options_pressed()
...
function InventoryWeaponsView:_on_navigation_input_changed()
...
function InventoryWeaponsView:cb_on_marks_pressed()
...
function InventoryWeaponsView:event_replace_list_item(item)
...
function InventoryWeaponsView:replace_item_instance(item)
	local gear_id = item.gear_id
...
function InventoryWeaponsView:cb_on_toggle_item_compare()
...
function InventoryWeaponsView:_update_weapon_stats_position(scenegraph_id, weapon_stats)
