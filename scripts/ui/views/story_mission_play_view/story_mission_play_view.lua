...
function StoryMissionPlayView:init(settings, context)
...
function StoryMissionPlayView:on_enter()
...
function StoryMissionPlayView:_cb_view_story_mission_lore()
	Managers.event:trigger("event_select_story_mission_background_option", 3)
end

function StoryMissionPlayView:_cb_on_mission_start()
...
function StoryMissionPlayView:_update_fetch_missions(t)
...
function StoryMissionPlayView:_fetch_success(data)
...
function StoryMissionPlayView:_fetch_failure(error_message)
...
function StoryMissionPlayView:_unload_reward_widgets()
	for i = 1, 2 do
		local widgets_by_name = self._widgets_by_name
		local widget = widgets_by_name["reward_" .. i]
		local content = widget.content
...
function StoryMissionPlayView:_set_selected_option(index, ignore_sound)
...
function StoryMissionPlayView:_cb_on_options_button_pressed(option_index, data)
	if option_index ~= self._selected_mission_index then
		self:_set_selected_option(option_index)
	end
end

function StoryMissionPlayView:_assign_option_data(option_index, data)
	local widgets_by_name = self._widgets_by_name
...
function StoryMissionPlayView:_set_selected_mission(mission)
...
function StoryMissionPlayView:_update_info_button_text()
...
function StoryMissionPlayView:_on_navigation_input_changed()
	StoryMissionPlayView.super._on_navigation_input_changed(self)
	self:_update_info_button_text()
end

function StoryMissionPlayView:on_resolution_modified(scale)
	StoryMissionPlayView.super.on_resolution_modified(self, scale)
end

function StoryMissionPlayView:on_back_pressed()
...
function StoryMissionPlayView:draw(dt, t, input_service, layer)
...
function StoryMissionPlayView:_update_can_start_mission()
	local player_level = self._player_level
	local mission = self._selected_mission
...
function StoryMissionPlayView:_set_info_text(level, text)
...
function StoryMissionPlayView:_set_play_button_game_mode_text(is_solo_play, is_private_match)
...
function StoryMissionPlayView:update(dt, t, input_service)
...
function StoryMissionPlayView:_get_matchmaking_status()
	return not self._party_manager:are_all_members_in_hub()
end

function StoryMissionPlayView:_handle_input(input_service, dt, t)
...
function StoryMissionPlayView:ui_renderer()
	return self._ui_renderer
end

function StoryMissionPlayView:_callback_open_options(region_data)
...
function StoryMissionPlayView:_callback_close_options()
	self:_destroy_options_element()
end

function StoryMissionPlayView:_destroy_options_element()
	self:_remove_element("mission_board_options_element")

	self._mission_board_options = nil
end

function StoryMissionPlayView:dialogue_system()
	return self._parent:dialogue_system()
end

function StoryMissionPlayView:fetch_regions()
...
function StoryMissionPlayView:_callback_toggle_private_matchmaking()
