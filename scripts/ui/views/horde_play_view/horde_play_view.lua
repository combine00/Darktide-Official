...
local _MISSION_DUMMY_DATA = {
	{
		map = "psykhanium",
		start_server_time = 1733953860000.0,
		mission_reward = 4180,
		displayIndex = 13,
		credits = 4180,
		xp = 2000,
		...
		challenge = 2,
		expiry_server_time = 1734040259999.0,
		start = 1733953860000.0,
		expiry = 1734040259999.0,
...
function HordePlayView:init(settings, context)
...
function HordePlayView:on_enter()
...
function HordePlayView:_cb_on_mission_start()
...
function HordePlayView:_update_fetch_missions(t)
...
function HordePlayView:_fetch_failure(error_message)
...
function HordePlayView:_cancel_promise_on_exit(promise)
...
function HordePlayView:_set_selected_option(index, ignore_sound)
...
function HordePlayView:_cb_on_options_button_pressed(option_index, data)
	if option_index ~= self._selected_mission_index then
		self:_set_selected_option(option_index)
	end
end

function HordePlayView:_assign_option_data(option_index, data)
	local widgets_by_name = self._widgets_by_name
...
function HordePlayView:_set_selected_mission(mission)
...
function HordePlayView:_on_navigation_input_changed()
	HordePlayView.super._on_navigation_input_changed(self)
end

function HordePlayView:on_resolution_modified(scale)
	HordePlayView.super.on_resolution_modified(self, scale)
end

function HordePlayView:on_back_pressed()
...
function HordePlayView:draw(dt, t, input_service, layer)
...
function HordePlayView:_update_can_start_mission()
	local player_level = self._player_level
	local mission = self._selected_mission
...
function HordePlayView:_set_info_text(level, text)
...
function HordePlayView:_set_play_button_game_mode_text(is_solo_play, is_private_match)
...
function HordePlayView:update(dt, t, input_service)
...
function HordePlayView:_get_matchmaking_status()
	return not self._party_manager:are_all_members_in_hub()
end

function HordePlayView:_handle_input(input_service, dt, t)
...
function HordePlayView:on_exit()
...
function HordePlayView:ui_renderer()
	return self._ui_renderer
end

function HordePlayView:_callback_open_options(region_data)
...
function HordePlayView:_callback_close_options()
	self:_destroy_options_element()
end

function HordePlayView:_destroy_options_element()
	self:_remove_element("mission_board_options_element")

	self._mission_board_options = nil
end

function HordePlayView:dialogue_system()
	return self._parent:dialogue_system()
end

function HordePlayView:fetch_regions()
...
function HordePlayView:_callback_toggle_private_matchmaking()
