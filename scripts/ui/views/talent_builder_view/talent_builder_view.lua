...
function TalentBuilderView:init(settings, context)
...
function TalentBuilderView:on_enter()
...
function TalentBuilderView:_update_summery_button_text()
...
function TalentBuilderView:event_on_profile_preset_changed(profile_preset, on_preset_deleted)
...
function TalentBuilderView:on_exit()
	local layout = self:get_active_layout()
...
function TalentBuilderView:get_talent_points_spent_on_nodes()
	return self._points_spent_on_node_widgets
end

function TalentBuilderView:_remove_node_point_on_widget(widget)
...
function TalentBuilderView:_add_node_point_on_widget(widget)
...
function TalentBuilderView:cb_on_talent_summary_pressed()
...
function TalentBuilderView:_close_summary_window()
...
function TalentBuilderView:_close_tutorial_window()
...
function TalentBuilderView:cb_on_clear_all_talents_pressed()
	local context = {
...
function TalentBuilderView:clear_node_points()
...
function TalentBuilderView:_setup_node_connection_widget()
	self._node_connection_widget = self:_create_widget("node_connection", self._definitions.node_connection_definition)
end

function TalentBuilderView:on_archetype_name_changed(archetype_name)
...
function TalentBuilderView:_refresh_all_nodes()
...
function TalentBuilderView:_on_input_scroll_axis_changed(scroll_value, cursor_position)
	if self._player_mode then
...
function TalentBuilderView:_scroll_to_height(height)
	local widgets_by_name = self._widgets_by_name
	local layout_background_widget = widgets_by_name.layout_background
	local scenegraph_id = layout_background_widget.scenegraph_id
...
function TalentBuilderView:_background_scrolled_amount()
	local widgets_by_name = self._widgets_by_name
	local layout_background_widget = widgets_by_name.layout_background
	local scenegraph_id = layout_background_widget.scenegraph_id
	local _, scenegraph_position_y, _ = self:_scenegraph_position(scenegraph_id)
...
function TalentBuilderView:_get_node_definition_by_type(node_type)
...
function TalentBuilderView:_create_node_widget(node)
...
function TalentBuilderView:_is_any_of_widgets_line_progress_done(widget)
	local parent_line_anim_data = widget.content.parent_line_anim_data
...
function TalentBuilderView:_draw_layout_node_connections(dt, t, input_service, ui_renderer, render_settings, layout)
...
function TalentBuilderView:_draw_connection_between_widgets(ui_renderer, visible, dt, parent_node, child_node, offset_x, offset_y, distance, angle)
	...
	elseif child_status == TalentBuilderView.NODE_STATUS.locked or child_status == TalentBuilderView.NODE_STATUS.unavailable or children_unlock_points > points_spent_on_parent then
		color_status = "locked"
...
function TalentBuilderView:_apply_node_connection_anims(node_connection_widget, parent_node, dt)
	return
end

function TalentBuilderView:_apply_node_connection_line_colors(color_status, alpha_multiplier)
	local node_connection_widget = self._node_connection_widget
...
function TalentBuilderView:_setup_layouts()
...
function TalentBuilderView:_get_player_mode_layout()
	local player = self._preview_player
...
function TalentBuilderView:update(dt, t, input_service)
...
function TalentBuilderView:draw(dt, t, input_service, layer)
	if self._player_mode and not self._preview_player then
		return
	end
...
function TalentBuilderView:draw_layout(dt, t, input_service, layer)
	TalentBuilderView.super.draw_layout(self, dt, t, input_service, layer)
end

function TalentBuilderView:_allowed_node_input()
...
function TalentBuilderView:can_exit()
	return not self:_is_handling_popup_window()
end

function TalentBuilderView:block_input(block)
	self._input_blocked = block
end

function TalentBuilderView:_handle_input(input_service, dt, t)
...
function TalentBuilderView:_update_gamepad_cursor(dt, t, input_service)
	...
	if cursor_active then
		...
		if settings.snap_selection_speed_threshold < len_gamepad_cursor_average_vel and best_pos then
...
function TalentBuilderView:apply_active_background_size()
	local widgets_by_name = self._widgets_by_name
	local layout_background_widget = widgets_by_name.layout_background
	local scenegraph_id = layout_background_widget.scenegraph_id
...
function TalentBuilderView:_on_navigation_input_changed()
...
function TalentBuilderView:_set_selected_node(node)
	local index = nil
	local node_widgets = self._node_widgets
...
function TalentBuilderView:_node_on_hovered(node)
	self:_play_sound(UISoundEvents.talent_node_hover_default)
end

function TalentBuilderView:_play_select_sound_for_node_widget(widget)
...
function TalentBuilderView:_on_node_widget_left_pressed(widget)
...
function TalentBuilderView:_on_node_widget_right_pressed(widget)
	if self._is_readonly or not self._is_own_player then
		return
	end
...
function TalentBuilderView:_get_relative_path()
	local path = debug.getinfo(1, "S").source:sub(2)

	return path:match("(.*/)")
end

function TalentBuilderView:_max_node_points()
	if self._player_mode then
		local player = self._preview_player
		local profile = player:profile()
...
function TalentBuilderView:clear_chosen_talents()
	local node_widgets = self._node_widgets
...
function TalentBuilderView:_set_node_points_spent_text(widget, points_spent, max_points)
...
function TalentBuilderView:_update_node_widgets_blocked_symbol_state()
...
function TalentBuilderView:_update_button_statuses(dt, t)
...
function TalentBuilderView:_is_handling_popup_window()
	return (self._summary_grid or self._tutorial_grid) and true or false
end

function TalentBuilderView:_on_node_hover_enter(node)
...
function TalentBuilderView:_setup_tooltip_info(node, instant_tooltip, is_base_talent_tooltip)
...
function TalentBuilderView:_update_base_talent_loadout_presentation()
	local player = self._preview_player
	local profile = player and player:profile()
	local active_layout = self:get_active_layout()
...
function TalentBuilderView:_handle_base_talent_loadout_hover()
	local widgets_by_name = self._widgets_by_name
	local talent_hover_data = self._talent_hover_data
	local hovered_base_talent_widget = nil
...
function TalentBuilderView:_update_node_widgets(dt, t)
...
function TalentBuilderView:_setup_talents_summary_grid()
	local definitions = self._definitions
...
function TalentBuilderView:_setup_tutorial_grid()
	local definitions = self._definitions
...
function TalentBuilderView:_present_tutorial_popup_page(page_index)
...
function TalentBuilderView:_handle_tutorial_button_gamepad_navigation(input_service)
	local widgets_by_name = self._widgets_by_name
...
function TalentBuilderView:_update_tutorial_button_texts()
...
function TalentBuilderView:cb_on_tutorial_button_1_pressed()
...
function TalentBuilderView:cb_on_tutorial_button_2_pressed()
	local current_page_index = self._active_tutorial_popup_page or 1
...
function TalentBuilderView:on_resolution_modified(scale)
	TalentBuilderView.super.on_resolution_modified(self, scale)
end

function TalentBuilderView:_can_drag_background()
