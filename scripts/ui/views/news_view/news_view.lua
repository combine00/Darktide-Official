...
function NewsView:_initialize_slides(slide_data)
	local slides = {}
...
function NewsView:_load_slides()
	self._initialized = false
...
function NewsView:_change_slide(slide_index, ignore_animation)
	local widgets_by_name = self._widgets_by_name
...
function NewsView:_circle_count()
	local slide_count = #self._slides

	return slide_count > 1 and slide_count or 0
end

function NewsView:load_texture(image_url, image_element)
	if image_url then
...
function NewsView:_handle_button_gamepad_navigation(input_service)
	local widgets_by_name = self._widgets_by_name
...
function NewsView:_unload_url_textures()
	local url_textures = self._url_textures
