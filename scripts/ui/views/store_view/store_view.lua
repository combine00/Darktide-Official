...
function StoreView:init(settings, context)
...
function StoreView:_update_store_page()
...
function StoreView:_on_navigation_input_changed()
...
function StoreView:_set_panels_store()
...
function StoreView:_initialize_opening_page()
	local path = {
		category_index = 1,
		page_index = 1
	}
...
function StoreView:_register_button_callbacks()
	local widgets_by_name = self._widgets_by_name

	function widgets_by_name.aquila_button.content.hotspot.pressed_callback()
...
function StoreView:setup_aquila_store()
...
function StoreView:_get_first_grid_panel_index()
	for i = 1, #self._grid_widgets do
...
function StoreView:_get_last_grid_panel_index()
...
function StoreView:_destroy_category_panel()
	if self._category_panel then
...
function StoreView:cb_on_back_pressed()
...
function StoreView:_setup_offscreen_gui()
...
function StoreView:_destroy_offscreen_gui()
...
function StoreView:event_register_store_view_camera(camera_unit)
...
function StoreView:_check_owned(store_item)
...
function StoreView:_on_category_index_selected(index, on_complete_callback)
...
function StoreView:_clear_telemetry_name()
...
function StoreView:_set_telemetry_name(category, page)
...
function StoreView:_on_page_index_selected(page_index)
...
function StoreView:_find_empty_element_with_type(pages, type)
...
function StoreView:_is_owned(items)
	local total_count = 0
	local owned_count = 0
	local owned_items = {}
...
function StoreView:_format_price(element, price)
...
function StoreView:_create_aquilas_presentation(offers)
	self:_destroy_aquilas_presentation()
...
function StoreView:_destroy_aquilas_presentation()
...
function StoreView:_setup_panels(category_pages_layout_data)
...
function StoreView:_setup_navigation_arrows(layout_pages)
	self._selected_page_index = 1
	self._widgets_by_name.navigation_arrow_left.content.visible = false
	self._widgets_by_name.navigation_arrow_right.content.visible = false

	function self._widgets_by_name.navigation_arrow_right.content.hotspot.pressed_callback()
		local page_index = self._selected_page_index + 1

		self:_on_page_index_selected(page_index)
	end

	function self._widgets_by_name.navigation_arrow_left.content.hotspot.pressed_callback()
		local page_index = self._selected_page_index - 1
...
function StoreView:_set_catalog_expire_time(catalog_timer)
...
function StoreView:_set_expire_time(offer)
	local t = Managers.time:time("main")
	local server_time = nil
	server_time = Managers.backend:get_server_time(t)
...
function StoreView:_update_panel_positions()
	if self._page_panel then
...
function StoreView:can_exit()
	return self._can_exit
end

function StoreView:on_exit()
	self:_clear_telemetry_name()
...
function StoreView:_unload_url_textures()
	local url_textures = self._url_textures
...
function StoreView:_setup_input_legend()
...
function StoreView:_handle_input(input_service)
...
function StoreView:update(dt, t, input_service)
...
function StoreView:_update_timers()
...
function StoreView:_blur_fade_in(duration, anim_func)
	self._world_spawner:set_camera_blur(0.75, duration, anim_func)

	self._blurred = true
end

function StoreView:_blur_fade_out(duration, anim_func)
	self._world_spawner:set_camera_blur(0, duration, anim_func)

	self._blurred = false
end

function StoreView:_destroy_current_grid()
	if self._grid_widgets then
		for i = 1, #self._grid_widgets do
			local widget = self._grid_widgets[i]
			local widget_name = widget.name
...
function StoreView:cb_on_grid_entry_left_pressed(widget, element)
	if self._purchase_promise then
		return
	end
...
function StoreView:_setup_grid(layout, grid_settings)
...
function StoreView:_find_widget_by_grid_cell(row, column)
...
function StoreView:_create_entry_widget_from_config(config, suffix, primary_callback_name)
...
function StoreView:_find_next_widget_selection(input_service)
	local grid_widgets = self._grid_widgets
...
function StoreView:_set_selected_grid_index(index)
	local grid_widgets = self._grid_widgets
...
function StoreView:_find_closest_widget_node_neighbour(source_widget, direction)
...
function StoreView:_generate_nine_grid_nodes(size, position)
	local nodes = {
		{
			{
				0,
...
function StoreView:can_handle_input()
	return not self:_is_animation_active(self._on_enter_anim_id)
end

function StoreView:_draw_grid(dt, t, input_service)
...
function StoreView:draw(dt, t, input_service, layer)
...
function StoreView:_select_aquila_widget_by_row(direction)
...
function StoreView:_select_aquila_widget_by_index(index)
	if not self._aquilas_widgets then
		return
	end
...
function StoreView:_debug_generate_layout(grid_settings)
...
function StoreView:_debug_draw_grid(dt, t, ui_renderer)
...
function StoreView:_update_wallets()
	if self._wallet_element then
...
function StoreView:on_resolution_modified(scale)
	StoreView.super.on_resolution_modified(self, scale)
	self:_update_panel_positions()
end

function StoreView:_update_vo(dt, t)
	if self._hub_interaction then
...
function StoreView:dialogue_system()
	local world_spawner = self._world_spawner
...
function StoreView:_cb_on_play_vo(id, event_name)
	self._current_vo_event = event_name
	self._current_vo_id = id
end

function StoreView:play_vo_events(events, voice_profile, optional_route_key, optional_delay, is_opinion_vo)
