...
function MasteryView:init(settings, context)
...
function MasteryView:_update_mastery_data(mastery_id)
...
function MasteryView:cb_on_help_pressed()
	local tutorial_overlay_data = {
		[#tutorial_overlay_data + 1] = {
			window_width = 800,
...
function MasteryView:_setup_input_legend()
...
function MasteryView:_setup_forward_gui()
...
function MasteryView:cb_on_close_pressed()
...
function MasteryView:_reset_mastery_info()
	local level = 0
	local exp = 0
...
function MasteryView:_setup_mastery_info()
...
function MasteryView:draw(dt, t, input_service, layer)
	local tutorial_overlay = self._tutorial_overlay
...
function MasteryView:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
	local tutorial_overlay = self._tutorial_overlay
	local tutorial_overlay_active = tutorial_overlay and tutorial_overlay:is_active()
...
function MasteryView:on_resolution_modified(scale)
	MasteryView.super.on_resolution_modified(self, scale)
end

function MasteryView:_setup_node_widgets()
	if self._node_widgets then
...
function MasteryView:update(dt, t, input_service)
...
function MasteryView:_unlock_progress_animation_update(dt, t)
...
function MasteryView:_tooltip_update()
	...
	if trait_widget then
		...
		local trait_scenegraph_position_x = trait_scenegraph_position[1]
		local trait_scenegraph_position_y = trait_scenegraph_position[2]
...
function MasteryView:_destroy_forward_gui()
...
function MasteryView:_setup_traits()
	if self._trait_widgets then
		...
		if item then
			...
			for jj = 1, #trait_status do
				...
				if valid then
					...
					if unlocked and max_unlocked_rarity_index < index then
						max_unlocked_rarity_index = index or max_unlocked_rarity_index
					end
		...
		if ii <= #available_traits_position then
			...
			if config.traits then
				for jj = 1, #config.traits do
					...
					trait.available = trait.rarity <= unlocked_rarity_level
				end
			end
...
function MasteryView:_update_traits(updated_trait_widget)
...
function MasteryView:_setup_milestones()
...
function MasteryView:_can_trait_be_acquired(trait_item_element)
	local points_available = self._mastery.points_available
	local unlocked_rarity_level = Mastery.get_max_blessing_rarity_unlocked_level_by_points_spent(self._traits)
...
function MasteryView:_check_buttons_visibility()
	return
end

function MasteryView:_change_buttons_disable_state(state)
	if self._trait_widgets then
		for i = 1, #self._trait_widgets do
			local trait_widget = self._trait_widgets[i]
...
function MasteryView:_unselect_trait()
...
function MasteryView:_update_animations(dt, t)
...
function MasteryView:_find_closest_neighbour_vertical(index, input_direction)
...
function MasteryView:_find_closest_neighbour_horizontal(index, input_direction)
	local grid_settings = MasteryViewSettings.trait_grid_settings
	local rows = grid_settings[1]
	local columns = grid_settings[2]
	local max_index = nil
...
function MasteryView:_handle_input(input_service, dt, t)
...
function MasteryView:_on_navigation_input_changed()
	if self._wintrack_element then
...
function MasteryView:cb_on_switch_focus()
