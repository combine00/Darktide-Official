	...
	init = function (parent, widget, element)
		...
		if element.icon then
			...
			if element.icon_size then
				...
				local larger_side_index = original_size[2] <= original_size[1] and 1 or 2
				local margin = 10
		...
		local passes = {
			...
			},
			{
				...
				change_function = function (content, style)
					...
					local can_be_acquired = parent:_can_trait_be_acquired(content.element)
		...
		for i = 1, #traits do
			...
			passes[#passes + 1] = {
				...
				change_function = function (content, style)
					...
					local can_be_acquired = parent:_can_trait_be_acquired(content.element)
					local unlocked_rarity = i <= MasteryUtils.get_max_blessing_rarity_unlocked_level_by_points_spent(parent._traits)
