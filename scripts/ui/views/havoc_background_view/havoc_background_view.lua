...
function HavocBackgroundView:init(settings, context)
...
function HavocBackgroundView:event_havoc_background_on_end_time_met()
	self._rewards = nil
	local on_complete_callback = callback(function ()
...
function HavocBackgroundView:_setup_core_ui(state)
	self:_setup_tab_bar({
		tabs_params = {}
	})
...
function HavocBackgroundView:_setup_off_cadence_ui()
...
function HavocBackgroundView:_setup_no_key_ui()
	self:_setup_core_ui(self._current_state)
...
function HavocBackgroundView:_setup_key_ui()
	self:_setup_core_ui(self._current_state)
...
function HavocBackgroundView:_setup_rewarding_ui()
...
function HavocBackgroundView:on_enter()
...
function HavocBackgroundView:_initialize_havoc_state(on_complete_callback)
	return Managers.data_service.havoc:latest():next(function (latest_data)
		...
		if not table.is_empty(latest_data) and latest_data.end_time <= server_time and not latest_data.is_rewarded then
...
function HavocBackgroundView:_on_view_load_complete(loaded)
	if self._destroyed then
		return
	end
...
function HavocBackgroundView:event_reset_havoc_background_view()
	self:_close_active_view()
...
function HavocBackgroundView:on_exit()
	Managers.event:unregister(self, "event_reset_havoc_background_view")
	HavocBackgroundView.super.on_exit(self)
end

function HavocBackgroundView:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function HavocBackgroundView:_set_intro_texts(intro_texts)
	local widgets_by_name = self._widgets_by_name
...
function HavocBackgroundView:_set_off_cadence_description_text_with_timer(next_cadence_start_date)
...
function HavocBackgroundView:reinitialize()
	if self._destroyed then
		return
	end
...
function HavocBackgroundView:update(dt, t, input_service)
