...
function CharacterAppearanceView:_archetype_page_data()
...
function CharacterAppearanceView:_get_pages()
	...
	function planet_page.on_enter(page)
	...
	function planet_page.on_leave(page)
...
function CharacterAppearanceView:_setup_page_widgets(widgets, widgets_by_name)
	self:_destroy_page_widgets()
...
function CharacterAppearanceView:_create_planet_widgets()
...
function CharacterAppearanceView:_create_childhood_widgets()
	local widgets = {}
...
function CharacterAppearanceView:_create_growing_up_widgets()
	local widgets = {}
...
function CharacterAppearanceView:_create_formative_event_widgets()
	local widgets = {}
...
function CharacterAppearanceView:_create_appearance_widgets()
...
function CharacterAppearanceView:_destroy_page_widgets()
...
function CharacterAppearanceView:_get_childhood_options()
...
function CharacterAppearanceView:_get_growing_up_options()
...
function CharacterAppearanceView:_get_formative_event_options()
...
function CharacterAppearanceView:event_register_companion_spawn_point(spawn_point_unit)
...
function CharacterAppearanceView:_spawn_profile(spawn_point_unit, optional_state_machine, optional_animation_event)
	local spawn_position = Unit.world_position(spawn_point_unit, 1)
...
function CharacterAppearanceView:_show_final_popup()
	local context = nil
...
function CharacterAppearanceView:_open_page(index)
	...
	if current_page and current_page.on_leave then
		current_page:on_leave(self._pages[index])
	end
	...
	if new_page.on_enter then
		new_page:on_enter(previous_page)
	end
...
function CharacterAppearanceView:_update_continue_button(check_id, disabled)
...
function CharacterAppearanceView:_clear_continue_disable_data()
	self._widgets_by_name.continue_button.content.disabled_by = nil
end

function CharacterAppearanceView:_toggle_continue_alternative_action(use_alternative)
...
function CharacterAppearanceView:_get_planet_options()
...
function CharacterAppearanceView:_generate_appearance_grid_widgets(grid_index, grid_data)
	...
	if grid_index > 1 then
		...
		if self._page_grids[prev_index] then
			prev_grid_size = self._page_grids[prev_index].size or prev_grid_size
		end

		if self._page_grids[prev_index] then
			prev_grid_position = self._page_grids[prev_index].position or prev_grid_position
		end
	end
...
function CharacterAppearanceView:_update_appearance_background()
...
function CharacterAppearanceView:_generate_backstory_grid_widgets(grid_index, grid_data)
	local widgets = {}
	local alignment_list = {}
	local grid_start_name = "grid_" .. grid_index .. "_"
	local grid_scenegraph = grid_start_name .. "pivot"
...
function CharacterAppearanceView:_generate_main_grid_widgets(grid_index, grid_data)
	local template_type = grid_data.template
	...
	local grid_background_widget, grid_top_widget, grid_description_widget = nil
	local support_widgets = {}
...
function CharacterAppearanceView:_add_widget_restrictions(widget, available, reason, reason_display_name)
	if not reason then
		return
	end
...
function CharacterAppearanceView:_destroy_page_grid(index)
...
function CharacterAppearanceView:_populate_page_grid(index, grid_data, on_present_callback)
	...
	if options and not table.is_empty(options) then
		...
		if has_selected_option then
			...
			if value then
				local is_item = type(value) == "table" and (value.gear_id or not not value.always_owned)
...
function CharacterAppearanceView:_on_entry_pressed(current_widget, option, grid_index, pressed_options)
...
function CharacterAppearanceView:_setup_background_world()
	...
	local level_name, store_level = nil
...
function CharacterAppearanceView:_filter_changed_items(items)
...
function CharacterAppearanceView:_get_appearance_options()
	...
	for i = 1, #appearance_options do
		...
		function appearance_option.on_pressed_function(grid_index, widget, pressed_options)
		...
		function appearance_option.on_focused_function(grid_index, widget)
			return
		end
	end
...
function CharacterAppearanceView:_get_companion_appearance_options()
	local appearance_options = {}
					...
					on_focused_function = function (grid_index, widget)
						...
						add_gamepad_focused_slots(self, entry_slot_name, selected_option, nil, function (original_option)
							...
							if selected_option then
								local init_pressed_options = self._navigation.grid == 3 and {
...
function CharacterAppearanceView:_check_valid_option(option)
	...
	local reason, reason_display_name = nil
...
function CharacterAppearanceView:_update_selection()
	for i = 1, #self._page_grids do
		local page_grid = self._page_grids[i]
...
function CharacterAppearanceView:_update_icons()
	for i = 1, #self._page_grids do
...
function CharacterAppearanceView:_get_personality_options()
...
function CharacterAppearanceView:_get_crime_options()
...
function CharacterAppearanceView:_generate_final_page_widgets(grid_index, grid_data, is_companion)
	local grid_start_name = "grid_" .. grid_index .. "_"
	local grid_scenegraph = grid_start_name .. "pivot"
	local grid_area_scenegraph = grid_start_name .. "area"
	local grid_content_scenegraph = grid_start_name .. "content"
	...
	local input_name_template, input_template = nil
	...
	for i = 1, #templates do
		...
		if grid_size[1] < accumulated_width + next_width then
...
function CharacterAppearanceView:_generate_final_backstory_text()
...
function CharacterAppearanceView:_randomize_character_name(input_widget)
...
function CharacterAppearanceView:_randomize_companion_name(input_widget)
...
function CharacterAppearanceView:_set_camera(camera_focus, no_height_compensation, time)
	time = time or 1
...
function CharacterAppearanceView:_check_widget_choice_detail_visibility(grid_index, widget_index)
	local page_grid = self._page_grids[grid_index]
...
function CharacterAppearanceView:_set_camera_height_option(camera_focus)
...
function CharacterAppearanceView:_fetch_suggested_names()
	local profile = self._character_create:profile()
	local selected_archetype = profile.archetype.name
...
function CharacterAppearanceView:_check_input_errors(widget, name)
...
function CharacterAppearanceView:_update_character_custom_name(widget, name)
	self._character_name_status.custom = true

	self:_update_character_name(widget, name)
end

function CharacterAppearanceView:_update_character_name(widget, name)
	self:_check_input_errors(widget, name)
	self._character_create:set_name(name)
end

function CharacterAppearanceView:_update_companion_custom_name(widget, name)
	self._companion_name_status.custom = true

	self:_update_companion_name(widget, name)
end

function CharacterAppearanceView:_update_companion_name(widget, name)
	self:_check_input_errors(widget, name)
	self._character_create:set_companion_name(name)
end

function CharacterAppearanceView:_stop_name_input(input_widget)
	if input_widget then
...
function CharacterAppearanceView:_grid_navigation(direction)
	...
	local selected_widget_data = nil
	...
	if direction == "left" then
		...
		for i = 1, #widgets_data do
			...
			if widget_data.index == current_widget_data.index then
				local closest_value, closest_x_distance, closest_y_distance = nil
...
function CharacterAppearanceView:_update_navigation(grid_index, widget_index, apply_focus)
...
function CharacterAppearanceView:_revert_all_gamepad_focused_loadout()
...
function CharacterAppearanceView:_remove_all_focus()
	for i = 1, #self._page_grids do
		local page_grid = self._page_grids[i]
