...
function utils.extract_changed_values(template, new_value)
	local values = {}
...
function utils.add_side_effect_values(dest, new)
...
function utils.save_account_settings(location_name, settings_name, value)
...
function utils.get_account_settings(location_name, settings_name)
	local player = Managers.player:local_player(1)
	local save_manager = Managers.save
...
function utils.get_user_setting(location, key)
	if location then
...
function utils.set_user_setting(location, key, value)
	if location then
...
function utils.generate_settings_options(settings_definitions)
	local settings = {}
...
function utils.get_default_account_save_data(template)
