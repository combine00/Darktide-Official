...
function CraftingMechanicusModifyView:init(settings, context)
...
function CraftingMechanicusModifyView:on_enter()
...
function CraftingMechanicusModifyView:_setup_crafting_recipe(reference_name, scenegraph_id)
	local layer = 10
...
function CraftingMechanicusModifyView:_setup_sort_options()
...
function CraftingMechanicusModifyView:on_resolution_modified(scale)
...
function CraftingMechanicusModifyView:_update_weapon_stats_position(scenegraph_id, weapon_stats)
...
function CraftingMechanicusModifyView:_set_selected_grid(new_selected_grid)
...
function CraftingMechanicusModifyView:update(dt, t, input_service)
...
function CraftingMechanicusModifyView:_handle_input(input_service)
	if self._using_cursor_navigation then
		return
	end
...
function CraftingMechanicusModifyView:cb_on_recipe_button_pressed(_, config)
	self._parent:previously_active_view_name()
	self._parent:go_to_crafting_view(config.recipe.view_name, self._previewed_item)
end

function CraftingMechanicusModifyView:_preview_item(item)
...
function CraftingMechanicusModifyView:on_exit()
...
function CraftingMechanicusModifyView:_cb_fetch_inventory_items(items, masteries_data)
	if self._destroyed then
		return
	end
...
function CraftingMechanicusModifyView:_setup_menu_tabs(content)
	local id = "tab_menu"
	local layer = 10
...
function CraftingMechanicusModifyView:_on_navigation_input_changed()
...
function CraftingMechanicusModifyView:ui_renderer()
