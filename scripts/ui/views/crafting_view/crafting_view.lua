...
function CraftingView:show_wallets(show)
...
function CraftingView:go_to_crafting_view(view_name, item)
	...
	if crafting_level_story_event then
		...
		if previous_crafting_level_story_event then
			local reverse_privious = story_level_order[crafting_level_story_event] <= story_level_order[previous_crafting_level_story_event]
...
function CraftingView:previously_active_view_name()
	return self._previously_active_view_name
end

function CraftingView:_close_active_view(is_handling_new_view)
	local world_spawner = self._world_spawner
...
function CraftingView:_switch_tab(index)
...
function CraftingView:_switch_tab_view(index)
	local tab_bar_menu = self._elements.tab_bar
...
function CraftingView:_handle_back_pressed()
...
function CraftingView:craft(recipe, ingredients, callback, done_callback, additional_context)
...
function CraftingView:_loadout_refresh(item)
