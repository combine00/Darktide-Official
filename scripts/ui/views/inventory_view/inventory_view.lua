...
function InventoryView:_get_inventory_item_by_id(gear_id)
	if not gear_id then
		return
	end
...
function InventoryView:_character_save_data()
...
function InventoryView:_setup_item_stats(reference_name, scenegraph_id)
	local layer = 10
...
function InventoryView:_update_item_stats_position(scenegraph_id, item_stats)
	local position = self:_scenegraph_world_position(scenegraph_id)

	item_stats:set_pivot_offset(position[1], position[2])
end

function InventoryView:_on_item_hover_start(item)
...
function InventoryView:_on_item_hover_stop()
	self._currently_hovered_item = nil
...
function InventoryView:_set_camera_focus_by_slot_name(slot_name, optional_camera_settings, force_instant_camera)
...
function InventoryView:event_force_wallet_update()
	self:_request_wallets_update()
end
...
function InventoryView:_create_exclamation_widget_from_config(config, suffix, scenegraph_id)
	local widget_type = config.widget_type
	local template = ContentBlueprints[widget_type]
	local size = template.size_function and template.size_function(self, config) or template.size
...
function InventoryView:_draw_elements(dt, t, ui_renderer, render_settings, input_service)
...
function InventoryView:_first_interactable_loadout_widget_index()
...
function InventoryView:_find_closest_widget_node_neighbour(direction)
...
function InventoryView:_get_coordinates_from_widget(widget)
...
function InventoryView:_find_closest_value(list, start_coordinates, direction, threshold, current_index, result_index)
...
function InventoryView:profile_preset_handling_input()
...
function InventoryView:on_item_stats_toggled(force_value, do_save)
...
function InventoryView:is_item_stats_toggled()
