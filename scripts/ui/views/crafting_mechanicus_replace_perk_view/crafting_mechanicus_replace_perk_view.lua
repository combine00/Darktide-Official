local CraftingMechanicusReplacePerkViewDefinitions = require("scripts/ui/views/crafting_mechanicus_replace_perk_view/crafting_mechanicus_replace_perk_view_definitions")
...
local CraftingMechanicusReplacePerkView = class("CraftingMechanicusReplacePerkView", "BaseView")

function CraftingMechanicusReplacePerkView:init(settings, context)
	CraftingMechanicusReplacePerkView.super.init(self, CraftingMechanicusReplacePerkViewDefinitions, settings, context)
...
function CraftingMechanicusReplacePerkView:on_enter()
	CraftingMechanicusReplacePerkView.super.on_enter(self)
	...
	self:_cb_fetch_perks_data():next(function ()
...
function CraftingMechanicusReplacePerkView:on_exit()
	...
	CraftingMechanicusReplacePerkView.super.on_exit(self)
end

function CraftingMechanicusReplacePerkView:_cb_fetch_perks_data()
	return self._perks_item:present_perks(self._item, self._ingredients, callback(self, "cb_on_new_perk_selected")):next(function (max_rank)
...
function CraftingMechanicusReplacePerkView:_get_wallet()
...
function CraftingMechanicusReplacePerkView:_present_crafting(optional_present_callback)
	...
	self._crafting_recipe:present_recipe(self._recipe, self._ingredients, nil, callback(self, "cb_on_perk_selected"), on_present_recipe_callback, self._can_craft_context, CraftingSettings.type)
...
function CraftingMechanicusReplacePerkView:_set_perks_item_focused(focus)
...
function CraftingMechanicusReplacePerkView:on_back_pressed()
...
function CraftingMechanicusReplacePerkView:cb_on_main_button_pressed()
	if self._craft_promise then
		return
	end
...
function CraftingMechanicusReplacePerkView:_perform_crafting()
	if self._craft_promise then
		return
	end
...
function CraftingMechanicusReplacePerkView:cb_on_perk_selected(widget, config)
...
function CraftingMechanicusReplacePerkView:_on_perk_selected(widget, config)
...
function CraftingMechanicusReplacePerkView:_draw_elements(dt, t, ui_renderer, render_settings, input_service)
...
function CraftingMechanicusReplacePerkView:_update_elements(dt, t, input_service)
	local existing_perk_index = self._ingredients.existing_perk_index
	local elements_array = self._elements_array
...
function CraftingMechanicusReplacePerkView:remove_ingredient(ingredient_index)
...
function CraftingMechanicusReplacePerkView:update(dt, t, input_service)
	if self._resync_can_craft then
		local marked_perk_item = self._perks_item:marked_perk_item()
		local marked_cost = self._perks_item:marked_perk_cost() or 0
		...
		self._can_craft_context.marked_perk_cost = marked_cost
		local perk_widget = self._crafting_recipe:selected_grid_widget()
		self._can_craft_context.current_perk_cost = perk_widget and perk_widget.content.cost or 0
	...
	return CraftingMechanicusReplacePerkView.super.update(self, dt, t, input_service)
end

function CraftingMechanicusReplacePerkView:cb_on_new_perk_selected(widget, config)
	self._resync_can_craft = true
end

function CraftingMechanicusReplacePerkView:_handle_input(input_service)
	...
	CraftingMechanicusReplacePerkView.super._handle_input(self, input_service)
end

function CraftingMechanicusReplacePerkView:_on_navigation_input_changed()
...
function CraftingMechanicusReplacePerkView:dialogue_system()
