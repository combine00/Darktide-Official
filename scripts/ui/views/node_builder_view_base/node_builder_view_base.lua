...
function NodeBuilderViewBase:init(definitions, settings, context)
...
function NodeBuilderViewBase:on_enter()
...
function NodeBuilderViewBase:_setup_node_connection_widget()
	self._node_connection_widget = self:_create_widget("node_connection", self._definitions.default_node_connection_definition)
end

function NodeBuilderViewBase:_setup_layouts()
	return
end

function NodeBuilderViewBase:on_exit()
	NodeBuilderViewBase.super.on_exit(self)
end

function NodeBuilderViewBase:_destroy_node_widgets()
...
function NodeBuilderViewBase:_destroy_node_widget(widget_name)
...
function NodeBuilderViewBase:_get_node_definition_by_type(node_type)
...
function NodeBuilderViewBase:_create_node_widget(node)
...
function NodeBuilderViewBase:_on_node_widget_left_pressed(widget)
	local success = false
...
function NodeBuilderViewBase:_has_points_spent_in_children(node)
...
function NodeBuilderViewBase:_has_points_spent_in_parents(node)
	local parents = node.parents
...
function NodeBuilderViewBase:_is_nodes_bridge_linked(node_1, node_2)
...
function NodeBuilderViewBase:_has_node_bridge_links(node)
...
function NodeBuilderViewBase:_can_remove_point_in_node(node)
...
function NodeBuilderViewBase:_can_node_traverse_to_start(node, ignore_list, step_count)
...
function NodeBuilderViewBase:_is_node_dependent_on_parent(node, parent_node)
	local points_spent_on_node_widgets = self._points_spent_on_node_widgets
...
function NodeBuilderViewBase:_on_node_widget_right_pressed(widget)
	local success = false
...
function NodeBuilderViewBase:_get_scenegraph_definition(name)
	local scenegraph_definition = self._definitions.scenegraph_definition

	return scenegraph_definition[name]
end

function NodeBuilderViewBase:apply_active_background_size()
...
function NodeBuilderViewBase:_add_scenegraph_definition(name, definition)
	local scenegraph_definition = self._definitions.scenegraph_definition
...
function NodeBuilderViewBase:_activate_layout_by_name(name)
...
function NodeBuilderViewBase:recreate_widget_for_node(node)
...
function NodeBuilderViewBase:_increase_render_priority_of_node(priority_node)
...
function NodeBuilderViewBase:_add_node(x, y)
...
function NodeBuilderViewBase:_refresh_all_nodes()
	local active_layout = self:get_active_layout()
	local nodes = active_layout.nodes
...
function NodeBuilderViewBase:_remove_node(widget_name)
	local active_layout = self:get_active_layout()
	active_layout.dirty = true
	local nodes = active_layout.nodes
...
function NodeBuilderViewBase:get_active_layout()
	return self._active_layout
end

function NodeBuilderViewBase:get_active_layout_index()
	for i = 1, #self._layouts do
		local layout = self._layouts[i]
...
function NodeBuilderViewBase:selected_node_widget()
...
function NodeBuilderViewBase:node_widgets()
	return self._node_widgets
end

function NodeBuilderViewBase:_set_active_layout_by_index(index)
...
function NodeBuilderViewBase:post_update(dt, t, input_service)
	return NodeBuilderViewBase.super.post_update(self, dt, t, input_service)
end

function NodeBuilderViewBase:can_exit()
	return self._can_close
end

function NodeBuilderViewBase:update(dt, t, input_service)
	local active_layout = self:get_active_layout()
...
function NodeBuilderViewBase:_node_widget_hovered(dt, t)
...
function NodeBuilderViewBase:on_resolution_modified(scale)
	self:_force_update_scenegraph()
end

function NodeBuilderViewBase:_update_button_statuses(dt, t)
	local widgets_by_name = self._widgets_by_name
end

function NodeBuilderViewBase:_update_node_widgets()
	local widgets = self._node_widgets
...
function NodeBuilderViewBase:_setup_tooltip_info(node, instant_tooltip)
	return
end

function NodeBuilderViewBase:_on_node_hover_enter(node)
	return
end

local temp_node_table = {}

function NodeBuilderViewBase:_nodes_in_exclusive_group(group_name)
...
function NodeBuilderViewBase:_node_with_incompatible_talent_is_selected(incompatible_talent)
	local layout = self:get_active_layout()
	local points_spent_on_node_widgets = self._points_spent_on_node_widgets
	local nodes = layout.nodes
...
function NodeBuilderViewBase:_node_incompatible_with_talent_is_selected(incompatible_talent)
	local layout = self:get_active_layout()
	local points_spent_on_node_widgets = self._points_spent_on_node_widgets
	local nodes = layout.nodes
...
function NodeBuilderViewBase:_node_by_name(name)
	local layout = self:get_active_layout()
	local nodes = layout.nodes
...
function NodeBuilderViewBase:_points_spent_in_group(group_name)
...
function NodeBuilderViewBase:_points_spent_on_node_in_exclusive_group(group_name)
	local points_spent_on_node_widgets = self._points_spent_on_node_widgets
	local layout = self:get_active_layout()
	local nodes = layout.nodes
...
function NodeBuilderViewBase:_node_availability_status(node, can_always_afford)
	local widget_name = node.widget_name
	local points_spent_on_node_widgets = self._points_spent_on_node_widgets
...
function NodeBuilderViewBase:_node_points_by_widget(widget)
...
function NodeBuilderViewBase:clear_node_points()
	table.clear(self._points_spent_on_node_widgets)

	self._node_points_spent = 0
end

function NodeBuilderViewBase:_max_node_points()
	local active_layout = self:get_active_layout()
...
function NodeBuilderViewBase:_points_available()
	local active_layout = self:get_active_layout()
...
function NodeBuilderViewBase:_add_node_point_on_widget(widget)
	local active_layout = self:get_active_layout()
...
function NodeBuilderViewBase:active_layout_node_by_name(name)
	local active_layout = self:get_active_layout()
...
function NodeBuilderViewBase:_remove_link_between_nodes(selected_node, pressed_node)
	local active_layout = self:get_active_layout()
...
function NodeBuilderViewBase:_assign_link_between_nodes(selected_node, pressed_node)
...
function NodeBuilderViewBase:_remove_node_point_on_widget(widget)
...
function NodeBuilderViewBase:_set_zoom(zoom)
...
function NodeBuilderViewBase:render_scale()
	return
end

function NodeBuilderViewBase:_force_update_scenegraph(ui_scenegraph)
...
function NodeBuilderViewBase:_background_position()
	local widgets_by_name = self._widgets_by_name
	local layout_background_widget = widgets_by_name.layout_background
	local scenegraph_id = layout_background_widget.scenegraph_id
...
function NodeBuilderViewBase:_background_size()
	local active_layout = self:get_active_layout()
...
function NodeBuilderViewBase:_get_text_height(text, text_style, optional_text_size)
	local ui_renderer = self._ui_renderer
...
function NodeBuilderViewBase:_allowed_node_input()
...
function NodeBuilderViewBase:_can_drag_background()
	return not self._hovered_node_widget
end

local _temp_cursor_drag_start_position = {}

function NodeBuilderViewBase:_handle_input(input_service, dt, t)
	local active_layout = self:get_active_layout()
...
function NodeBuilderViewBase:_on_input_scroll_axis_changed(scroll_value, cursor_position)
	...
	local cursor_x = cursor_position[1] / render_scale
	local cursor_y = cursor_position[2] / render_scale
	local zoom_ratio = zoom / current_zoom
	local new_cursor_x = cursor_x * zoom_ratio
	local new_cursor_y = cursor_y * zoom_ratio
	local diff_x = new_cursor_x - cursor_x
	local diff_y = new_cursor_y - cursor_y
	scenegraph_settings.x = x - diff_x
	scenegraph_settings.y = y - diff_y
end

function NodeBuilderViewBase:_get_location_pixel_size_by_layout_size(layout_size, location_fraction_x, location_fraction_y)
	return layout_size[1] * location_fraction_x, layout_size[2] * location_fraction_y
end

function NodeBuilderViewBase:_refresh_coordinates(scenegraph_settings, is_node)
...
function NodeBuilderViewBase:safe_rect()
	local safe_rect_default_value = 0

	return safe_rect_default_value * 0.01
end

function NodeBuilderViewBase:_get_or_create_scenegraph_settings(scenegraph_id, position_x, position_y, is_node)
...
function NodeBuilderViewBase:_handle_scenegraph_coordinates(widget_name, scenegraph_id, input_service, move_input_prefix, render_settings, is_background, optional_scenegraph_settings, cursor_start_position, cursor_pixel_threshold)
	...
	if self._dragging_scenegraph_id then
		...
		local final_x = scenegraph_settings.x
		local final_y = scenegraph_settings.y
...
function NodeBuilderViewBase:_update_scenegraph_positions()
...
function NodeBuilderViewBase:draw(dt, t, input_service, layer)
...
function NodeBuilderViewBase:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
	NodeBuilderViewBase.super._draw_widgets(self, dt, t, input_service, ui_renderer, render_settings)
end

function NodeBuilderViewBase:draw_layout(dt, t, input_service, layer)
	local hovering_input_surface = self._widgets_by_name.input_surface.content.hotspot.is_hover
...
function NodeBuilderViewBase:_draw_layout_node_connections(dt, t, input_service, ui_renderer, render_settings, layout)
...
function NodeBuilderViewBase:_draw_connection_between_widgets(ui_renderer, visible, dt, parent_node, child_node, offset_x, offset_y, distance, angle)
	local node_connection_widget = self._node_connection_widget
...
function NodeBuilderViewBase:_apply_node_connection_anims(node_connection_widget, parent_node, dt)
	return
end

function NodeBuilderViewBase:_apply_node_connection_line_colors(color_status)
	local node_connection_widget = self._node_connection_widget
