...
function InventoryCosmeticsView:_load_layout(selected_slot)
...
function InventoryCosmeticsView:event_force_refresh_inventory()
...
function InventoryCosmeticsView:_destroy_side_panel()
...
function InventoryCosmeticsView:_setup_side_panel(item, is_locked, dx, dy)
	self:_destroy_side_panel()
...
function InventoryCosmeticsView:_trigger_zoom_logic(optional_slot_name, optional_time)
	local selected_slot = self._selected_slot
...
function InventoryCosmeticsView:_filtered_layout()
...
function InventoryCosmeticsView:_requested_index_on_present()
	local requested_index = nil
...
function InventoryCosmeticsView:_start_show_layout()
...
function InventoryCosmeticsView:_verify_items(source_items, owned_gear)
	local selected_slot = self._selected_slot
	local selected_slot_name = selected_slot.name
...
function InventoryCosmeticsView:_parse_store_items(selected_slot_name, offers, items)
	for i = 1, #offers do
...
function InventoryCosmeticsView:_fetch_inventory_items(selected_slot)
	...
	local promises = {
		[#promises + 1] = self._promise_container:cancel_on_destroy(Managers.data_service.gear:fetch_inventory(character_id, filter)):next(function (items)
		...
		[#promises + 1] = self._promise_container:cancel_on_destroy(Managers.data_service.store:get_credits_cosmetics_store()):next(function (data)
...
function InventoryCosmeticsView:_achievement_items(selected_slot_name)
...
function InventoryCosmeticsView:_prepare_cosmetic_layout_data(result)
...
function InventoryCosmeticsView:is_item_owned(target_gear_id)
...
function InventoryCosmeticsView:_update_equip_button_status()
	...
	if is_equipped then
		target_status = "equipped"
	elseif is_premium then
		target_status = "purchase"
	elseif is_disabled or is_locked then
...
function InventoryCosmeticsView:profile_preset_handling_input()
...
function InventoryCosmeticsView:draw(dt, t, input_service, layer)
...
function InventoryCosmeticsView:_update_zoom_logic(dt, input_service)
...
function InventoryCosmeticsView:_should_refresh(dt, t)
...
function InventoryCosmeticsView:_set_camera_item_slot_focus(slot_name, time, func_ptr, zoom_level)
	local world_spawner = self._world_spawner
...
function InventoryCosmeticsView:_can_preview_voice()
...
function InventoryCosmeticsView:_stop_current_voice()
...
function InventoryCosmeticsView:cb_preview_voice()
...
function InventoryCosmeticsView:cb_on_camera_zoom_toggled()
...
function InventoryCosmeticsView:cb_on_purchase_pressed()
...
function InventoryCosmeticsView:_update_vo(dt, t)
...
function InventoryCosmeticsView:dialogue_system()
	local world_spawner = self._world_spawner
...
function InventoryCosmeticsView:_cb_on_play_vo(id, event_name)
	self._current_vo_event = event_name
	self._current_vo_id = id
end

function InventoryCosmeticsView:play_vo_events(events, voice_profile, optional_route_key, optional_delay, is_opinion_vo)
