...
function ViewElementExpertiseStats:init(parent, draw_layer, start_scale, optional_menu_settings)
...
function ViewElementExpertiseStats:destroy(ui_renderer)
	ViewElementExpertiseStats.super.destroy(self, ui_renderer)
end

function ViewElementExpertiseStats:show_overlay(show)
...
function ViewElementExpertiseStats:start_animation()
	local ui_scenegraph = self._ui_scenegraph
	local func = UIAnimation.function_by_time
...
function ViewElementExpertiseStats:set_alpha_multiplier(alpha_multiplier)
	self._alpha_multiplier = alpha_multiplier or 0
end

function ViewElementExpertiseStats:draw(dt, t, ui_renderer, render_settings, input_service)
...
function ViewElementExpertiseStats:_update_animations(dt, t)
...
function ViewElementExpertiseStats:update(dt, t, input_service)
	ViewElementExpertiseStats.super.update(self, dt, t, input_service)
end

function ViewElementExpertiseStats:_handle_input(dt, t, input_service)
	return
end

function ViewElementExpertiseStats:_verify_weapon(item)
	if not item then
		return false
	end
...
function ViewElementExpertiseStats:present(item, expertise_data)
...
function ViewElementExpertiseStats:_present(item, expertise_data)
...
function ViewElementExpertiseStats:_add_stats_to_layout(item, layout)
...
function ViewElementExpertiseStats:_cb_present_grid_layout()
	return
end

function ViewElementExpertiseStats:_on_navigation_input_changed()
	ViewElementExpertiseStats.super._on_navigation_input_changed(self)
end

function ViewElementExpertiseStats:update_preview_values(expertise_data)
...
function ViewElementExpertiseStats:_update_modification_cap(use_start)
