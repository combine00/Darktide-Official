...
function ViewElementInventoryWeaponPreview:center_align(duration, additional_position_offset)
	...
	world_spawner:set_target_camera_position(target_world_position.x + extra_position_x, target_world_position.y + extra_position_y, target_world_position.z + extra_position_z, time, func_ptr)
...
function ViewElementInventoryWeaponPreview:set_weapon_zoom(fraction, use_custom_zoom, optional_node_name, optional_pos, optional_min_zoom, optional_max_zoom)
	local world_spawner = self._world_spawner
	local ui_weapon_spawner = self._ui_weapon_spawner
	...
	if use_custom_zoom and ui_weapon_spawner then
		...
		if spawn_data then
			...
			if offset_pos then
				...
				local percent = math.remap(optional_min_zoom or 0, optional_max_zoom or 1, 0, 1, amount)
				...
				world_spawner:set_target_camera_offset(math.lerp(default_x, diff[1], percent), amount, math.lerp(default_z, diff[3], percent))
	...
	world_spawner:set_target_camera_offset(nil, amount, nil)
end
...
function ViewElementInventoryWeaponPreview:set_force_allow_rotation(allow)
...
function ViewElementInventoryWeaponPreview:destroy(ui_renderer)
