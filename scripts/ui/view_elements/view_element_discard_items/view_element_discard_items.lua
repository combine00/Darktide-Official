...
function ViewElementDiscardItems:init(parent, draw_layer, start_scale, optional_menu_settings)
...
function ViewElementDiscardItems:_initialize_all(items, display_name, selection_callback, unselected_callback)
...
function ViewElementDiscardItems:_sort_items_by_rarity(items)
...
function ViewElementDiscardItems:_update_selected_items_by_rarity()
...
function ViewElementDiscardItems:update(dt, t, input_service)
...
function ViewElementDiscardItems:draw(dt, t, ui_renderer, render_settings, input_service)
	if self._input_disabled then
		input_service = input_service:null_service()
	end
...
function ViewElementDiscardItems:_setup_buttons_interactions()
	local widgets_by_name = self._widgets_by_name
...
function ViewElementDiscardItems:_initialize_description_text()
	local widgets_by_name = self._widgets_by_name
...
function ViewElementDiscardItems:_initialize_rarity_options()
	local items_by_rarity = self._items_by_rarity
...
function ViewElementDiscardItems:_on_rarity_button_pressed(widget, rarity_index)
...
function ViewElementDiscardItems:refresh(items)
	if items then
...
function ViewElementDiscardItems:_update_items_to_discard()
	local items_by_rarity = self._items_by_rarity
...
function ViewElementDiscardItems:_can_discard_item(item)
...
function ViewElementDiscardItems:add_items()
	return
end

function ViewElementDiscardItems:_on_select_pressed()
...
function ViewElementDiscardItems:_on_unselect_pressed()
	local items_to_discard = self._items_to_discard
...
function ViewElementDiscardItems:_on_rating_stepper_left_pressed()
	self:_increment_rating_value(-1 * ItemUtils.get_expertise_multiplier())
end

function ViewElementDiscardItems:_on_rating_stepper_right_pressed()
	self:_increment_rating_value(1 * ItemUtils.get_expertise_multiplier())
end

function ViewElementDiscardItems:_increment_rating_value(add)
...
function ViewElementDiscardItems:_on_navigation_input_changed()
...
function ViewElementDiscardItems:_set_selected_gamepad_navigation_index(index)
	local button_gamepad_navigation_list = self._button_gamepad_navigation_list
	local new_available_index = nil
...
function ViewElementDiscardItems:_handle_button_gamepad_navigation(input_service)
...
function ViewElementDiscardItems:_handle_input(input_service, dt, t)
	self:_handle_button_gamepad_navigation(input_service)
end

function ViewElementDiscardItems:set_pivot_offset(x, y)
...
function ViewElementDiscardItems:input_disabled()
