...
function ViewElementNewsSlide:init(parent, draw_layer, start_scale)
...
function ViewElementNewsSlide:destroy()
...
local function _compare_slides(a, b)
	...
	local b_index = b.sort_index or 0

	return a_index > b_index
end

function ViewElementNewsSlide:_initialize_slides(backend_data)
	...
	for i = 1, #backend_data do
		...
		local title, image_url, body_text, body_number = nil
...
function ViewElementNewsSlide:_slide_count()
	local slides = self._slide_data

	return slides and #slides or 0
end

function ViewElementNewsSlide:_has_progress_bar()
	return self:_slide_count() > 1
end

function ViewElementNewsSlide:_create_progress_bar()
...
function ViewElementNewsSlide:_delete_progress_bar()
...
function ViewElementNewsSlide:_position_progress_bar()
	local widgets = self._progress_widgets
	local item_count = widgets and #widgets or 0
	...
	local width = 0
	local old_index = self._old_index
...
function ViewElementNewsSlide:_request_slide_change(target)
...
function ViewElementNewsSlide:_default_transition_time()
	return self:_slide_count() == 2 and Settings.transition_time or 0
end

function ViewElementNewsSlide:_change_slide(target, optional_start_time)
...
local function _top_center_aspect_uvs(target_x, target_y, real_x, real_y)
	...
	y_ratio = y_ratio / ratio
	x_ratio = x_ratio / ratio
	local x_diff = (1 - x_ratio) / 2

	return x_diff, 0, 1 - x_diff, y_ratio
end

function ViewElementNewsSlide:_update_slide_image()
...
function ViewElementNewsSlide:update(dt, t, input_service)
	if self:_has_progress_bar() then
		self._slide_time = self._slide_time + dt
	end

	if Settings.total_time <= self._slide_time then
		local target = self._current_index + 1

		if self:_slide_count() < target then
			target = 1
		end
...
function ViewElementNewsSlide:_get_text_dimensions(pass_name, ui_renderer)
	local news_button = self._widgets_by_name.news_button
...
function ViewElementNewsSlide:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
	local news_button = self._widgets_by_name.news_button
...
function ViewElementNewsSlide:view_requested()
	local current_index = self._current_index
...
function ViewElementNewsSlide:can_open_view()
	local current_index = self._current_index
	local slide_data = self._slide_data and self._slide_data[current_index]
	local backend_index = slide_data and slide_data.backend_index
	local backend_data = self._backend_data and self._backend_data[backend_index]
