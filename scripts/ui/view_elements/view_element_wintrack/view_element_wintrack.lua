...
function ViewElementWintrack:_destroy_grid_gui()
...
function ViewElementWintrack:assign_rewards(rewards, points_per_reward)
...
function ViewElementWintrack:is_initialized()
	return self._initialize
end

function ViewElementWintrack:_replace_border()
...
function ViewElementWintrack:destroy(ui_renderer)
...
function ViewElementWintrack:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function ViewElementWintrack:_is_reward_widget_visible(widget, extra_margin)
...
function ViewElementWintrack:draw_reward_widgets(dt, t, input_service, render_settings)
...
function ViewElementWintrack:on_resolution_modified(scale)
	ViewElementWintrack.super.on_resolution_modified(self, scale)
end

function ViewElementWintrack:draw(dt, t, ui_renderer, render_settings, input_service)
...
function ViewElementWintrack:_draw_reward_render_target(render_settings)
	local ui_reward_renderer = self._ui_reward_renderer
...
function ViewElementWintrack:on_reward_claimed(index)
	local reward_widgets = self._reward_widgets
	local reward_item_widgets = self._reward_item_widgets
...
function ViewElementWintrack:set_handle_input(handle_input)
	self._handle_input = handle_input
end

function ViewElementWintrack:update(dt, t, input_service)
	...
	local visual_points, added_points = nil
...
function ViewElementWintrack:_update_claim_button_state(force_effect)
	local widgets_by_name = self._widgets_by_name
...
function ViewElementWintrack:play_claim_button_animation()
...
function ViewElementWintrack:stop_claim_button_animation()
...
function ViewElementWintrack:_on_navigation_input_changed()
...
function ViewElementWintrack:_update_reward_tooltip_hint()
...
function ViewElementWintrack:_get_progress_bars_width()
...
function ViewElementWintrack:_handle_reward_scroll(input_service, dt)
...
function ViewElementWintrack:_handle_track_scroll(input_service, dt)
...
function ViewElementWintrack:set_pivot_offset(x, y)
...
function ViewElementWintrack:visual_points()
	return self._visual_points or self._points
end

function ViewElementWintrack:points()
	return self._points
end

function ViewElementWintrack:max_points()
	return self._max_points
end

function ViewElementWintrack:add_points(points_to_add, animate, force_reward_page_change)
	if self._max_points <= self._points then
		return
	end
...
function ViewElementWintrack:_create_reward_widgets(rewards, ui_renderer)
	local widgets = {}
	local item_widgets = {}
	...
	for i = 1, amount do
		...
		if item_pass_template then
			...
			local callback_name, secondary_callback_name, double_click_callback = nil
...
function ViewElementWintrack:_get_progress_required_to_page(index)
...
function ViewElementWintrack:_update_animations(dt, t)
...
function ViewElementWintrack:_update_reward_widget_positions(dt, t)
	local bar_width = self:_get_progress_bars_width()
...
function ViewElementWintrack:_update_reward_item_widgets_visibility(ui_renderer)
...
function ViewElementWintrack:_set_bars_progress(experience_progress, reward_progress, animate)
	if animate then
...
function ViewElementWintrack:_animate_progress_bars(dt, t)
...
function ViewElementWintrack:_cb_on_page_indicator_pressed(index)
	return
end

function ViewElementWintrack:_set_page_indicator_focus(index)
	local page_tumb_widgets = self._page_tumb_widgets
...
function ViewElementWintrack:_setup_item_stats(reference_name, scenegraph_id)
...
function ViewElementWintrack:_update_item_stats_position(scenegraph_id, item_stats)
...
function ViewElementWintrack:currently_hovered_item()
	return self._currently_hovered_item
end

function ViewElementWintrack:_on_reward_items_hover_start(items, index, widget)
	index = index or 1
...
function ViewElementWintrack:_on_reward_items_hover_stop()
...
function ViewElementWintrack:focus_on_reward(index)
	local reward_item_widgets = self._reward_item_widgets
...
function ViewElementWintrack:remove_focus_on_reward()
...
function ViewElementWintrack:apply_focus_on_reward()
