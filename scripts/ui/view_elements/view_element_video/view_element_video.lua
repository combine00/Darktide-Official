...
function ViewElementVideo:init(parent, draw_layer, start_scale)
...
function ViewElementVideo:destroy(ui_renderer)
	self:_destroy_current_video()
	ViewElementVideo.super.destroy(self, ui_renderer)
end

function ViewElementVideo:set_pivot_offset(x, y)
	self:_set_scenegraph_position("pivot", x, y)
end

function ViewElementVideo:update(dt, t, input_service)
...
function ViewElementVideo:draw(dt, t, _, render_settings, input_service)
...
function ViewElementVideo:_destroy_current_video()
	self._playing = false
...
function ViewElementVideo:setup_video(video_name, loop_video, video_ui_renderer)
	self:_destroy_current_video()
...
function ViewElementVideo:set_playback_speed(speed)
...
function ViewElementVideo:play_video()
	local video_player = self:_video_player()
	local restart = false
...
function ViewElementVideo:is_playing()
	return not self._video_completed and self._video_speed ~= 0 and self._playing
end

function ViewElementVideo:stop_video()
	self:_destroy_current_video()
end

function ViewElementVideo:pause_video()
	local video_player = self:_video_player()
...
function ViewElementVideo:_video_player()
	local video_player_reference = self._video_player_reference
