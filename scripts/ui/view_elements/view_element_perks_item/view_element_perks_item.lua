...
function ViewElementPerksItem:init(parent, draw_layer, start_scale, optional_menu_settings)
...
function ViewElementPerksItem:destroy(ui_renderer)
...
function ViewElementPerksItem:show_overlay(show)
...
function ViewElementPerksItem:clear_marks()
...
function ViewElementPerksItem:cb_on_grid_entry_left_pressed(widget, config)
...
function ViewElementPerksItem:marked_perk_item()
	return self._marked_perk_item
end

function ViewElementPerksItem:marked_perk_cost()
	return self._marked_widget and self._marked_widget.content.cost
end

function ViewElementPerksItem:_on_perk_hover(config)
	self._hovered_perk_item = config.perk_item
end

function ViewElementPerksItem:start_animation()
	local ui_scenegraph = self._ui_scenegraph
	local func = UIAnimation.function_by_time
...
function ViewElementPerksItem:disable()
	self._disabled = true

	self:select_grid_index(nil)
end

function ViewElementPerksItem:enable()
...
function ViewElementPerksItem:hide()
	self._active = false
end

function ViewElementPerksItem:show()
	self._active = true
end

function ViewElementPerksItem:active()
	return self._active and not self._disabled
end

function ViewElementPerksItem:select_best_widget(allow_only_marked_widget)
...
function ViewElementPerksItem:set_alpha_multiplier(alpha_multiplier)
	self._alpha_multiplier = alpha_multiplier or 0
end

function ViewElementPerksItem:draw(dt, t, ui_renderer, render_settings, input_service)
...
function ViewElementPerksItem:_update_animations(dt, t)
...
function ViewElementPerksItem:present_perks(item, ingredients, external_left_click_callback, do_animation)
	if not item then
		self._active = true
		self._disabled = false
...
function ViewElementPerksItem:_setup_tabs()
	local tab_settings = {
...
function ViewElementPerksItem:_switch_to_rank_tab(rank, initializing)
	self._rank = rank
...
function ViewElementPerksItem:update(dt, t, input_service)
	if not self._active then
		return
	end
...
function ViewElementPerksItem:_update_tabs()
...
function ViewElementPerksItem:_handle_input(dt, t, input_service)
...
function ViewElementPerksItem:ingredients()
	return self._ingredients
end

function ViewElementPerksItem:_present(first_presentation)
...
function ViewElementPerksItem:_cb_present_grid_layout()
...
function ViewElementPerksItem:_on_navigation_input_changed()
