...
function ViewElementWallet:init(parent, draw_layer, start_scale)
...
function ViewElementWallet:_generate_currencies(currencies_to_use, widget_size, wallets_per_column, grid_spacing)
	local widgets = {}
...
function ViewElementWallet:set_pivot_offset(x, y, horizontal_alignment, vertical_alignment)
...
function ViewElementWallet:update_wallets()
...
function ViewElementWallet:_get_wallet_text_size(widget, currency_name, ui_renderer)
...
function ViewElementWallet:get_amount_by_currency(currency_name)
	return self._amount_by_currency and self._amount_by_currency[currency_name] or 0
end

function ViewElementWallet:get_wallet_by_type(currency_name)
	return self._wallets_by_type and self._wallets_by_type[currency_name] or nil
end

function ViewElementWallet:_generate_currency_widget(currency_name, currency_data, widget_size)
...
function ViewElementWallet:highlight_currencies(currency_names)
...
function ViewElementWallet:update(dt, t, input_service)
	return ViewElementWallet.super.update(self, dt, t, input_service)
end

function ViewElementWallet:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function ViewElementWallet:get_size()
	return self._ui_scenegraph.wallet_area.size
end

function ViewElementWallet:on_exit()
