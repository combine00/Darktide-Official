...
function RenderTargetIconGeneratorBase:init(render_settings)
...
function RenderTargetIconGeneratorBase:update_all()
...
function RenderTargetIconGeneratorBase:_reset_active_spawning()
...
function RenderTargetIconGeneratorBase:prioritize_request(reference_id)
...
function RenderTargetIconGeneratorBase:_update_request(request, data, prioritized)
	local request_id = request.id
...
function RenderTargetIconGeneratorBase:_request_by_id(request_id, ignore_assert)
	local requests_by_size = self._requests_by_size
...
function RenderTargetIconGeneratorBase:_request_by_reference_id(reference_id)
	local requests_by_size = self._requests_by_size
...
function RenderTargetIconGeneratorBase:has_request(reference_id)
	local request = self:_request_by_reference_id(reference_id)

	return request ~= nil
end

function RenderTargetIconGeneratorBase:_get_key_by_size(size)
	local key_string = tostring(size[1]) .. "x" .. tostring(size[2])

	return key_string
end

function RenderTargetIconGeneratorBase:increment_icon_request_by_reference_id(existing_reference_id)
...
function RenderTargetIconGeneratorBase:_generate_icon_request(request_id_prefix, data, on_load_callback, optional_render_context, prioritized, on_unload_callback, current_reference_id)
...
function RenderTargetIconGeneratorBase:unload_request_reference(reference_id, keep_reference)
...
function RenderTargetIconGeneratorBase:_handle_request_queue()
...
function RenderTargetIconGeneratorBase:_handle_next_request_in_queue()
...
function RenderTargetIconGeneratorBase:_handle_request(request)
	local grid_index = request.grid_index
	local atlas_id = request.atlas_id
...
function RenderTargetIconGeneratorBase:_initialize_world()
...
function RenderTargetIconGeneratorBase:_setup_viewport(custom_size)
...
function RenderTargetIconGeneratorBase:_resume_rendering()
	local world_spawner = self._world_spawner

	world_spawner:set_world_disabled(false)
end

function RenderTargetIconGeneratorBase:_pause_rendering()
	local world_spawner = self._world_spawner

	world_spawner:set_world_disabled(true)
end

function RenderTargetIconGeneratorBase:prepare_for_destruction()
	self._shutting_down = true
end

function RenderTargetIconGeneratorBase:destroy()
...
function RenderTargetIconGeneratorBase:update(dt, t)
	if self._initialized_render_target_atlas then
...
function RenderTargetIconGeneratorBase:_should_render()
	return
end

function RenderTargetIconGeneratorBase:_prepare_request_capture(request)
	return
end

function RenderTargetIconGeneratorBase:_is_ready_to_capture_request()
	return
end

function RenderTargetIconGeneratorBase:_on_capture_complete()
	return
end

function RenderTargetIconGeneratorBase:_camera_unit()
	return
end
