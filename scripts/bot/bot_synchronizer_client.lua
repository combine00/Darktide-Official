...
function BotSynchronizerClient:init(peer_id, network_delegate, host_channel_id)
...
function BotSynchronizerClient:destroy()
	self._network_delegate:unregister_channel_events(self._host_channel_id, unpack(RPCS))
end

function BotSynchronizerClient:rpc_add_bot_player(channel_id, local_player_id, slot)
...
function BotSynchronizerClient:rpc_remove_bot_player(channel_id, local_player_id)
	local peer_id = Network.peer_id(channel_id)
	local player_manager = Managers.player
