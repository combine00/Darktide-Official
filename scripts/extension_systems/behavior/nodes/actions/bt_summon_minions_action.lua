...
function BtSummonMinionsAction:leave(unit, breed, blackboard, scratchpad, action_data, t, reason, destroy)
	return
end

function BtSummonMinionsAction:run(unit, breed, blackboard, scratchpad, action_data, dt, t)
	...
	if not active_timer then
		if scratchpad.shout_wwise_event_timing and scratchpad.shout_wwise_event_timing <= t then
		...
		if not scratchpad.pre_stinger then
			...
			if unit_aggro_state then
				...
				if unit_aggro_state == slot14 then
	...
	if scratchpad.delay < t and not scratchpad.summoned_success then
...
function BtSummonMinionsAction:_summon_minions(unit, breed, blackboard, scratchpad, action_data, dt, t)
	...
	if unit_aggro_state then
		...
		if unit_aggro_state == slot30 then
