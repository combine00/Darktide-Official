...
function BtCompanionApproachAction:run(unit, breed, blackboard, scratchpad, action_data, dt, t)
	...
	if can_start_leap then
		if not scratchpad.skip_fast_jump and scratchpad.forbid_fast_jump_t < t then
			...
			if Vector3.equal(found_flat_direction, Vector3.zero()) then
				...
				if angle <= action_data.max_angle_for_fast_jump and action_data.fast_jump_speed_threshold < speed then
