...
local _setup_progress_bar, _closets_aggro_target = nil

function BtChaosMutatorDaemonhostPassiveAction:enter(unit, breed, blackboard, scratchpad, action_data, t)
...
function BtChaosMutatorDaemonhostPassiveAction:init_values(blackboard, action_data, node_data)
	local statistics_component = Blackboard.write_component(blackboard, "statistics")
	statistics_component.player_deaths = 0
end

function BtChaosMutatorDaemonhostPassiveAction:leave(unit, breed, blackboard, scratchpad, action_data, t, reason, destroy)
...
function BtChaosMutatorDaemonhostPassiveAction:_switch_stage(unit, breed, scratchpad, action_data, stage, t)
...
function BtChaosMutatorDaemonhostPassiveAction:_update(unit, breed, blackboard, scratchpad, action_data, t)
	...
	if scratchpad.speed then
		...
		if scratchpad.t_until_next_stage < scratchpad.timer then
			scratchpad.timer = 0
...
function BtChaosMutatorDaemonhostPassiveAction:_kill(scratchpad, action_data, unit)
	if scratchpad.chanting_effect_id then
		scratchpad.fx_system:stop_template_effect(scratchpad.chanting_effect_id)
...
function BtChaosMutatorDaemonhostPassiveAction:_populate_cultists(scratchpad, action_data, unit)
...
function BtChaosMutatorDaemonhostPassiveAction:_get_alive_cultists(scratchpad)
...
function BtChaosMutatorDaemonhostPassiveAction:_check_damage(scratchpad)
	local cultist_units = scratchpad.chanting_units
...
function BtChaosMutatorDaemonhostPassiveAction:_get_closest_player(unit, position, valid_enemy_player_units, scratchpad, action_data)
	local navigation_extension = ScriptUnit.extension(unit, "navigation_system")
	local nav_world = navigation_extension:nav_world()
	...
	local close_distance_offset = action_data.close_distance_offset
	local far_distance_offset = action_data.far_distance_offset
...
function BtChaosMutatorDaemonhostPassiveAction:_get_players_los(valid_enemy_player_units, scratchpad)
	local perception_extension = scratchpad.perception_extension
...
function BtChaosMutatorDaemonhostPassiveAction:_get_time_until_next_stage(scratchpad)
...
function BtChaosMutatorDaemonhostPassiveAction:run(unit, breed, blackboard, scratchpad, action_data, dt, t)
...
function BtChaosMutatorDaemonhostPassiveAction:_update_looping_vo(unit, breed, scratchpad, action_data, stage, t)
	local stage_settings = action_data.stage_settings[stage]
...
function BtChaosMutatorDaemonhostPassiveAction:_start_chanting(unit, scratchpad)
	local cultist_units = scratchpad.chanting_units
	local fx_system = scratchpad.fx_system
...
function _setup_progress_bar(unit, breed, blackboard, scratchpad, action_data, t)
	...
	elseif scratchpad.triggered_health_bar then
		...
		if scratchpad._health_timer_threshold <= scratchpad._health_timer then
			scratchpad._health_timer = 0
