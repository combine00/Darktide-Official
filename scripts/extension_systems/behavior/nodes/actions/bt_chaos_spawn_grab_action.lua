...
function BtChaosSpawnGrabAction:enter(unit, breed, blackboard, scratchpad, action_data, t)
...
function BtChaosSpawnGrabAction:init_values(blackboard)
...
function BtChaosSpawnGrabAction:leave(unit, breed, blackboard, scratchpad, action_data, t, reason, destroy)
	...
	local disabled_state_component = scratchpad.target_unit_disabled_character_state_component
	...
	if reason ~= "done" and ALIVE[grabbed_unit] then
		...
		if disabling_unit == unit then
			...
			local disabled_state_input = scratchpad.target_unit_disabled_state_input
	...
	elseif reason == "done" and grabbed_unit and disabled_state_component and unit ~= disabled_state_component.disabling_unit then
		scratchpad.behavior_component.grabbed_unit = nil
	end
...
function BtChaosSpawnGrabAction:run(unit, breed, blackboard, scratchpad, action_data, dt, t)
	...
	if state == "grabbing" then
		...
		if grabbed_unit and disabled_state_component then
			local disabled_by_other_minion = ALIVE[disabled_state_component.disabling_unit] and disabled_state_component.disabling_unit ~= unit

			if disabled_by_other_minion then
				return "done"
			end
		end
...
function BtChaosSpawnGrabAction:_start_grabbing(unit, scratchpad, action_data, t)
	...
	local target_unit_character_state = target_unit_data_extension:read_component("character_state")
	scratchpad.target_unit_disabled_character_state_component = disabled_character_state_component
	scratchpad.target_unit_disabled_state_input = disabled_state_input
	scratchpad.target_unit_character_state = target_unit_character_state
end

function BtChaosSpawnGrabAction:_update_grabbing(unit, scratchpad, action_data, t, dt)
	if scratchpad.wants_to_set_eat_timing then
		local target_unit_disabled_character_state_component = scratchpad.target_unit_disabled_character_state_component
		local _, disabling_unit = PlayerUnitStatus.is_grabbed(target_unit_disabled_character_state_component)
	...
	if scratchpad.grab_timing and scratchpad.grab_timing <= t and scratchpad.perception_component.has_line_of_sight then
		if scratchpad.grab_stop_timing and scratchpad.grab_stop_timing <= t then
			...
			local character_state = scratchpad.target_unit_character_state

			if PlayerUnitStatus.is_disabled(character_state) then
				return
			end
...
function BtChaosSpawnGrabAction:_check_if_want_to_smash(unit, scratchpad, action_data, t)
...
function BtChaosSpawnGrabAction:_start_smashing(unit, scratchpad, action_data, t)
...
function BtChaosSpawnGrabAction:_update_smashing(unit, scratchpad, action_data, blackboard, breed, t)
...
function BtChaosSpawnGrabAction:_start_throwing_target(unit, scratchpad, action_data, t)
...
function BtChaosSpawnGrabAction:_align_throwing(unit, scratchpad, action_data)
...
function BtChaosSpawnGrabAction:_update_throwing(unit, scratchpad, action_data, breed, t)
	...
	if scratchpad.throw_at_t and scratchpad.throw_at_t < t then
		...
		target_unit_disabled_state_input.trigger_animation = "none"
...
function BtChaosSpawnGrabAction:_test_throw_trajectory(unit, scratchpad, action_data, test_direction, to)
...
function BtChaosSpawnGrabAction:_grab_target(unit, target_unit, scratchpad, action_data, t)
	local disabled_state_input = scratchpad.target_unit_disabled_state_input
	local disabled_character_state_component = scratchpad.target_unit_disabled_character_state_component
	...
	disabled_character_state_component.target_drag_position = init_target_drag_position
...
function BtChaosSpawnGrabAction:_calculate_randomized_throw_directions(action_data)
...
function BtChaosSpawnGrabAction:_add_threat_to_other_targets(unit, breed, scratchpad, excluded_target)
...
function BtChaosSpawnGrabAction:_rotate_towards_target_unit(unit, scratchpad, action_data)
	local target_unit = scratchpad.perception_component.target_unit
	local flat_rotation = MinionMovement.rotation_towards_unit_flat(unit, target_unit)
...
function BtChaosSpawnGrabAction:_ray_cast(physics_world, from, to)
