...
function BtChaosMutatorRitualistChantingAction:enter(unit, breed, blackboard, scratchpad, action_data, t)
...
function BtChaosMutatorRitualistChantingAction:leave(unit, breed, blackboard, scratchpad, action_data, t, reason, destroy)
	...
	local wwise_stop_event = "wwise/events/minions/stop_mutator_daemonhost_cult_chant"

	fx_system:trigger_wwise_event(wwise_stop_event, nil, unit)
end

function BtChaosMutatorRitualistChantingAction:run(unit, breed, blackboard, scratchpad, action_data, dt, t)
	...
	if behavior_component.move_state == "chanting" then
		...
		local level_unit_id = target_unit and Unit.alive(target_unit) and Managers.state.unit_spawner:game_object_id(target_unit) or NetworkConstants.invalid_level_unit_id
...
return BtChaosMutatorRitualistChantingAction
