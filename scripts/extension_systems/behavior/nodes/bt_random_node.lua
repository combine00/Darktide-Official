...
function BtRandomNode:init(...)
	BtRandomNode.super.init(self, ...)

	self._children = {}
end

function BtRandomNode:init_values(blackboard, action_data, node_data)
...
function BtRandomNode:ready(lua_node)
	local children = self._children
...
function BtRandomNode:add_child(node)
	self._children[#self._children + 1] = node
end

function BtRandomNode:leave(unit, breed, blackboard, scratchpad, action_data, t, reason, destroy, node_data)
	local node_identifier = self.identifier
	node_data[node_identifier] = true
end

function BtRandomNode:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)
...
function BtRandomNode:run(unit, breed, blackboard, scratchpad, action_data, dt, t, node_data, running_child_nodes)
	local node_identifier = self.identifier
