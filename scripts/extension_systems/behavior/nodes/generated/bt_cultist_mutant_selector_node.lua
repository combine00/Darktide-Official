...
local BtCultistMutantSelectorNode = class("BtCultistMutantSelectorNode", "BtNode")

function BtCultistMutantSelectorNode:init(...)
	BtCultistMutantSelectorNode.super.init(self, ...)

	self._children = {}
end

function BtCultistMutantSelectorNode:init_values(blackboard, action_data, node_data)
	BtCultistMutantSelectorNode.super.init_values(self, blackboard, action_data, node_data)
...
function BtCultistMutantSelectorNode:add_child(node)
	self._children[#self._children + 1] = node
end

function BtCultistMutantSelectorNode:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)
	...
	local node_charge = children[5]
	local is_running = last_leaf_node_running and last_running_node == node_charge
	local condition_result = nil
	...
	if condition_result then
		new_running_child_nodes[node_identifier] = node_charge
...
function BtCultistMutantSelectorNode:run(unit, breed, blackboard, scratchpad, action_data, dt, t, node_data, running_child_nodes)
	local node_identifier = self.identifier
...
return BtCultistMutantSelectorNode
