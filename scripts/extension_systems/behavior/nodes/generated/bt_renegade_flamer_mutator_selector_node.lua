...
local BtRenegadeFlamerMutatorSelectorNode = class("BtRenegadeFlamerMutatorSelectorNode", "BtNode")

function BtRenegadeFlamerMutatorSelectorNode:init(...)
	BtRenegadeFlamerMutatorSelectorNode.super.init(self, ...)

	self._children = {}
end

function BtRenegadeFlamerMutatorSelectorNode:init_values(blackboard, action_data, node_data)
	BtRenegadeFlamerMutatorSelectorNode.super.init_values(self, blackboard, action_data, node_data)
...
function BtRenegadeFlamerMutatorSelectorNode:add_child(node)
	self._children[#self._children + 1] = node
end

function BtRenegadeFlamerMutatorSelectorNode:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)
	...
	local node_aggroed_patrol = children[8]
	local is_running = last_leaf_node_running and last_running_node == node_aggroed_patrol
	local condition_result = nil
	...
	if condition_result then
		new_running_child_nodes[node_identifier] = node_aggroed_patrol

		return node_aggroed_patrol
	end
...
function BtRenegadeFlamerMutatorSelectorNode:run(unit, breed, blackboard, scratchpad, action_data, dt, t, node_data, running_child_nodes)
	local node_identifier = self.identifier
...
return BtRenegadeFlamerMutatorSelectorNode
