...
function BtRenegadeTwinCaptainSelectorNode:init(...)
	BtRenegadeTwinCaptainSelectorNode.super.init(self, ...)

	self._children = {}
end

function BtRenegadeTwinCaptainSelectorNode:init_values(blackboard, action_data, node_data)
...
function BtRenegadeTwinCaptainSelectorNode:add_child(node)
	self._children[#self._children + 1] = node
end

function BtRenegadeTwinCaptainSelectorNode:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)
	...
	if condition_result then
		local leaf_node = node_plasma_pistol_combat:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)

		if leaf_node then
			new_running_child_nodes[node_identifier] = node_plasma_pistol_combat
...
function BtRenegadeTwinCaptainSelectorNode:run(unit, breed, blackboard, scratchpad, action_data, dt, t, node_data, running_child_nodes)
	local node_identifier = self.identifier
