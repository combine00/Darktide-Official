...
local BtChaosArmoredInfectedSelectorNode = class("BtChaosArmoredInfectedSelectorNode", "BtNode")

function BtChaosArmoredInfectedSelectorNode:init(...)
	BtChaosArmoredInfectedSelectorNode.super.init(self, ...)

	self._children = {}
end

function BtChaosArmoredInfectedSelectorNode:init_values(blackboard, action_data, node_data)
	BtChaosArmoredInfectedSelectorNode.super.init_values(self, blackboard, action_data, node_data)
...
function BtChaosArmoredInfectedSelectorNode:add_child(node)
	self._children[#self._children + 1] = node
end

function BtChaosArmoredInfectedSelectorNode:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)
	...
	local node_far_and_close_combat = children[8]
	local tree_node = node_far_and_close_combat.tree_node
	local condition_args = tree_node.condition_args
	local is_running = last_leaf_node_running and last_running_node == node_far_and_close_combat
	local condition_result = nil
	...
	if condition_result then
		local leaf_node = node_far_and_close_combat:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)

		if leaf_node then
			new_running_child_nodes[node_identifier] = node_far_and_close_combat
...
function BtChaosArmoredInfectedSelectorNode:run(unit, breed, blackboard, scratchpad, action_data, dt, t, node_data, running_child_nodes)
	local node_identifier = self.identifier
...
return BtChaosArmoredInfectedSelectorNode
