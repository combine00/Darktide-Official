...
local BtRenegadeGunnerSelectorNode = class("BtRenegadeGunnerSelectorNode", "BtNode")

function BtRenegadeGunnerSelectorNode:init(...)
	BtRenegadeGunnerSelectorNode.super.init(self, ...)

	self._children = {}
end

function BtRenegadeGunnerSelectorNode:init_values(blackboard, action_data, node_data)
	BtRenegadeGunnerSelectorNode.super.init_values(self, blackboard, action_data, node_data)
...
function BtRenegadeGunnerSelectorNode:add_child(node)
	self._children[#self._children + 1] = node
end

function BtRenegadeGunnerSelectorNode:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)
	...
	local node_has_cover = children[7]
	...
	if condition_result then
		local leaf_node = node_has_cover:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)

		if leaf_node then
			new_running_child_nodes[node_identifier] = node_has_cover
...
function BtRenegadeGunnerSelectorNode:run(unit, breed, blackboard, scratchpad, action_data, dt, t, node_data, running_child_nodes)
	local node_identifier = self.identifier
...
return BtRenegadeGunnerSelectorNode
