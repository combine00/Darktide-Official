...
function BtCultistRitualistSelectorNode:init(...)
	BtCultistRitualistSelectorNode.super.init(self, ...)

	self._children = {}
end

function BtCultistRitualistSelectorNode:init_values(blackboard, action_data, node_data)
...
function BtCultistRitualistSelectorNode:add_child(node)
	self._children[#self._children + 1] = node
end

function BtCultistRitualistSelectorNode:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)
...
function BtCultistRitualistSelectorNode:run(unit, breed, blackboard, scratchpad, action_data, dt, t, node_data, running_child_nodes)
	local node_identifier = self.identifier
