...
function BotNavigationExtension:init(extension_init_context, unit, extension_init_data, game_object_data)
...
function BotNavigationExtension:update(unit, t)
	self:_update_destination(unit, t)
end

function BotNavigationExtension:fixed_update(unit, dt, t, ...)
...
function BotNavigationExtension:_update_destination(unit, t)
...
function BotNavigationExtension:_update_astar(t)
	local astar = self._astar
...
function BotNavigationExtension:_path_failed(t)
...
function BotNavigationExtension:_path_successful(t)
...
function BotNavigationExtension:_update_path(unit, t)
...
function BotNavigationExtension:_goal_reached(position, goal, previous_goal, t)
...
function BotNavigationExtension:_reevaluate_current_nav_transition(path_index, t)
...
function BotNavigationExtension:update_position(unit)
...
function BotNavigationExtension:move_to(target_position, callback)
	if self._astar_cancelled then
...
function BotNavigationExtension:teleport(destination)
	local astar = self._astar
...
function BotNavigationExtension:stop()
	local current_position = POSITION_LOOKUP[self._unit]

	self:teleport(current_position)
end

function BotNavigationExtension:current_goal()
	local current_transition = self._current_transition
...
function BotNavigationExtension:is_following_last_goal()
	local current_transition = self._current_transition
...
function BotNavigationExtension:destination_reached()
	return self._final_goal_reached
end

function BotNavigationExtension:path_callback()
	return self._path_callback
end

function BotNavigationExtension:successive_failed_paths()
	return self._num_successive_failed_paths, self._last_successful_path_t
end

function BotNavigationExtension:destination()
	if self._has_queued_target then
...
function BotNavigationExtension:position_when_destination_reached()
	if self._final_goal_reached then
...
function BotNavigationExtension:latest_position_on_nav_mesh()
	return self._is_latest_position_on_nav_mesh_valid and self._latest_position_on_nav_mesh:unbox() or nil
end

function BotNavigationExtension:is_in_transition()
	return self._current_transition ~= nil
end

function BotNavigationExtension:transition_type()
	local transition = self._current_transition

	return transition.type
end

function BotNavigationExtension:transition_unit()
	local transition = self._current_transition
...
function BotNavigationExtension:transition_requires_jump(position)
	local transition = self._current_transition
...
function BotNavigationExtension:traverse_logic()
	return self._traverse_logic
end

function BotNavigationExtension:nav_world()
	return self._nav_world
end

function BotNavigationExtension:allow_nav_tag_layer(layer_name, layer_allowed)
...
function BotNavigationExtension:set_nav_tag_layer_cost(layer_name, layer_cost)
	local layer_id = Managers.state.nav_mesh:nav_tag_layer_id(layer_name)
