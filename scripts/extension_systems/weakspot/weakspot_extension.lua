...
function WeakspotExtension:init(extension_init_context, unit, extension_init_data)
	self._breed = extension_init_data.breed
	self._weakspot_config = self._breed.weakspot_config
	self._unit = unit
end

function WeakspotExtension:weakspot_attacked(attacking_unit, attack_result, damage_dealt, damage_efficiency, hit_zone_name, hit_world_position, attack_direction)
	local config = self._weakspot_config
	local impact_fx = config.impact_fx

	if impact_fx then
		local hit_actor, hit_normal, impact_fx_data, attack_was_stopped, damage_profile, attack_type = nil
