...
function ProximityAreaBuffDrone:_handle_end_of_lifetime_triggers()
...
function ProximityAreaBuffDrone:start_job(is_job)
...
function ProximityAreaBuffDrone:_add_buff_to_unit(t, unit)
	...
	local special_rule_index, special_rule_component_index = nil
...
local function _trigger_hordes_aoe_shock(owner_unit, drone_position, shock_range, t)
	...
	local num_hits = broadphase:query(drone_position, shock_range, BROADPHASE_RESULTS, enemy_side_names)
...
function ProximityAreaBuffDrone:_update_buff_keywords_triggered_effects(t)
	local owner_unit = self._owner_unit_or_nil
	...
	if owner_has_aoe_shock_keyword then
		...
		local trigger_aoe_shock = not self._next_aoe_shock_t or self._next_aoe_shock_t <= t
