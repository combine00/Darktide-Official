...
function ActionWeaponShout:init(action_context, action_params, action_settings)
...
function ActionWeaponShout:start(action_settings, t, time_scale, action_start_params)
...
function ActionWeaponShout:fixed_update(dt, t, time_in_action)
	local action_settings = self._action_settings
	local block_duration = action_settings.block_duration

	if self._block_component.is_blocking and (not block_duration or action_settings.block_duration <= time_in_action) then
...
function ActionWeaponShout:finish(reason, data, t, time_in_action, action_settings)
...
function ActionWeaponShout:_start_shout(t)
	local action_settings = self._action_settings
...
function ActionWeaponShout:_update_shout(dt)
...
function ActionWeaponShout:_collect_targets(t, action_settings, side, attack_direction)
	...
	if shout_shape and shout_shape == "cone" then
		...
		repeat
			...
			local num_hits = broadphase:query(broadphase_pos, radius_to_use, broadphase_results, enemy_side_names)
			...
			for ii = 1, num_hits do
				...
				local within_angle = math.acos(dot) <= shout_dot
