...
local _shotshell_template, _damage_template_from_impact_config, _line_effect = nil
...
function ActionShootPellets:_shoot(position, rotation, power_level, charge_level, t, fire_config)
	...
	if process_hits then
		...
		for hit_unit, number_of_hits in pairs(self._num_hits_per_unit) do
			if number_of_hits == num_pellets_total then
				hit_all_pellets_on_same = true
			end
		end
...
function ActionShootPellets:_save_pellet_hits(shotshell_template, hit_results, position, direction, max_distance, charge_level)
...
function ActionShootPellets:_process_hits(power_level, t, fire_config)
...
function ActionShootPellets:_add_shotshell_buff(t, buff_name, shotshell_template, hit_unit, player_unit, damage_config, weapon_item, number_of_hits)
...
function ActionShootPellets:_count_number_of_pellets_fired_this_frame()
...
function _damage_template_from_impact_config(impact_config, percentage_of_pellets_hit)
	...
	for i = 1, #impact_config do
		...
		if damage_config and damage_config.pellets_threshold <= percentage_of_pellets_hit then
