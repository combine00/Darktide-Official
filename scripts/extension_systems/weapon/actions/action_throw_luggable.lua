...
function ActionThrowLuggable:init(action_context, ...)
	ActionThrowLuggable.super.init(self, action_context, ...)

	local unit_data_extension = action_context.unit_data_extension
	self._action_aim_projectile_component = unit_data_extension:read_component("action_aim_projectile")
	self._action_throw_luggable_component = unit_data_extension:write_component("action_throw_luggable")
end

function ActionThrowLuggable:start(action_settings, t, time_scale, action_start_params)
	ActionThrowLuggable.super.start(self, action_settings, t, time_scale, action_start_params)

	local action_throw_luggable_component = self._action_throw_luggable_component
	action_throw_luggable_component.thrown = false
	local used_input = action_start_params.used_input
	local inventory_component = self._inventory_component
	local wielded_slot = inventory_component.wielded_slot
	local slot_name_or_nil = PlayerUnitVisualLoadout.slot_name_from_wield_input(used_input, inventory_component, self._visual_loadout_extension, self._weapon_extension, self._ability_extension, self._input_extension)
	...
	action_throw_luggable_component.slot_to_wield = slot_to_wield
end

function ActionThrowLuggable:finish(...)
	ActionThrowLuggable.super.finish(self, ...)

	local action_throw_luggable_component = self._action_throw_luggable_component
	action_throw_luggable_component.thrown = false
	action_throw_luggable_component.slot_to_wield = "none"
end

function ActionThrowLuggable:fixed_update(dt, t, time_in_action)
	local action_throw_luggable_component = self._action_throw_luggable_component
	...
	if not action_throw_luggable_component.thrown and time_scaled_throw_time <= time_in_action then
		action_throw_luggable_component.thrown = true
	...
	if time_scaled_total_time <= time_in_action then
		...
		local slot_to_wield = action_throw_luggable_component.slot_to_wield
...
function ActionThrowLuggable:_throw_unit(existing_unit, throw_type)
