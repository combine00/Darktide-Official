...
function ActionChainLightning:init(action_context, action_params, action_settings)
	...
	if self._is_server then
		...
		function self._jump_on_add_func(node, func_context)
...
function ActionChainLightning:start(action_settings, t, time_scale, action_start_params)
...
function ActionChainLightning:_create_chain_root_node(action_settings)
...
function ActionChainLightning:_clear_initial_targets()
...
function ActionChainLightning:fixed_update(dt, t, time_in_action, frame)
...
function ActionChainLightning:finish(reason, data, t, time_in_action, action_settings, next_action_params)
...
function ActionChainLightning:_play_shoot_sound()
	local action_settings = self._action_settings
	local action_module_charge_component = self._action_module_charge_component
...
function ActionChainLightning:_play_jump_sound()
	local action_settings = self._action_settings
	local action_module_charge_component = self._action_module_charge_component
	local fx_extension = self._fx_extension
	local fx_settings = action_settings.fx
...
function ActionChainLightning:_start_looping_shoot_sound()
	local is_critical_strike = self._critical_strike_component.is_active
...
function ActionChainLightning:_handle_modules_and_components_start(t, action_settings)
	local overload_module = self._overload_module
	local action_module_charge_component = self._action_module_charge_component
	local is_critical_strike = self._critical_strike_component.is_active
...
function ActionChainLightning:_handle_modules_and_components_finish(reason, data, t)
	local overload_module = self._overload_module
...
function ActionChainLightning:_handle_buffs_start()
	local action_settings = self._action_settings
...
function ActionChainLightning:_handle_buffs_finish()
	local action_settings = self._action_settings
...
function ActionChainLightning:_find_root_targets(t, time_in_action)
	local action_settings = self._action_settings
...
function ActionChainLightning:_find_new_chain_targets(t, broadphase, enemy_side_names, max_angle, close_max_angle, vertical_max_angle, max_z_diff, max_jumps, radius, source_node, initial_travel_direction)
	local temp_targets = self._temp_targets
	local hit_units = self._hit_units
...
function ActionChainLightning:_deal_damage(t, time_in_action, source_item)
	local action_settings = self._action_settings
