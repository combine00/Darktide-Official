...
function ActionZealotChannel:init(action_context, action_params, action_settings)
...
function ActionZealotChannel:start(action_settings, t, time_scale, action_start_params)
...
function ActionZealotChannel:fixed_update(dt, t, time_in_action)
	if not self._is_server then
...
function ActionZealotChannel:_on_channel_tick(dt, in_coherence_units, t, time_in_action)
	for in_coherence_unit, _ in pairs(in_coherence_units) do
...
function ActionZealotChannel:_zealot_stagger(radius, power_level)
	local action_settings = self._action_settings
	local player_unit = self._player_unit
...
function ActionZealotChannel:finish(reason, data, t, time_in_action, action_settings)
