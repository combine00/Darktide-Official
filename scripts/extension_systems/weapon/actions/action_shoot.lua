...
function ActionShoot:_update_delta_charge(fire_config, dt)
...
function ActionShoot:fixed_update(dt, t, time_in_action, frame)
	local action_component = self._action_component
...
function ActionShoot:_prepare_fire_config(fire_config, weapon_extension)
	...
	local override_charge_level = nil
...
function ActionShoot:_spend_ammunition(dt, t, charge_level, fire_config)
	local action_settings = self._action_settings
	local inventory_slot_component = self._inventory_slot_component
...
function ActionShoot:_shoot(position, rotation, power_level, charge_level, t)
	return
end
...
function ActionShoot:server_correction_occurred()
...
function ActionShoot:_play_shoot_sound(fire_config)
	local action_settings = self._action_settings
	local muzzle_fx_source_name = self:_muzzle_fx_source()
...
function ActionShoot:_update_looping_shoot_sound(fire_config)
...
function ActionShoot:_play_muzzle_flash_vfx(fire_config, shoot_rotation, charge_level)
	...
	local scale, all_clients, create_network_index = nil
...
function ActionShoot:_play_muzzle_smoke(fire_config)
	local fx = self._action_settings.fx
...
function ActionShoot:_play_line_fx(line_effect, position, end_position, optional_attachment_id)
...
function ActionShoot:_fire_rate_settings()
	local weapon_handling_template = self._weapon_extension:weapon_handling_template() or EMPTY_TABLE
...
function ActionShoot:_scale_auto_fire_time_with_buffs(auto_fire_time)
