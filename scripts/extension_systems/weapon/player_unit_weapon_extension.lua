...
function PlayerUnitWeaponExtension:explosion_template_lerp_values(explosion_template_name_or_nil, override_action_name_or_nil)
	local weapon = self:_wielded_weapon(self._inventory_component, self._weapons)
...
function PlayerUnitWeaponExtension:on_slot_wielded(slot_name, t, skip_wield_action)
	local weapon = self._weapons[slot_name]
...
function PlayerUnitWeaponExtension:blocked_attack(attacking_unit, hit_world_position, block_broken, weapon_template, attack_type, block_cost, is_perfect_block)
	local weapon = self:_wielded_weapon(self._inventory_component, self._weapons)
...
function PlayerUnitWeaponExtension:action_settings_from_action_input(action_input)
	local inventory_component = self._inventory_component
	local wielded_slot = inventory_component.wielded_slot
...
function PlayerUnitWeaponExtension:condition_func_params(wielded_slot)
...
function PlayerUnitWeaponExtension:update_weapon_special_implementation(fixed_frame)
...
function PlayerUnitWeaponExtension:set_wielded_weapon_weapon_special_active(t, want_active, optional_reason)
	local inventory_component = self._inventory_component
	local buff_extension = self._buff_extension
	local wielded_slot = inventory_component.wielded_slot
	...
	elseif not want_active and was_special_active then
		...
		if not did_deactivate then
			if tweak_data_or_nil and tweak_data_or_nil.manual_toggle_only and optional_reason ~= "manual_toggle" then
				-- Nothing
...
function PlayerUnitWeaponExtension:_remove_buffs(buffs, buff_target, slot_name, inventory_slot_component)
	local buff_list = buffs[buff_target]
	local num_buffs = #buff_list
...
function PlayerUnitWeaponExtension:_apply_special_rules(slot_name, special_rules)
...
function PlayerUnitWeaponExtension:_remove_special_rules(slot_name)
	local active_special_rules = self._active_special_rules
...
function PlayerUnitWeaponExtension:has_special_rule(slot_name, special_rule_name)
	local active_special_rules = self._active_special_rules
...
function PlayerUnitWeaponExtension:weapon_shout_template()
...
function PlayerUnitWeaponExtension:_init_persistent_data(slot_name, weapon_template)
...
function PlayerUnitWeaponExtension:_clear_persistent_data(slot_name)
	local persistent_data = self._persistent_data_by_slot[slot_name]

	table.clear(persistent_data)
end

function PlayerUnitWeaponExtension:set_persistent_data(slot_name, key, value)
	local persistent_data = self._persistent_data_by_slot[slot_name]
...
function PlayerUnitWeaponExtension:get_persistent_data(slot_name, key)
	local persistent_data = self._persistent_data_by_slot[slot_name]
