...
function WeaponSpecialFakeSeparateAmmoPool:init(context, init_data)
...
function WeaponSpecialFakeSeparateAmmoPool:on_wieldable_slot_equipped()
...
function WeaponSpecialFakeSeparateAmmoPool:fixed_update(dt, t)
	return
end

function WeaponSpecialFakeSeparateAmmoPool:process_hit(t, weapon, action_settings, num_hit_enemies, target_is_alive, target_unit, damage, result, damage_efficiency, stagger_result, hit_position, attack_direction, abort_attack, optional_origin_slot)
	return
end

function WeaponSpecialFakeSeparateAmmoPool:blocked_attack(attacking_unit, block_cost, block_broken, is_perfect_block)
	return
end

function WeaponSpecialFakeSeparateAmmoPool:on_special_activation(t, num_hit_enemies)
	self._inventory_slot_component.num_special_charges = math.max(self._inventory_slot_component.num_special_charges - 1, 0)
end

function WeaponSpecialFakeSeparateAmmoPool:on_special_deactivation(t)
	return
end

function WeaponSpecialFakeSeparateAmmoPool:on_sweep_action_start(t)
	return
end

function WeaponSpecialFakeSeparateAmmoPool:on_sweep_action_finish(t, num_hit_enemies)
	return
end

function WeaponSpecialFakeSeparateAmmoPool:on_exit_damage_window(t, num_hit_enemies)
	return
end
