...
function WeaponSpecialCharging:init(context, init_data)
...
function WeaponSpecialCharging:on_wieldable_slot_equipped()
	return
end

function WeaponSpecialCharging:fixed_update(dt, t)
...
function WeaponSpecialCharging:on_special_activation(t)
	self._weapon_tweak_templates_component.charge_template_name = self._weapon_template.special_charge_template
end

function WeaponSpecialCharging:on_special_deactivation(t)
...
function WeaponSpecialCharging:on_sweep_action_start(t)
...
function WeaponSpecialCharging:on_sweep_action_finish(t, num_hit_enemies)
	return
end

function WeaponSpecialCharging:process_hit(t, weapon, action_settings, num_hit_enemies, target_is_alive, target_unit, hit_position, attack_direction, abort_attack, optional_origin_slot)
	return
end

function WeaponSpecialCharging:blocked_attack(attacking_unit, block_cost, block_broken, is_perfect_block)
	return
end

function WeaponSpecialCharging:on_exit_damage_window(t, num_hit_enemies, aborted)
	return
end
