...
function PlayerUnitLedgeFinderExtension:has_significant_obstacle_in_front()
...
function PlayerUnitLedgeFinderExtension:calculate_ledges(fixed_frame, player_position, base_position_offset, position_1p, rotation_1p)
	self._fixed_frame = fixed_frame
...
function PlayerUnitLedgeFinderExtension:_try_find_ledges(ledges, base_position, player_position, forward, right)
	...
	for eval_ray_index = 1, NUM_RAYS do
		...
		if left_hit or right_hit then
			...
			for comp_ray_index = eval_ray_index + 1, NUM_RAYS do
				...
				if player_width >= comp_distance - eval_distance then
					break
				end
			...
			if possible_ledge then
				...
				local eval_left_pos, eval_right_pos = nil
...
function PlayerUnitLedgeFinderExtension:_pair_potential_ledge_points(ledges, potential_left_ledge_points, potential_right_ledge_points, ledge_point_unit_map)
	...
	for left_i = 1, num_left_ledge_points do
		...
		local best_right_i, best_find_point = nil
...
function PlayerUnitLedgeFinderExtension:_finalize_ledges(ledges, player_position, peeking_potential, height)
