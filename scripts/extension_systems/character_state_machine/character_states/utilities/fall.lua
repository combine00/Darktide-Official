...
function Fall.set_fall_height(locomotion_component, inair_state_component)
	local player_position = locomotion_component.position
	inair_state_component.fell_from_height = player_position.z
end

function Fall.fall_distance(locomotion_component, inair_state_component)
	local player_position = locomotion_component.position

	return inair_state_component.fell_from_height - player_position.z
end

function Fall.can_damage(unit, player_character_constants, locomotion_component, in_air_character_state_component)
...
function Fall.check_damage(unit, player_character_constants, locomotion_component, in_air_character_state_component)
...
function Fall.trigger_impact_sound(unit, fx_extension, player_character_constants, locomotion_component, in_air_character_state_component)
	if not Fall.can_damage(unit, player_character_constants, locomotion_component, in_air_character_state_component) then
		return
	end
