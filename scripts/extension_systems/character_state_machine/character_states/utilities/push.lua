...
function Push.add(unit, locomotion_push_component, push_direction, push_template, attack_type, is_predicted)
	...
	if push_template.abort_stickyness then
		...
		if action_sweep_component.is_sticky then
			...
			Interrupt.action(t, unit, "push", nil, ignore_immunity)
		end
	end
end
