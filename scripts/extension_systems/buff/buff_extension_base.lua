...
function BuffExtensionBase:init(extension_init_context, unit, extension_init_data, game_object_data_or_game_session, nil_or_game_object_id, pre_allocate_event_param_tables)
...
function BuffExtensionBase:_update_stat_buffs_and_keywords(t, on_remove_buff)
...
function BuffExtensionBase:buff_start_time(buff_name)
	local buff_instance = self._stacking_buffs[buff_name]
...
function BuffExtensionBase:_remove_buff(index, extension_destroyed)
	local buffs_by_index = self._buffs_by_index
...
function BuffExtensionBase:_on_remove_buff_stack(buff_instance, previous_stack_count)
	local template = buff_instance:template()
...
function BuffExtensionBase:_on_add_buff_stack(buff_instance, previous_stack_count)
	local template = buff_instance:template()
	local shared_effects = template.effects
...
function BuffExtensionBase:has_buff_using_buff_template(buff_template_name)
	local buffs = self._buffs
...
function BuffExtensionBase:has_buff_using_buff_template_with_owner(buff_template_name, owner_unit)
	local buffs = self._buffs
...
function BuffExtensionBase:has_buff_using_buff_templates(...)
	local buffs = self._buffs
...
function BuffExtensionBase:has_active_buff_with_buff_template(buff_template_name)
...
function BuffExtensionBase:owner_of_buff_with_id(buff_id)
	local buffs = self._buffs
...
function BuffExtensionBase:has_any_keyword(keywords)
...
function BuffExtensionBase:had_keyword(keyword, interval)
...
function BuffExtensionBase:portable_random()
...
function BuffExtensionBase:_start_node_effects(node_effects)
...
function BuffExtensionBase:_check_stack_node_effects(stack_node_effects, current_stack_count, previous_stack_count)
...
function BuffExtensionBase:rpc_add_buff(channel_id, game_object_id, buff_template_id, server_index, optional_lerp_value, optional_item_slot_id, optional_parent_buff_template_id, from_talent)
...
function BuffExtensionBase:rpc_add_buff_with_stacks(channel_id, game_object_id, buff_template_id, server_index_array, optional_lerp_value, optional_item_slot_id, optional_parent_buff_template_id)
	local template_name = NetworkLookup.buff_templates[buff_template_id]
	local template = BuffTemplates[template_name]
	local t = FixedFrame.get_latest_fixed_time()
	local optional_item_slot_name = optional_item_slot_id and NetworkLookup.player_inventory_slot_names[optional_item_slot_id]
...
function BuffExtensionBase:rpc_buff_proc_set_active_time(channel_id, game_object_id, server_index, activation_frame)
	local index = self._buff_index_map[server_index]
...
function BuffExtensionBase:rpc_buff_set_start_time(channel_id, game_object_id, server_index, activation_frame)
	local index = self._buff_index_map[server_index]
...
function BuffExtensionBase:_set_start_time_from_rpc(index, start_time)
