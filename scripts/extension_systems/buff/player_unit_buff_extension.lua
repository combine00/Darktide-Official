...
function PlayerUnitBuffExtension:_init_sync_data(game_object_data)
...
function PlayerUnitBuffExtension:hot_join_sync(unit, sender, channel_id)
	return
end

function PlayerUnitBuffExtension:game_object_initialized(game_session, game_object_id)
...
function PlayerUnitBuffExtension:extensions_ready(world, unit)
...
function PlayerUnitBuffExtension:post_update(unit, dt, t)
...
function PlayerUnitBuffExtension:mark_buff_finished(local_index)
	local buff_instance = self._buffs_by_index[local_index]
...
function PlayerUnitBuffExtension:_on_add_buff_stack(existing_buff_instance, previous_stack_count)
...
function PlayerUnitBuffExtension:_add_rpc_synced_buff(template, t, ...)
	local index = self:_add_buff(template, t, ...)
	local game_object_id = self._game_object_id
...
function PlayerUnitBuffExtension:_remove_rpc_synced_buff(index)
	local game_object_id = self._game_object_id
...
function PlayerUnitBuffExtension:_set_proc_active_start_time(index, activation_time)
	if self._is_server then
...
function PlayerUnitBuffExtension:_set_start_time_from_rpc(index, start_time)
	if self._is_server then
		-- Nothing
	end
...
function PlayerUnitBuffExtension:start_on_screen_effect(index, on_screen_effect, stop_type, world)
...
function PlayerUnitBuffExtension:_stop_on_screen_effects(index, world)
	local on_screen_effects = self._on_screen_effects[index]
...
function PlayerUnitBuffExtension:_is_cinematic_active()
...
function PlayerUnitBuffExtension:_update_on_screen_fx()
	local buff_context = self._buff_context
	local is_local_unit = buff_context.is_local_unit
	local player = self._player
	local is_human_controlled = player:is_human_controlled()
