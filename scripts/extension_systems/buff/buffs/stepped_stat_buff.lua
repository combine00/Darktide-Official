...
function SteppedStatBuff:update_stat_buffs(current_stat_buffs, t)
...
function SteppedStatBuff:visual_stack_count()
	local steps = self:stat_buff_stacking_count()

	return steps
end

function SteppedStatBuff:_get_step_stat_buffs()
	local template = self._template
