...
function ParentProcBuff:init(context, template, start_time, instance_id, ...)
...
function ParentProcBuff:destroy()
...
function ParentProcBuff:update(dt, t, portable_random)
...
function ParentProcBuff:update_proc_events(t, proc_events, num_proc_events, portable_random, local_portable_random)
...
function ParentProcBuff:_add_child_buff_stack(t, num_children_to_add)
	local buff_extension = self._buff_extension
...
function ParentProcBuff:_remove_child_buff_stack(t, num_children_to_remove)
	local buff_extension = self._buff_extension
	local child_buff_indicies = self._child_buff_indicies
	local num_child_stacks = self._num_child_stacks
...
function ParentProcBuff:has_hud()
	return ParentProcBuff.super.super.has_hud(self)
end

function ParentProcBuff:visual_stack_count()
	local template_context = self._template_context
...
function ParentProcBuff:_show_in_hud()
	local template = self._template
...
function ParentProcBuff:_hud_show_stack_count()
	return true
end

function ParentProcBuff:duration_progress()
	local character_state_component = self._character_state_component
	local is_knocked_down = PlayerUnitStatus.is_knocked_down(character_state_component)
