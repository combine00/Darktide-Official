...
function WeaponTraitParentProcBuff:_remove_child_buff_stack(t, num_children_to_remove)
	local buff_extension = self._buff_extension
	local child_buff_indicies = self._child_buff_indicies
	local num_child_stacks = self._num_child_stacks
...
function WeaponTraitParentProcBuff:duration_progress()
	local template = self._template
	local template_overrides = self._template_override_data
	local child_duration = template_overrides.child_duration or template.child_duration
...
function WeaponTraitParentProcBuff:has_hud()
	return WeaponTraitParentProcBuff.super.super.has_hud(self)
end

function WeaponTraitParentProcBuff:visual_stack_count()
	local template_context = self._template_context
...
function WeaponTraitParentProcBuff:_show_in_hud()
	local template = self._template
	local template_context = self._template_context
...
function WeaponTraitParentProcBuff:_hud_show_stack_count()
	local template = self._template
