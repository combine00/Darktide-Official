...
function BotUnitInput:init(physics_world, player)
...
function BotUnitInput:extensions_ready(world, unit)
...
function BotUnitInput:fixed_update(unit, dt, t, frame)
...
function BotUnitInput:update(unit, dt, t)
	local input = self._input
...
function BotUnitInput:_update_actions(input)
...
function BotUnitInput:_store_ephemeral_input(input)
	local ephemeral_input = self._ephemeral_input
	local ephemeral_actions = InputHandlerSettings.ephemeral_actions
	local num_ephemeral_actions = #ephemeral_actions
...
function BotUnitInput:set_aim_position(position)
	self._aim_position:store(position)
end

function BotUnitInput:set_aim_rotation(rotation)
	self._aim_rotation:store(rotation)
end

function BotUnitInput:set_aiming(aiming, soft, use_rotation)
	self._aiming = aiming
	self._aim_with_rotation = use_rotation and aiming or false
	self._soft_aiming = soft and aiming or false
end

function BotUnitInput:set_look_at_player_unit(player_unit, rotation_allowed)
...
function BotUnitInput:interact(reset)
	self._interact = true
...
function BotUnitInput:_update_movement(unit, input, dt, t)
...
function BotUnitInput:_calculate_look_at_player_rotation(unit, camera_position, up)
...
function BotUnitInput:_obstacle_check(position, current_speed_sq, goal_vector, goal_direction, up)
...
function BotUnitInput:get_orientation()
	local player = self._player
...
function BotUnitInput:get(action)
	if action == "move" then
