...
function HazardPropSystem:init(context, system_init_data, ...)
...
function HazardPropSystem:_fetch_settings(mission, circumstance_name)
...
function HazardPropSystem:destroy()
	if not self._is_server then
		self._network_event_delegate:unregister_events(unpack(CLIENT_RPCS))
	end
end

function HazardPropSystem:hot_join_sync(sender, channel)
...
function HazardPropSystem:rpc_hazard_prop_set_state(channel_id, level_unit_id, state_id)
...
function HazardPropSystem:rpc_hazard_prop_set_content(channel_id, level_unit_id, content_id)
	local unit = Managers.state.unit_spawner:unit(level_unit_id, true)
	local extension = self._unit_to_extension_map[unit]
...
function HazardPropSystem:rpc_hazard_prop_hot_join(channel_id, level_unit_id, state_id, content_id)
	local unit = Managers.state.unit_spawner:unit(level_unit_id, true)
	local extension = self._unit_to_extension_map[unit]
	local state = NetworkLookup.hazard_prop_states[state_id]
	local content = NetworkLookup.hazard_prop_content[content_id]
...
function HazardPropSystem:extensions_ready(world, unit)
...
function HazardPropSystem:on_gameplay_post_init(level)
	self:_populate_hazard_props()
end

function HazardPropSystem:_shuffle(source)
	local seed = self._seed
	self._seed = table.shuffle(source, seed)
end

function HazardPropSystem:re_populate_hazard_props()
	if not self._is_server then
		return
	end
...
function HazardPropSystem:_populate_hazard_props()
	if not self._is_server then
		return
	end
...
function HazardPropSystem:_sort_remainders(input)
	local sorted = {}
