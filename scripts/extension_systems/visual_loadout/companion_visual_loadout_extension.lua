...
function CompanionVisualLoadoutExtension:extensions_ready(world, unit)
...
function CompanionVisualLoadoutExtension:_update_soft_oob(unit)
	for slot_name, slot_data in pairs(self._slots) do
		if unit == slot_data.unit then
			if self._unit_is_local then
				Log.info("CompanionVisualLoadoutExtension", "%s's %s is out-of-bounds, despawning (%s).", self._breed_name, unit, tostring(Unit.world_position(unit, 1)))
			...
			elseif self._is_server then
				...
				if dropped_actor then
					Log.info("CompanionVisualLoadoutExtension", "%s's %s is out-of-bounds, destroying actor (%s).", self._breed_name, unit, tostring(Unit.world_position(unit, 1)))
...
function CompanionVisualLoadoutExtension:set_unit_local()
...
function CompanionVisualLoadoutExtension:_drop_slot(slot_name)
	local slots = self._slots
	local slot_data = slots[slot_name]
	local slot_state = slot_data.state
...
function CompanionVisualLoadoutExtension:drop_slot(slot_name)
...
function CompanionVisualLoadoutExtension:_send_on_death_event()
	for slot_name, slot_data in pairs(self._slots) do
		if slot_data.state ~= "unequipped" then
			local item_unit = slot_data.unit
...
function CompanionVisualLoadoutExtension:_set_slot_visibility(slot_name, visibility)
	local slots = self._slots
	local slot_data = slots[slot_name]
...
function CompanionVisualLoadoutExtension:set_slot_visibility(slot_name, visibility)
...
function CompanionVisualLoadoutExtension:unit_3p_from_slot(slot_name)
...
function CompanionVisualLoadoutExtension:is_slot_visible(slot_name)
	local slots = self._slots
	local slot_data = slots[slot_name]
...
function CompanionVisualLoadoutExtension:is_inventory_slot_ranged(slot_name)
...
function CompanionVisualLoadoutExtension:can_drop_slot(slot_name)
	local slots = self._slots
	local slot_data = slots[slot_name]
...
function CompanionVisualLoadoutExtension:hot_join_sync(unit, sender, channel)
	local game_object_id = self._game_object_id
	local slots = self._slots
...
function CompanionVisualLoadoutExtension:rpc_minion_wield_slot(channel_id, go_id, slot_id)
	local slot_name = NetworkLookup.minion_inventory_slot_names[slot_id]

	self:_wield_slot(slot_name)
end

function CompanionVisualLoadoutExtension:rpc_minion_unwield_slot(channel_id, go_id, slot_id)
	local slot_name = NetworkLookup.minion_inventory_slot_names[slot_id]

	self:_unwield_slot(slot_name)
end

function CompanionVisualLoadoutExtension:rpc_minion_drop_slot(channel_id, go_id, slot_id)
	local slot_name = NetworkLookup.minion_inventory_slot_names[slot_id]

	self:_drop_slot(slot_name)
end

function CompanionVisualLoadoutExtension:rpc_minion_unequip_slot(channel_id, go_id, slot_id)
	local slot_name = NetworkLookup.minion_inventory_slot_names[slot_id]

	self:_unequip_slot(slot_name)
end

function CompanionVisualLoadoutExtension:rpc_minion_set_slot_visibility(channel_id, go_id, slot_id, visibility)
	local slot_name = NetworkLookup.minion_inventory_slot_names[slot_id]
