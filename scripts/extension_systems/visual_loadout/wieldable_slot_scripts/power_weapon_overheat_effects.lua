...
function PowerWeaponOverheatEffects:init(context, slot, weapon_template, fx_sources, item, unit_1p, unit_3p)
...
function PowerWeaponOverheatEffects:fixed_update(unit, dt, t, frame)
	return
end

function PowerWeaponOverheatEffects:update(unit, dt, t)
	...
	if wants_fx then
		...
		if overheat_state == "idle" and current_overheat ~= overheat_percentage then
			local wanted_stage = nil

			if thresholds.critical <= overheat_percentage then
				wanted_stage = "critical"
			elseif thresholds.high <= overheat_percentage then
				wanted_stage = "high"
			else
				wanted_stage = "low"
			end
...
function PowerWeaponOverheatEffects:_update_stage_loop(unit, dt, t, current_stage, wanted_stage)
...
function PowerWeaponOverheatEffects:update_first_person_mode(first_person_mode)
	return
end

function PowerWeaponOverheatEffects:destroy()
	self:_stop_stage_loop(true)
end

function PowerWeaponOverheatEffects:wield()
	return
end

function PowerWeaponOverheatEffects:unwield()
	self:_stop_stage_loop(true)
end

function PowerWeaponOverheatEffects:_stop_stage_loop(force_stop)
