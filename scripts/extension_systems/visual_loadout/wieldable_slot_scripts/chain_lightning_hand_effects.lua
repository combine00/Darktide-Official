...
function ChainLightningHandEffects:destroy()
	self:_reset()
end

function ChainLightningHandEffects:wield()
	self:_reset()
end

function ChainLightningHandEffects:unwield()
	self:_reset()
end

function ChainLightningHandEffects:update_unit_position(unit, dt, t)
...
function ChainLightningHandEffects:_update_vfx(t)
...
function ChainLightningHandEffects:update_first_person_mode(first_person_mode)
	self._is_in_first_person = first_person_mode
end

function ChainLightningHandEffects:_update_no_target_vfx(t, world, current_particle_id, has_any_particle_effect, node_unit, node)
...
function ChainLightningHandEffects:_update_hand_vfx(t, world, current_particle_id, node_unit, node)
	if current_particle_id then
		return current_particle_id
	else
...
function ChainLightningHandEffects:_stop_vfx(world, particle_id)
	if particle_id then
		World.destroy_particles(world, particle_id)
	end
end

function ChainLightningHandEffects:_reset()
	local world = self._world
	self._right_hand_particle_id = self:_stop_vfx(world, self._right_hand_particle_id)
	self._left_hand_particle_id = self:_stop_vfx(world, self._left_hand_particle_id)
	self._no_target_particle_right_id = self:_stop_vfx(world, self._no_target_particle_right_id)
