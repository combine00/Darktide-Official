...
local ChainLightningAbilityLinkEffects = class("ChainLightningAbilityLinkEffects")
...
local HAND_SHOOT_VFX = "content/fx/particles/abilities/chainlightning/protectorate_chainlightning_attack_hands_fp"
...
function FxData:spawn_vfx(world, link_effect_name, source_unit, source_node, target_unit, target_node)
...
function FxData:spawn_vfx_world_position(world, link_effect_name, impact_effect_name, source_unit, source_node, target_pos, target_normal)
	local num_effects = self._num_effects + 1
	local entry = self._effects[num_effects]
	self._num_effects = num_effects
	local source_pos = Unit_world_position(source_unit, source_node)
...
function FxData:update_target_pos(target_pos, target_normal)
...
function FxData:stop_vfx(world)
	local effects = self._effects
	...
	for ii = 1, self._num_effects do
		...
		local impact_effect_id = entry.impact_effect_id

		if impact_effect_id then
			World_stop_spawning_particles(world, impact_effect_id)
		end
...
function FxData:update_vfx(t, world)
	local effects = self._effects
	...
	for ii = 1, self._num_effects do
		...
		if impact_effect_id then
			local target_normal = entry.target_normal:unbox()
			local impact_rotation = target_normal and Quaternion_look(target_normal) or rotation

			World_move_particles(world, impact_effect_id, target_pos, impact_rotation)
		end
	end
end

function FxData:clear()
	self.active = false
	local effects = self._effects
...
local FxDataTables = class("FxDataTablesChainLightningAbility")

function FxDataTables:init()
...
function FxDataTables:next_table()
...
function FxDataTables:return_table(fx_data_table)
	fx_data_table:clear()
end

function ChainLightningAbilityLinkEffects:init(context, slot, weapon_template, fx_sources)
	...
	self._func_context = {
		...
		use_hands_fx = weapon_chain_settings.use_hands_fx,
...
function ChainLightningAbilityLinkEffects:_create_chain_root_node()
...
function ChainLightningAbilityLinkEffects:_clear_initial_targets()
...
function ChainLightningAbilityLinkEffects:destroy()
	self:_reset()
end

function ChainLightningAbilityLinkEffects:wield()
	self:_reset()
end

function ChainLightningAbilityLinkEffects:unwield()
	self:_reset()
end

function ChainLightningAbilityLinkEffects:update_unit_position(unit, dt, t)
...
function ChainLightningAbilityLinkEffects:_find_root_targets(t)
...
function ChainLightningAbilityLinkEffects:_find_no_target(t)
	...
	if chain_root_node:num_children() <= 0 then
		...
		if not chain_root_node:value("fx_data") then
			...
			local link_effect_name, impact_effect_name = _link_effect_name(context, "no_target")
			...
			local use_hands_fx = context.use_hands_fx

			if spawn_left then
				local parent_unit, source_node_index = context.fx_extension:vfx_spawner_unit_and_node(context.left_fx_source_name)

				fx_data_table:spawn_vfx_world_position(context.world, link_effect_name, impact_effect_name, parent_unit, source_node_index, target_pos, target_normal)

				if use_hands_fx then
					fx_data_table:spawn_vfx_world_position(context.world, HAND_SHOOT_VFX, nil, parent_unit, source_node_index, target_pos, target_normal)
				end
			end
...
function ChainLightningAbilityLinkEffects:_no_target_raycast(position, rotation, max_length)
...
function ChainLightningAbilityLinkEffects:_find_no_target_pos(t)
...
function ChainLightningAbilityLinkEffects:_clear_no_target()
...
function ChainLightningAbilityLinkEffects:_validate_targets(t)
	local chain_root_node = self._chain_root_node
...
function ChainLightningAbilityLinkEffects:_find_new_targets(t)
...
function ChainLightningAbilityLinkEffects:_find_new_chain_targets(t, broadphase, enemy_side_names, max_angle, close_max_angle, vertical_max_angle, max_z_diff, max_jumps, radius, root_target, initial_travel_direction)
	local func_context = self._func_context
	local hit_units = self._hit_units
	local temp_targets = self._temp_targets
...
function ChainLightningAbilityLinkEffects:_update_fx(t)
	local temp_targets = self._temp_targets
...
function ChainLightningAbilityLinkEffects:update_first_person_mode(first_person_mode)
	return
end

function ChainLightningAbilityLinkEffects:_reset()
	self._next_jump_time = 0
	self._next_no_target_jump_time = 0
...
function _root_on_add_func(node, context)
	...
	if not node:value("fx_data") then
		...
		if spawn_left then
			...
			if use_hands_fx then
				fx_data_table:spawn_vfx(context.world, HAND_SHOOT_VFX, parent_unit, source_node_index, child_unit, target_node_index)
			end
		end
...
function _link_effect_name(context, power_override)
	...
	return resolved_link_effect and link_effect_name, resolved_impact_effect and impact_effect_name
end

return ChainLightningAbilityLinkEffects
