...
local _set_intensity, _unit_components = nil
...
function ForceWeaponWindSlashStageEffects:wield()
	self:_update_effects(false, true, true)
end

function ForceWeaponWindSlashStageEffects:unwield()
	self:_stop_stage_particle_loop(true)
	self:_stop_stage_sound_loop()
...
function ForceWeaponWindSlashStageEffects:fixed_update(dt, t, frame)
	return
end

function ForceWeaponWindSlashStageEffects:update(dt, t)
	self:_update_effects(true, true, false)
end

function ForceWeaponWindSlashStageEffects:_update_effects(update_stage_interfacing, update_loops, force_update)
	...
	for ii = #thresholds, 1, -1 do
		if thresholds[ii].threshold <= num_charges then
...
function ForceWeaponWindSlashStageEffects:_update_stage_interfacing(current_stage, wanted_stage)
	local trigger_fx = STAGE_RANKING[current_stage] < STAGE_RANKING[wanted_stage]
	...
	if trigger_fx then
		...
		if resolved then
			...
			if should_play_husk_effect and has_husk_events then
...
function ForceWeaponWindSlashStageEffects:_update_particle_loop(current_stage, wanted_stage)
...
function ForceWeaponWindSlashStageEffects:_update_sound_loop(current_stage, wanted_stage)
...
function ForceWeaponWindSlashStageEffects:_stop_stage_particle_loop(force_stop)
	local looping_stage_effect_id = self._looping_stage_effect_id
...
function ForceWeaponWindSlashStageEffects:_stop_stage_sound_loop(force_stop)
	local wwise_world = self._wwise_world
	local sfx_source_id = self._fx_extension:sound_source(self._fx_source_name)
...
function ForceWeaponWindSlashStageEffects:_update_material_variables(wanted_stage)
...
function ForceWeaponWindSlashStageEffects:update_first_person_mode(first_person_mode)
	self:_stop_stage_particle_loop(true)
