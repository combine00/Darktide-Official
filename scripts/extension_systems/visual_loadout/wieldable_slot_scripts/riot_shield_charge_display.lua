...
function RiotShieldChargeDisplay:init(context, slot, weapon_template, fx_sources, item, unit_1p, unit_3p)
...
function RiotShieldChargeDisplay:fixed_update(unit, dt, t, frame)
	return
end

function RiotShieldChargeDisplay:update(unit, dt, t)
	local above_threshold = false
	local charge_level, max_visual_charges, crossed_threshold = nil
	local tweak_data = self._weapon_special_tweak_data
	local max_charges = nil
	...
	for ii = 1, num_displays do
		...
		display.component:set_frame_enabled(display.unit, max_charges and max_charges > 0)
	end
...
function RiotShieldChargeDisplay:update_first_person_mode(first_person_mode)
	return
end

function RiotShieldChargeDisplay:wield()
	self._num_special_charges = self._inventory_slot_component.num_special_charges
end

function RiotShieldChargeDisplay:unwield()
	return
end

function RiotShieldChargeDisplay:destroy()
	return
end
