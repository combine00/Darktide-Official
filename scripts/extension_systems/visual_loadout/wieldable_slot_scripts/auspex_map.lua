...
local AuspexMap = class("AuspexMap")

function AuspexMap:init(context, slot, weapon_template, fx_sources)
...
function AuspexMap:wield()
	return
end

function AuspexMap:unwield()
...
function AuspexMap:fixed_update(unit, dt, t, frame)
	return
end

function AuspexMap:update_unit_position(unit, dt, t)
...
function AuspexMap:_stop_scan_units_effects()
...
function AuspexMap:update_first_person_mode(first_person_mode)
...
function AuspexMap:_start_searching_sfx_loop()
...
function AuspexMap:_stop_searching_sfx_loop()
	if self._is_husk then
		return
	end
...
function AuspexMap:_start_confirm_sfx_loop()
	if self._is_husk then
		return
	end
...
function AuspexMap:_stop_confirm_sfx_loop()
	if self._is_husk then
		return
	end
...
function AuspexMap:_set_outline_unit(unit, active)
...
function AuspexMap:has_outline_unit(unit, active)
	return self._has_outline_unit
end

function AuspexMap:_set_highlight_unit(unit, active)
	local scannable_unit_extension = unit and ScriptUnit.has_extension(unit, "mission_objective_zone_scannable_system")
...
function AuspexMap:_set_scanner_light(enabled, scanner_light_components)
...
function AuspexMap:_set_scanner_lights(enabled, color)
...
function AuspexMap:_set_screen_active(enabled)
...
implements(AuspexMap, WieldableSlotScriptInterface)
