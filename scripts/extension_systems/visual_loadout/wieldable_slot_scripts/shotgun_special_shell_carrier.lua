...
local _components, _set_anim_variable = nil
local ShotgunSpecialShellCarrier = class("ShotgunSpecialShellCarrier")

function ShotgunSpecialShellCarrier:init(context, slot, weapon_template, fx_sources, item, unit_1p, unit_3p)
...
function ShotgunSpecialShellCarrier:fixed_update(unit, dt, t)
	return
end

function ShotgunSpecialShellCarrier:update(unit, dt, t, frame)
	self:_update_ammo_count()
end

function ShotgunSpecialShellCarrier:update_first_person_mode(first_person_mode)
	self:_update_ammo_count(true)
end

function ShotgunSpecialShellCarrier:wield()
	self:_update_ammo_count(true)
end

function ShotgunSpecialShellCarrier:unwield()
	return
end

function ShotgunSpecialShellCarrier:destroy()
	return
end

function ShotgunSpecialShellCarrier:_update_ammo_count(force_update)
