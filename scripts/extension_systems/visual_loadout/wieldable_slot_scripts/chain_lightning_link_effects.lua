...
function FxData:spawn_vfx(world, link_effect_name, source_unit, source_node, target_unit, target_node)
	...
	if MAX_NUM_EFFECTS_PER_TABLE < num_effects then
		return
	end
...
function FxData:spawn_vfx_world_position(world, source_unit, source_node, target_pos, target_normal, effect_name, use_target_pos, use_impact_normal_rotation, skip_length_variable)
	local num_effects = self._num_effects + 1
...
function ChainLightningLinkEffects:_create_chain_root_node()
...
function ChainLightningLinkEffects:_clear_initial_targets()
...
function ChainLightningLinkEffects:destroy()
	self:_reset()
end

function ChainLightningLinkEffects:wield()
	self:_reset()
end

function ChainLightningLinkEffects:unwield()
	self:_reset()
end

function ChainLightningLinkEffects:update_unit_position(unit, dt, t)
...
function ChainLightningLinkEffects:_find_root_targets(t)
...
function ChainLightningLinkEffects:_find_no_target(t)
	local chain_root_node = self._chain_root_node
...
function ChainLightningLinkEffects:_no_target_raycast(position, rotation, max_length)
...
function ChainLightningLinkEffects:_find_no_target_pos(t)
...
function ChainLightningLinkEffects:_clear_no_target()
...
function ChainLightningLinkEffects:_validate_targets(t)
	local chain_root_node = self._chain_root_node
...
function ChainLightningLinkEffects:_find_new_targets(t)
...
function ChainLightningLinkEffects:_find_new_chain_targets(t, broadphase, enemy_side_names, max_angle, close_max_angle, vertical_max_angle, max_z_diff, max_jumps, radius, root_target, initial_travel_direction)
	local func_context = self._func_context
	local hit_units = self._hit_units
	local temp_targets = self._temp_targets
...
function ChainLightningLinkEffects:_update_fx(t)
	local temp_targets = self._temp_targets
...
function ChainLightningLinkEffects:update_first_person_mode(first_person_mode)
	return
end

function ChainLightningLinkEffects:_reset()
	self._next_jump_time = 0
	self._next_no_target_jump_time = 0
