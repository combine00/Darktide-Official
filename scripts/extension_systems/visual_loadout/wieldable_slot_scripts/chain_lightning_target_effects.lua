...
function ChainLightningTargetEffects:init(context, slot, weapon_template, fx_sources, item, unit_1p, unit_3p)
...
function ChainLightningTargetEffects:fixed_update(unit, dt, t, frame)
	return
end

function ChainLightningTargetEffects:update(unit, dt, t)
...
function ChainLightningTargetEffects:update_first_person_mode(first_person_mode)
	return
end

function ChainLightningTargetEffects:wield()
	return
end

function ChainLightningTargetEffects:unwield()
	self:_destroy_effects()
end

function ChainLightningTargetEffects:destroy()
	self:_destroy_effects()
end

function ChainLightningTargetEffects:_create_effect(unit, targeting_fx)
	local targeting_effects = self._targeting_effects
...
function ChainLightningTargetEffects:_destroy_effect(unit)
	local world = self._world
	local targeting_effects = self._targeting_effects
...
function ChainLightningTargetEffects:_destroy_effects()
	local world = self._world
	local targeting_effects = self._targeting_effects
