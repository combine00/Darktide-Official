...
function ShockMaulHitEffects:init(context, slot, weapon_template, fx_sources, item, unit_1p, unit_3p)
...
function ShockMaulHitEffects:fixed_update(unit, dt, t)
...
function ShockMaulHitEffects:update(unit, dt, t, frame)
	return
end

function ShockMaulHitEffects:update_first_person_mode(first_person_mode)
	return
end

function ShockMaulHitEffects:wield()
	self:_register()

	self._is_wielded = true
end

function ShockMaulHitEffects:unwield()
	self:_unregister()
	self:_stop_sound()
...
function ShockMaulHitEffects:destroy()
	self:_unregister()
	self:_stop_sound()
...
function ShockMaulHitEffects:_register()
...
function ShockMaulHitEffects:_unregister()
...
function ShockMaulHitEffects:event_on_player_hit(attacking_unit, attack_result, did_damage, hit_weakspot, hit_world_position, damage_efficiency, is_critical_strike, damage_profile)
...
function ShockMaulHitEffects:_use_1p(particle_alias)
...
function ShockMaulHitEffects:_start_particle_effect(particle_alias)
...
function ShockMaulHitEffects:_stop_particle_effect()
	local world = self._world
...
function ShockMaulHitEffects:_play_sound(sound_alias)
	local use_1p = self:_use_1p()
...
function ShockMaulHitEffects:_stop_sound()
	local wwise_world = self._wwise_world
