...
function VisualLoadoutSystem:init(...)
...
function VisualLoadoutSystem:destroy()
	if not self._is_server then
...
function VisualLoadoutSystem:update(context, dt, t, ...)
...
function VisualLoadoutSystem:queue_gib_spawn(unit, gib_settings, gib_position, gib_rotation, hit_zone_name, attack_direction, damage_profile, hit_zone_gib_template, optional_override_gib_forces)
...
function VisualLoadoutSystem:rpc_player_wield_slot(channel_id, go_id, slot_id)
...
function VisualLoadoutSystem:rpc_player_unwield_slot(channel_id, go_id, slot_id)
	local unit = Managers.state.unit_spawner:unit(go_id)
	local slot_name = NetworkLookup.player_inventory_slot_names[slot_id]
	local visual_loadout_extension = self._unit_to_extension_map[unit]
