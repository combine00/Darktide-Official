...
function MispredictPackageHandler:init(unit_data_extension, item_definitions, mission)
...
function MispredictPackageHandler:item_equipped(item)
	self:_load_item_packages(item)
end

function MispredictPackageHandler:item_unequipped(item, fixed_frame)
...
function MispredictPackageHandler:_load_item_packages(item)
...
function MispredictPackageHandler:_unload_item_packages(item)
	local mission = self._mission
	local dependencies = ItemPackage.compile_item_instance_dependencies(item, self._item_definitions, nil, mission)
...
function MispredictPackageHandler:destroy()
	for fixed_frame, items in pairs(self._pending_unloads) do
		for i = 1, #items do
			local item = items[i]
