...
function PlayerUnitVisualLoadoutExtension:update_delayed_unequipped_slots(unit, dt, t, frame)
...
function PlayerUnitVisualLoadoutExtension:_unequip_item_from_slot(slot_name, from_server_correction_occurred, fixed_frame, from_destroy)
	local equipment = self._equipment
	local slot = equipment[slot_name]
	local item = slot.item
	local voice_fx_preset = item.voice_fx_preset
...
function PlayerUnitVisualLoadoutExtension:equipped_t_from_slot(slot_name)
	local slot = self._equipment[slot_name]
...
function PlayerUnitVisualLoadoutExtension:currently_wielded_slot()
...
function PlayerUnitVisualLoadoutExtension:resolve_looping_gear_sound(looping_sound_alias, use_husk_event, optional_external_properties)
...
function PlayerUnitVisualLoadoutExtension:profile_properties()
...
function PlayerUnitVisualLoadoutExtension:telemetry_wielded_weapon()
	local inventory = self._inventory_component
	local equipment = self._equipment
...
function PlayerUnitVisualLoadoutExtension:companion_slots()
...
function PlayerUnitVisualLoadoutExtension:is_slot_unit_spawned(slot_name)
	local gear_full = self._equipment[slot_name]

	return gear_full and gear_full.attachment_spawn_status == "fully_spawned"
end

function PlayerUnitVisualLoadoutExtension:is_slot_unit_valid(slot_name)
	local gear_full = self._equipment[slot_name]
