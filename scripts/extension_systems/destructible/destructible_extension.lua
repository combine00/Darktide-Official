...
function DestructibleExtension:add_to_broadphase()
...
function DestructibleExtension:remove_from_broadphase()
...
function DestructibleExtension:hot_join_sync(unit, peer_id, channel_id)
...
function DestructibleExtension:set_enabled_from_component(value)
...
function DestructibleExtension:add_damage(damage_amount, hit_actor, attack_direction, attacking_unit)
...
function DestructibleExtension:_setup_nav_gate()
	local unit = self._unit
...
function DestructibleExtension:rpc_destructible_damage_taken()
	Unit.flow_event(self._unit, "lua_damage_taken")
end

function DestructibleExtension:rpc_destructible_last_destruction()
	Unit.flow_event(self._unit, "lua_last_destruction")
end

function DestructibleExtension:rpc_sync_destructible(current_stage_index, visible, from_hot_join_sync)
	local unit = self._unit
	local destruction_info = self._destruction_info
	local visibility_info = self._visibility_info
...
function DestructibleExtension:broadphase_position()
	return self._broadphase_position:unbox()
end

function DestructibleExtension:_set_stage(new_stage, from_hot_join_sync)
...
function DestructibleExtension:_dequeue_stage(attack_direction, from_hot_join_sync)
	local destruction_info = self._destruction_info
	local visibility_info = self._visibility_info
	local current_stage_index = destruction_info.current_stage_index
...
function DestructibleExtension:_add_damage(damage_amount, attack_direction, force_destruction, attacking_unit)
	local destruction_info = self._destruction_info
	local unit = self._unit
	local current_stage_index = destruction_info.current_stage_index
...
function DestructibleExtension:set_collectible_data(data)
	self._collectible_data = data
end

function DestructibleExtension:despawn_when_destroyed()
	return self._despawn_when_destroyed
end

function DestructibleExtension:_handle_stage_zero(current_stage_index)
	if current_stage_index == 0 then
		self:_disable_nav_volume()
		self:remove_from_broadphase()
