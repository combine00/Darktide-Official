...
function SplineFollowerExtension:init(extension_init_context, unit, extension_init_data, ...)
...
function SplineFollowerExtension:extensions_ready(world, unit)
...
function SplineFollowerExtension:_setup_level_event_settings()
...
function SplineFollowerExtension:game_object_initialized(session, object_id)
	self._game_session = session
	self._game_object_id = object_id
end

function SplineFollowerExtension:update(unit, dt, t)
...
function SplineFollowerExtension:_position_rotation_from_game_object()
...
function SplineFollowerExtension:_error_speed_calculation(dt, t, game, id, movement)
...
function SplineFollowerExtension:_players_in_proximity()
...
function SplineFollowerExtension:_set_state(state)
	self._current_state = state
end

function SplineFollowerExtension:_init_movement_spline(world, unit, spline_points, spline_name)
	local spline_curve = SplineCurve:new(spline_points, "Bezier", "SplineMovementHermiteInterpolatedMetered", spline_name, 10)

	return spline_curve
end

function SplineFollowerExtension:follow_spline(name)
...
function SplineFollowerExtension:current_spline_index()
	return self._current_spline_index
end

function SplineFollowerExtension:is_moving()
	return self._is_moving
end

function SplineFollowerExtension:objective_name()
	return self._objective_name
end

function SplineFollowerExtension:hot_join_sync(current_spline_index, is_moving, objective_name)
