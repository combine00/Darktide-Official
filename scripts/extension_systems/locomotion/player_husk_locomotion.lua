...
function PlayerHuskLocomotionExtension:init(extension_init_context, unit, extension_init_data, game_session, game_object_id, owner_id)
...
function PlayerHuskLocomotionExtension:destroy()
	self._network_event_delegate:unregister_unit_events(self._game_object_id, unpack(RPCS))
end

function PlayerHuskLocomotionExtension:extensions_ready(world, unit)
	local init_context = {
		is_local_unit = false,
		is_husk = true,
		is_server = false,
...
function PlayerHuskLocomotionExtension:_get_rotation(session, id)
...
function PlayerHuskLocomotionExtension:_get_aim_direction(session, id)
	local aim_direction = GameSession.game_object_field(session, id, "aim_direction")

	return aim_direction
end

function PlayerHuskLocomotionExtension:post_update(unit, dt, t)
	...
	if parent_unit_id ~= NetworkConstants.invalid_level_unit_id then
		if parent_unit_id > 65535 then
			parent_unit = Managers.state.unit_spawner:unit(parent_unit_id - 65535, true)
		else
			parent_unit = Managers.state.unit_spawner:unit(parent_unit_id, false)
		end
	end
...
function PlayerHuskLocomotionExtension:move_speed()
	return self._move_speed
end

function PlayerHuskLocomotionExtension:move_speed_squared()
	return self._move_speed_squared
end

function PlayerHuskLocomotionExtension:_link(node, parent_unit, parent_node)
...
function PlayerHuskLocomotionExtension:_unlink()
...
function PlayerHuskLocomotionExtension:rpc_set_player_link(channel_id, game_object_id, node, parent_unit_id, parent_node)
	local parent_unit = Managers.state.unit_spawner:unit(parent_unit_id)

	self:_link(node, parent_unit, parent_node)
end

function PlayerHuskLocomotionExtension:rpc_player_unlink(channel_id, game_object_id)
	self:_unlink()
end

function PlayerHuskLocomotionExtension:current_velocity()
