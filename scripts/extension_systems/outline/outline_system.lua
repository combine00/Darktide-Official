...
function OutlineSystem:has_outline(unit, outline_name)
	local extension = self._unit_extension_data[unit]
...
function OutlineSystem:set_global_visibility(visible)
	self._disabled = not visible
end

function OutlineSystem:rpc_add_outline_to_unit(channel_id, go_id, outline_id)
...
function OutlineSystem:rpc_remove_outline_from_unit(channel_id, go_id, outline_id)
	local outline_name = NetworkLookup.outline_types[outline_id]
	local unit = Managers.state.unit_spawner:unit(go_id)
