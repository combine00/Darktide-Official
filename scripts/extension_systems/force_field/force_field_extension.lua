...
function ForceFieldExtension:init(extension_init_context, unit, extension_init_data, game_object_data_or_game_session, unit_spawn_parameter_or_game_object_id)
	self._unit = unit
...
function ForceFieldExtension:destroy()
...
function ForceFieldExtension:game_object_initialized(session, object_id)
...
function ForceFieldExtension:extensions_ready(world, unit)
	self._health_extension = ScriptUnit.extension(unit, "health_system")
end

function ForceFieldExtension:fixed_update(unit, dt, t)
...
function ForceFieldExtension:update(unit, dt, t)
	self:_update_effects(dt, t)
end

function ForceFieldExtension:_update_effects(dt, t)
...
function ForceFieldExtension:is_unit_colliding(unit_pos, unit_radius, handle_height)
...
function ForceFieldExtension:force_field_unit()
	return self._unit
end

function ForceFieldExtension:remaining_duration()
	return self._duration
end

function ForceFieldExtension:remaining_life()
...
function ForceFieldExtension:is_dead()
	return self._health_extension:is_dead()
end

function ForceFieldExtension:is_sphere_shield()
	return self._sphere_shield
end

function ForceFieldExtension:reflected_direction(unit, direction)
	local normal = nil
...
function ForceFieldExtension:on_player_enter(unit, t)
...
function ForceFieldExtension:on_player_exit(unit, t)
	if not self._is_server then
		return
	end
...
function ForceFieldExtension:on_death(t)
...
function ForceFieldExtension:_trigger_death_effects()
	local world = self._world
	local wwise_world = self._wwise_world
	local source_id = self._source_id
	local effect_id = self._effect_id
