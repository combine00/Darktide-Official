...
function CorruptorSystem:init(context, ...)
...
function CorruptorSystem:destroy()
	if not self._is_server then
...
function CorruptorSystem:rpc_set_corruptor_eye_active(channel_id, level_unit_id, activated)
...
function CorruptorSystem:rpc_set_corruptor_eye_hidden(channel_id, level_unit_id, hidden)
	local unit = Managers.state.unit_spawner:unit(level_unit_id, true)
	local extension = self._unit_to_extension_map[unit]
