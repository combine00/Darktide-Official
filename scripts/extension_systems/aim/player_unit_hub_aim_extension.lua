...
function PlayerUnitHubAimExtension:init(extension_init_context, unit, extension_init_data, game_object_data_or_game_session, nil_or_game_object_id)
...
function PlayerUnitHubAimExtension:state_machine_changed(unit)
	self._hub_aim_constraints = HubAimConstraints:new(unit, self._init_context)
end

function PlayerUnitHubAimExtension:game_object_initialized(session, object_id)
	self._game_session_id = session
	self._game_object_id = object_id
end

function PlayerUnitHubAimExtension:fixed_update(unit, dt, t, frame)
...
function PlayerUnitHubAimExtension:update(unit, dt, t)
...
function PlayerUnitHubAimExtension:_directions_and_passive_state(unit)
...
function PlayerUnitHubAimExtension:_aim_state(hub_jog_character_state_component, is_passive)
	if is_passive then
...
function PlayerUnitHubAimExtension:_target_goal_positions(unit, head_direction, torso_direction, is_passive, is_moving)
	local unit_rotation = Unit.world_rotation(unit, 1)
	local unit_forward = Quaternion.forward(unit_rotation)
