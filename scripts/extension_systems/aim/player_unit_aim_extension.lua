...
function PlayerUnitAimExtension:init(extension_init_context, unit, extension_init_data, ...)
...
function PlayerUnitAimExtension:state_machine_changed(unit)
	self._aim_constraint_variable = Unit.animation_find_constraint_target(unit, self._aim_constraint_target_name)
end

function PlayerUnitAimExtension:game_object_initialized(session, object_id)
	self._game_session_id = session
	self._game_object_id = object_id
end

function PlayerUnitAimExtension:fixed_update(unit, dt, t, frame)
...
function PlayerUnitAimExtension:update(unit, dt, t)
