...
function MinigameFrequency:init(unit, is_server, seed, context)
...
function MinigameFrequency:hot_join_sync(sender, channel)
...
function MinigameFrequency:start(player)
...
function MinigameFrequency:stop()
...
function MinigameFrequency:_get_random_frequency(seed)
	local tx, ty = nil
...
function MinigameFrequency:setup_game()
...
function MinigameFrequency:_change_target_frequency()
...
function MinigameFrequency:on_action_pressed(t)
...
function MinigameFrequency:_adjust_value_with_auto_aim(current_value, target_value, change_ratio, dt, min_scale, max_scale, input)
...
function MinigameFrequency:on_axis_set(t, x, y)
	...
	if changed then
		...
		if self._frequency_rpc_throttle_time <= t - self._last_frequency_rpc_time then
			self:send_rpc("rpc_minigame_sync_frequency_set_frequency", self._frequency.x, self._frequency.y)
...
function MinigameFrequency:update(dt, t)
	MinigameFrequency.super.update(self, dt, t)

	if self._pending_frequency_update and self._frequency_rpc_throttle_time <= t - self._last_frequency_rpc_time then
		self:send_rpc("rpc_minigame_sync_frequency_set_frequency", self._frequency.x, self._frequency.y)
...
function MinigameFrequency:uses_joystick()
	return true
end

function MinigameFrequency:is_on_target()
...
function MinigameFrequency:frequency()
	return self._frequency
end

function MinigameFrequency:set_frequency(x, y)
	self._frequency.x = x
	self._frequency.y = y
	self._cached_on_target = nil
...
function MinigameFrequency:target_frequency()
	return self._target_frequency
end

function MinigameFrequency:set_target_frequency(x, y)
...
function MinigameFrequency:_frequency_to_percentage(frequency)
...
function MinigameFrequency:_update_frequency_sound(frequency)
...
function MinigameFrequency:_set_frequency_target_sound(frequency)
	local freq_percent = self:_frequency_to_percentage(frequency)
...
function MinigameFrequency:_calculate_frequency_alignment(current, target)
