...
function BlackboardSystem:init(...)
...
function BlackboardSystem:register_extension_update(unit, extension_name, extension)
...
function BlackboardSystem:_cleanup_extension(unit)
...
function BlackboardSystem:on_remove_extension(unit, extension_name)
...
function BlackboardSystem:physics_async_update(context, dt, t, ...)
...
function BlackboardSystem:_update_blackboards_prioritized(dt, t)
	local blackboard_prioritized_updates = self._blackboard_prioritized_updates
	local num_blackboard_prioritized_updates = #blackboard_prioritized_updates
...
function BlackboardSystem:_update_blackboards(dt, t)
	local blackboard_updates = self._blackboard_updates
	local num_blackboard_updates = #blackboard_updates
	local blackboards = BLACKBOARDS
...
function BlackboardSystem:register_priority_update_unit(unit)
	local blackboard_prioritized_updates = self._blackboard_prioritized_updates
	local blackboard_updates = self._blackboard_updates
...
function BlackboardSystem:unregister_priority_update_unit(unit)
	local blackboard_prioritized_updates = self._blackboard_prioritized_updates
