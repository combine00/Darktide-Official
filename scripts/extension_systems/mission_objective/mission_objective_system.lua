...
function MissionObjectiveSystem:has_active_objective(objective_name)
...
function MissionObjectiveSystem:_setup_mission_objective(objective_name)
	local mission_objective_data = self._objective_definitions[objective_name]
	local mission_objective_type = mission_objective_data.mission_objective_type
...
function MissionObjectiveSystem:start_mission_objective_stage(objective_name, stage)
	if self._is_server then
		local objective_name_id = NetworkLookup.mission_objective_names[objective_name]
...
function MissionObjectiveSystem:end_mission_objective(objective_name)
	local objective = self._active_objectives[objective_name]
...
function MissionObjectiveSystem:external_update_mission_objective(objective_name, dt, increment)
	if self._is_server then
		local objective = self._active_objectives[objective_name]
...
function MissionObjectiveSystem:_propagate_objective_progression(objective)
...
function MissionObjectiveSystem:propagate_objective_increment(objective)
	local objective_name = objective:name()
	local objective_name_id = NetworkLookup.mission_objective_names[objective_name]
...
function MissionObjectiveSystem:is_current_active_objective(objective_name)
	return self._active_objectives[objective_name] ~= nil
end

function MissionObjectiveSystem:active_objective(objective_name)
	return self._active_objectives[objective_name]
end

function MissionObjectiveSystem:active_objectives()
	return self._active_objectives
end

function MissionObjectiveSystem:objective_progress(objective_name)
...
function MissionObjectiveSystem:set_objective_progress(objective_name, progression)
	local active_objective = self._active_objectives[objective_name]
...
function MissionObjectiveSystem:set_objective_show_ui(objective_name, show)
	local active_objective = self._active_objectives[objective_name]
...
function MissionObjectiveSystem:set_objective_ui_state(objective_name, state)
	local active_objective = self._active_objectives[objective_name]
...
function MissionObjectiveSystem:set_objective_show_bar(objective_name, show)
	local active_objective = self._active_objectives[objective_name]
...
function MissionObjectiveSystem:set_objective_show_timer(objective_name, show)
	local active_objective = self._active_objectives[objective_name]
...
function MissionObjectiveSystem:objective_event_music_progress()
	local active_objectives = self._active_objectives
...
function MissionObjectiveSystem:register_objective_synchronizer(objective_name, objective_unit)
...
function MissionObjectiveSystem:objective_synchronizer_unit(objective_name)
	return self._objective_registered_synchronizer[objective_name]
end

function MissionObjectiveSystem:register_objective_unit(objective_name, objective_unit, objective_stage)
...
function MissionObjectiveSystem:unregister_objective_unit(objective_name, objective_unit, objective_stage)
	if not self._objective_definitions[objective_name] then
		return
	end
...
function MissionObjectiveSystem:add_marker(objective_name, unit)
	local objective = self._active_objectives[objective_name]
...
function MissionObjectiveSystem:remove_marker(objective_name, unit)
	local objective = self._active_objectives[objective_name]
...
function MissionObjectiveSystem:enable_unit(objective_name, unit, stage)
	stage = stage or 1
...
function MissionObjectiveSystem:disable_unit(objective_name, unit, stage)
	stage = stage or 1
	local registered_units = self._objective_registered_units[objective_name]
	local stage_units = registered_units[stage]
...
function MissionObjectiveSystem:flow_callback_register_objective_unit(objective_name, objective_unit)
...
function MissionObjectiveSystem:flow_callback_start_mission_objective(objective_name)
	local objective = self._active_objectives[objective_name]
...
function MissionObjectiveSystem:flow_callback_update_mission_objective(objective_name)
...
function MissionObjectiveSystem:flow_callback_end_mission_objective(objective_name)
	self:end_mission_objective(objective_name)
end

function MissionObjectiveSystem:flow_callback_set_objective_show_ui(objective_name, show)
	self:set_objective_show_ui(objective_name, show)
end

function MissionObjectiveSystem:flow_callback_set_objective_ui_state(objective_name, state)
...
function MissionObjectiveSystem:flow_callback_set_objective_show_bar(objective_name, show)
	self:set_objective_show_bar(objective_name, show)
end

function MissionObjectiveSystem:flow_callback_set_objective_show_timer(objective_name, show)
	self:set_objective_show_timer(objective_name, show)
end

function MissionObjectiveSystem:send_rpc_to_clients(rpc_name, ...)
...
function MissionObjectiveSystem:rpc_update_mission_objective_progression(channel_id, objective_name_id, progression)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]
	local objective = self._active_objectives[objective_name]
...
function MissionObjectiveSystem:rpc_mission_objective_show_ui(channel_id, objective_name_id, show)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]

	self:set_objective_show_ui(objective_name, show)
end

function MissionObjectiveSystem:rpc_mission_objective_set_ui_state(channel_id, objective_name_id, state_id)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]
...
function MissionObjectiveSystem:rpc_mission_objective_show_bar(channel_id, objective_name_id, show)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]

	self:set_objective_show_bar(objective_name, show)
end

function MissionObjectiveSystem:rpc_mission_objective_show_timer(channel_id, objective_name_id, show)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]
