...
function PlayerUnitActionInputExtension:init(extension_init_context, unit, extension_init_data, game_object_data_or_game_session, nil_or_game_object_id)
	self._action_input_parsers = {}
	self._disabled = extension_init_data.is_social_hub
end

function PlayerUnitActionInputExtension:on_reload(...)
	for id, action_input_parser in pairs(self._action_input_parsers) do
		action_input_parser:on_reload(...)
	end
end

function PlayerUnitActionInputExtension:extensions_ready(world, unit)
...
function PlayerUnitActionInputExtension:peek_next_input(id)
...
function PlayerUnitActionInputExtension:last_action_auto_completed(id)
	local parser = self._action_input_parsers[id]
...
function PlayerUnitActionInputExtension:consume_next_input(id, t)
	local parser = self._action_input_parsers[id]
...
function PlayerUnitActionInputExtension:clear_input_queue_and_sequences(id)
	local parser = self._action_input_parsers[id]
...
function PlayerUnitActionInputExtension:action_transitioned_with_automatic_input(id, action_input, t)
	local parser = self._action_input_parsers[id]

	parser:action_transitioned_with_automatic_input(action_input, t)
end

function PlayerUnitActionInputExtension:mispredict_happened(fixed_frame, game_session, game_object_id)
...
function PlayerUnitActionInputExtension:update(dt, t)
	local parsers = self._action_input_parsers
...
function PlayerUnitActionInputExtension:fixed_update(unit, dt, t, fixed_frame)
	local parsers = self._action_input_parsers
...
function PlayerUnitActionInputExtension:has_any_queued_input()
...
function PlayerUnitActionInputExtension:bot_queue_action_input(id, action_input, raw_input)
	local parser = self._action_input_parsers[id]

	return parser:bot_queue_action_input(action_input, raw_input)
end

function PlayerUnitActionInputExtension:bot_queue_request_is_consumed(id, global_bot_request_id)
	local parser = self._action_input_parsers[id]

	return parser:bot_queue_request_is_consumed(global_bot_request_id)
end

function PlayerUnitActionInputExtension:bot_queue_clear_requests(id)
	local parser = self._action_input_parsers[id]

	parser:bot_queue_clear_requests()
end

function PlayerUnitActionInputExtension:network_data()
