...
function PlayerUnitTalentExtension:init(extension_init_context, unit, extension_init_data, ...)
	self._unit = unit
...
function PlayerUnitTalentExtension:_init_components()
...
function PlayerUnitTalentExtension:extensions_ready(world, unit)
...
function PlayerUnitTalentExtension:game_object_initialized(game_object_id)
	local fixed_t = self._initialized_fixed_frame_t

	self:_apply_talents(self._archetype, self._talents, fixed_t)
end

function PlayerUnitTalentExtension:buff_template_tier(buff_template_name)
	return self._buff_template_tiers[buff_template_name] or 1
end

function PlayerUnitTalentExtension:update(unit, dt, t)
	return
end

function PlayerUnitTalentExtension:fixed_update(unit, dt, t, fixed_frame, context, ...)
...
function PlayerUnitTalentExtension:destroy()
	self:_remove_gameplay_features(self._latest_fixed_frame)
end

function PlayerUnitTalentExtension:talents()
	return self._talents
end

function PlayerUnitTalentExtension:has_special_rule(special_rule_name)
	local active_special_rules = self._active_special_rules

	return active_special_rules[special_rule_name]
end

function PlayerUnitTalentExtension:select_new_talents(talents, fixed_t)
...
function PlayerUnitTalentExtension:remove_gameplay_features(fixed_t)
	self:_remove_gameplay_features(fixed_t)
end
...
function PlayerUnitTalentExtension:_apply_talents(archetype, talents, fixed_t)
...
function PlayerUnitTalentExtension:_remove_gameplay_features(fixed_t)
...
function PlayerUnitTalentExtension:_send_rpc_update_to_client(player, talents)
