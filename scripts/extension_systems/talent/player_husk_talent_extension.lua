...
function PlayerHuskTalentExtension:init(extension_init_context, unit, extension_init_data, ...)
	self._unit = unit
...
function PlayerHuskTalentExtension:_init_components()
...
function PlayerHuskTalentExtension:extensions_ready(world, unit)
	self._first_person_extension = ScriptUnit.extension(unit, "first_person_system")

	self:_update_talents(self._talents)
end

function PlayerHuskTalentExtension:buff_template_tier(buff_template_name)
	return self._buff_template_tiers[buff_template_name] or 1
end

function PlayerHuskTalentExtension:fixed_update(unit, dt, t, fixed_frame, context, ...)
...
function PlayerHuskTalentExtension:destroy()
	self._network_event_delegate:unregister_unit_events(self._game_object_id, unpack(RPCS))
end

function PlayerHuskTalentExtension:has_special_rule(special_rule_name)
...
function PlayerHuskTalentExtension:_update_talents(talents)
	self._talents = talents
...
function PlayerHuskTalentExtension:rpc_update_talents(channel_id, unit_id, talent_id_array, talent_tier_array)
