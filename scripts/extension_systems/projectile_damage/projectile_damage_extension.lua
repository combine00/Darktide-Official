...
function ProjectileDamageExtension:fixed_update(unit, dt, t)
	...
	elseif proximity_triggered then
		if self._proximity_check_started and not self._proximity_check_done and self._arm_time <= t then
			...
			local num_hits = broadphase:query(position, proximity_radius, BROADPHASE_RESULTS, enemy_side_names)
...
function ProjectileDamageExtension:post_update(unit, dt, t, context)
...
function ProjectileDamageExtension:on_impact(hit_position, hit_unit, hit_actor, hit_direction, hit_normal, current_speed, force_delete, is_target_unit)
	local owner_unit = self._owner_unit
	local projectile_unit = self._projectile_unit
	local projectile_template = self._projectile_template
	local damage_settings = projectile_template.damage
...
function ProjectileDamageExtension:_spawn_cluster(cluster_settings, direction)
	...
	if randomized_settings then
		...
		if chance_value > 0 and math.random() < chance_value then
...
function ProjectileDamageExtension:_spawn_unit(spawn_unit_settings)
	local locomotion_extension = self._locomotion_extension
	local _, position = locomotion_extension:previous_and_current_positions()
	...
	local material, placed_on_unit = nil
...
function ProjectileDamageExtension:origin_item_slot()
	return self._origin_item_slot
end

function ProjectileDamageExtension:hit_units()
...
function ProjectileDamageExtension:_handle_buff_system_explosion_proc_events(owner_unit, projectile_template, triggered_on_impact)
...
function ProjectileDamageExtension:_handle_explosion_achivements(player, explosion_hit_units_table)
...
function ProjectileDamageExtension:_record_impact_concluded_stats()
