...
function BotGroup:init(side, nav_world, traverse_logic, extension_manager)
...
function BotGroup:add_bot_unit(unit)
...
function BotGroup:data()
	local data = self._bot_data

	return data
end

function BotGroup:data_by_unit(unit)
	local data = self._bot_data[unit]

	return data
end

function BotGroup:remove_bot_unit(unit)
	self._bot_data[unit] = nil
	self._num_bots = self._num_bots - 1
end

function BotGroup:set_in_carry_event(enable)
...
function BotGroup:aoe_threat_created(position, shape, size, rotation, duration)
...
function BotGroup:set_in_cover(bot_unit, cover_hash)
	self._used_covers[bot_unit] = cover_hash
end

function BotGroup:in_cover(wanted_cover_hash)
...
function BotGroup:register_ally_needs_aid_priority(bot_unit, target_ally)
...
function BotGroup:is_prioritized_ally(bot_unit, target_ally)
	local is_prioritized_ally = self._ally_needs_aid_priority[target_ally] == bot_unit

	return is_prioritized_ally
end

function BotGroup:pickup_order(bot_unit, slot_name)
...
function BotGroup:ammo_pickup_order_unit(bot_unit)
	local bot_data = self._bot_data[bot_unit]
...
function BotGroup:has_pending_pickup_order(bot_unit)
	local bot_data = self._bot_data[bot_unit]
...
function BotGroup:update(side, dt, t)
	self._t = t
...
function BotGroup:_check_unaggroed_daemonhost_presence(side, unit_position)
...
function BotGroup:_update_move_targets(bot_data, num_bots, nav_world, side)
...
function BotGroup:_find_most_lonely_move_target(targets, origin_unit)
...
function BotGroup:_find_least_lonely_move_target(targets, last_target)
	local num_targets = #targets
...
function BotGroup:_selected_unit_is_in_disallowed_nav_tag_volume(nav_world, selected_unit_position)
...
function BotGroup:_find_origin(nav_world, unit_position)
...
function BotGroup:_find_destination_points_outside_volume(nav_world, selected_unit_position, volume_points, origin_point, needed_points)
...
function BotGroup:_calculate_center_of_volume(volume_points)
...
function BotGroup:_find_points(nav_world, origin_point, rotation, left_vectors, right_vectors, space_per_player, range, needed_points)
...
function BotGroup:_raycast(nav_world, point, vector, range)
...
function BotGroup:_find_cluster_position(nav_world, selected_unit, selected_unit_position, in_unaggroed_daemonhost_presence, daemonhost_position_or_nil)
...
function BotGroup:_find_destination_points(nav_world, origin_point, rotation, needed_points)
...
function BotGroup:_assign_destination_points(bot_data, points, follow_unit, follow_unit_table)
...
function BotGroup:_find_closest_move_target(targets, last_target, position)
	local Vector3_distance = Vector3.distance
...
function BotGroup:_update_priority_targets(bot_data, side, dt)
...
function BotGroup:_calculate_priority_target_utility(self_position, current_target, target, duration)
...
function BotGroup:_update_ally_needs_aid_priority(bot_data)
	local ally_needs_aid_priority = self._ally_needs_aid_priority
...
function BotGroup:_update_pickups(bot_data, side, dt, t)
	local Vector3_distance = Vector3.distance
	local ALIVE = ALIVE
...
function BotGroup:_update_pickups_and_deployables_near_player(bot_data, player_unit, t)
	local Vector3_distance = Vector3.distance
...
function BotGroup:_update_mule_pickups(bot_data, side, t)
	local Vector3_distance_squared = Vector3.distance_squared
...
function BotGroup:rpc_bot_unit_order(channel_id, order_type_id, bot_unit_id, order_target_id, ordering_player_peer_id, ordering_local_player_id)
	local unit_spawner_manager = Managers.state.unit_spawner
...
function BotGroup:rpc_bot_lookup_order(channel_id, order_type_id, bot_unit_id, order_target_id, ordering_player_peer_id, ordering_local_player_id)
