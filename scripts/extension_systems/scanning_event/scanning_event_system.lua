...
function ScanningEventSystem:init(context, system_init_data, ...)
...
function ScanningEventSystem:hot_join_sync(sender, channel)
...
function ScanningEventSystem:get_scanning_device_units()
	return self._unit_to_extension_map
end

function ScanningEventSystem:on_gameplay_post_init(level)
	local unit_to_extension_map = self._unit_to_extension_map
...
function ScanningEventSystem:destroy()
	self._network_event_delegate:unregister_events(unpack(RPCS))
	ScanningEventSystem.super.destroy(self)
end

function ScanningEventSystem:rpc_scanning_device_finished(channel_id, unit_id)
...
function ScanningEventSystem:rpc_scanning_device_hot_join(channel_id, unit_id, past_spline_start_position, at_end_position, reached_end_of_spline)
	local is_level_unit = true
	local unit = Managers.state.unit_spawner:unit(unit_id, is_level_unit)
