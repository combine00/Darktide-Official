...
function NavGraphExtension:init(extension_init_context, unit, extension_init_data, ...)
	self._unit = unit
...
function NavGraphExtension:extensions_ready(world, unit)
...
function NavGraphExtension:_create_nav_graphs()
...
function NavGraphExtension:destroy()
	local nav_graphs = self._nav_graphs
...
function NavGraphExtension:setup_from_component(component, unit, enabled_on_spawn, optional_simple_smart_objects, optional_smart_objects)
...
function NavGraphExtension:_check_component_registered(component_guid)
...
function NavGraphExtension:add_smart_object(smart_object, smart_object_id)
	local smart_objects = self._smart_objects
	smart_objects[#smart_objects + 1] = smart_object
...
function NavGraphExtension:_create_nav_graph(smart_object, smart_object_id)
	local smart_object_id_to_nav_graph = self._smart_object_id_to_nav_graph
	local nav_world = self._nav_world
	local debug_color = Color.orange()
	local layer_type = smart_object:layer_type()
...
function NavGraphExtension:remove_smart_object(smart_object_id)
...
function NavGraphExtension:smart_objects()
	return self._smart_objects
end

function NavGraphExtension:smart_object_from_id(smart_object_id)
	local smart_objects = self._smart_objects
...
function NavGraphExtension:add_nav_graph_to_database(smart_object_id)
...
function NavGraphExtension:add_nav_graphs_to_database()
	local nav_graphs = self._nav_graphs
	local nav_graph_added = self._nav_graph_added
...
function NavGraphExtension:remove_nav_graph_from_database(smart_object_id)
	local nav_graph = self._smart_object_id_to_nav_graph[smart_object_id]
	local unit_id = self._unit_id
	local nav_graph_added = self._nav_graph_added
...
function NavGraphExtension:remove_nav_graphs_from_database()
	local nav_graphs = self._nav_graphs
	local nav_graph_added = self._nav_graph_added
...
function NavGraphExtension:nav_graph_added(smart_object_id)
	local nav_graph = self._smart_object_id_to_nav_graph[smart_object_id]
...
function NavGraphExtension:unit()
	return self._unit
end
