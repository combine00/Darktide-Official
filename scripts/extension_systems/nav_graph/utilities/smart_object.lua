...
function SmartObject.increment_smart_object_id()
...
function SmartObject.reset_last_smart_object_id()
	SmartObject.LAST_SMART_OBJECT_ID = 0
end

function SmartObject:init()
...
function SmartObject:to_simple()
...
function SmartObject:from_simple(simple_smart_object)
	...
	local data = self._data
	local simple_smart_object_data = simple_smart_object.data
...
function SmartObject:id()
	return self._id
end

function SmartObject:set_entrance_exit_positions(entrance_position, exit_position)
	self._entrance_position:store(entrance_position)
	self._exit_position:store(exit_position)
end

function SmartObject:get_entrance_exit_positions()
	local entrance_position = self._entrance_position:unbox()
	local exit_position = self._exit_position:unbox()
...
function SmartObject:data()
	return self._data
end

function SmartObject:set_is_bidirectional(is_bidirectional)
	self._data.is_bidirectional = is_bidirectional
end

function SmartObject:is_bidirectional()
	return self._data.is_bidirectional
end

function SmartObject:set_layer_type(layer_type)
	self._layer_type = layer_type
end

function SmartObject:layer_type()
	return self._layer_type
end

function SmartObject:set_ledge(ledge_type, ledge_position, ledge_position1, ledge_position2)
...
function SmartObject:set_jump_distance(distance)
