...
function PropAnimationExtension:init(extension_init_context, unit, extension_init_data, game_object_data_or_game_session, nil_or_game_object_id)
...
function PropAnimationExtension:game_object_initialized(game_session, game_object_id)
	self._unit_id = game_object_id
	self._is_level_unit = false
	self._game_session = game_session
end

function PropAnimationExtension:setup_from_component(animation_variables, optional_state_machine_override)
...
function PropAnimationExtension:hot_join_sync(unit, sender, channel)
...
function PropAnimationExtension:anim_event(event_name)
	local unit = self._unit
	local unit_id = self._unit_id
	local is_level_unit = self._is_level_unit
...
function PropAnimationExtension:anim_event_with_variable_float(event_name, variable_name, variable_value)
	local unit = self._unit
	local unit_id = self._unit_id
	local is_level_unit = self._is_level_unit
	local variable_index = Unit.animation_find_variable(unit, variable_name)
...
function PropAnimationExtension:set_variable(name, value)
	local unit = self._unit
	local unit_id = self._unit_id
	local is_level_unit = self._is_level_unit
	local variables = self._variables
