...
function MinionToughnessHuskExtension:init(extension_init_context, unit, extension_init_data, game_session, game_object_id, owner_id)
	self._unit = unit
...
function MinionToughnessHuskExtension:destroy()
...
function MinionToughnessHuskExtension:update(context, dt, t)
...
function MinionToughnessHuskExtension:current_toughness_percent()
...
function MinionToughnessHuskExtension:stored_attacks()
	return self._stored_attacks
end

function MinionToughnessHuskExtension:toughness_damage()
	local toughness_damage, _ = _get_network_values(self._game_session, self._game_object_id)

	return toughness_damage
end

function MinionToughnessHuskExtension:toughness_templates()
	return self._toughness_template, nil
end

function MinionToughnessHuskExtension:_switch_state(context, new_state)
	local toughness_template = self._toughness_template
	local linked_actor_name = toughness_template.linked_actor
...
function MinionToughnessHuskExtension:_set_linked_actor_active(linked_actor_name, active)
...
function MinionToughnessHuskExtension:_store_toughness_attack_absorbed(damage_amount, impact_world_position)
	local attack = {
...
function MinionToughnessHuskExtension:rpc_minion_toughness_attack_absorbed(channel_id, game_object_id, impact_world_position, damage_amount)
