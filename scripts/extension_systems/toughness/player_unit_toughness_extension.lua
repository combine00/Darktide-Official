...
function PlayerUnitToughnessExtension:remaining_toughness()
...
function PlayerUnitToughnessExtension:handle_max_toughness_changes_due_to_buffs(max_toughness_before, max_toughness_after)
...
function PlayerUnitToughnessExtension:get_and_clear_restored_amount()
...
function PlayerUnitToughnessExtension:recover_flat_toughness(amount, ignore_stat_buffs, reason)
	if self:_toughness_regen_disabled(nil, nil, reason) then
		return 0
	end
...
function PlayerUnitToughnessExtension:recover_max_toughness(reason, ignore_state_block)
...
function PlayerUnitToughnessExtension:set_toughness_regen_delay()
	local t = Managers.time:time("gameplay")
...
function PlayerUnitToughnessExtension:_toughness_regen_disabled(ignore_state_block, optional_recovery_type, optional_reason)
...
function PlayerUnitToughnessExtension:_handle_procs(amount, recovered_amount, reason)
	if amount <= 0 then
		return
	end
...
function PlayerUnitToughnessExtension:_record_toughness_broken()
	local player = Managers.state.player_unit_spawn:owner(self._unit)
