...
function InteractorExtension:reset_interaction(reset_focus_unit)
	local interaction_component = self._interaction_component
...
function InteractorExtension:extensions_ready(world, unit)
	local unit_data_extension = ScriptUnit.extension(unit, "unit_data_system")
...
function InteractorExtension:_consume_conflicting_gamepad_inputs(t)
...
function InteractorExtension:_find_object_in_direct_line_of_sight(interactor_unit, fp_position, fp_forward)
...
function InteractorExtension:_find_object_near_line_of_sight(interactor_unit, fp_position, fp_forward, use_priorities, optional_max_interaction_cos_angle)
	local max_interact_distance = self:_max_interaction_distance()
...
function InteractorExtension:_find_interaction_object(interactor_unit)
...
function InteractorExtension:_find_interaction_object_3p(interactor_unit)
	local first_person_component = self._first_person_component
	local fp_position = first_person_component.position
	local fp_rotation = first_person_component.rotation
	local fp_forward = Vector3.normalize(Quaternion.forward(fp_rotation))
...
function InteractorExtension:_max_interaction_distance()
