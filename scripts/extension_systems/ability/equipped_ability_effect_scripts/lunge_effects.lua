...
function LungeEffects:init(equipped_ability_effect_scripts_context, ability_template)
...
function LungeEffects:extensions_ready(world, unit)
	self._buff_extension = ScriptUnit.has_extension(unit, "buff_system")
end
...
function LungeEffects:_start_looping_conditional_effects()
...
function LungeEffects:_stop_looping_effects()
	local world = self._world
...
function LungeEffects:_destroy_looping_effects()
	local world = self._world
	local effect_ids = self._active_looping_vfx_ids
	local num_active_effects = #effect_ids
...
function LungeEffects:_set_wwise_state()
	local lunge_template = self._lunge_template
...
function LungeEffects:_reset_wwise_state()
	local lunge_template = self._lunge_template
	local wwise_state = lunge_template and lunge_template.wwise_state
