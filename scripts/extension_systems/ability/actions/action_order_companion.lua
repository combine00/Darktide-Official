...
function ActionOrderCompanion:fixed_update(dt, t, time_in_action)
	if self._is_server and not self._triggered and self._trigger_time <= time_in_action then
		local player_unit = self._player_unit
		local companion_unit = self._companion_unit
...
function ActionOrderCompanion:finish(reason, data, t, time_in_action, action_settings)
