...
function RespawnBeaconExtension:setup_from_component(side, debug_ignore_check_distances)
	self._side = side
...
function RespawnBeaconExtension:update(unit, dt, t, context)
...
function RespawnBeaconExtension:move_hogtied_players(players)
	local valid_spawn_positions = self._valid_spawn_positions
...
function RespawnBeaconExtension:clear_occupied_units()
	for i = 1, #self._valid_spawn_positions do
		self._valid_spawn_positions[i].occupied_unit = nil
	end
end
