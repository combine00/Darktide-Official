...
function HexGrid:init(center, xy_extents, z_extents, x_cell_size, z_cell_size)
...
function HexGrid:directions()
	return DIRECTIONS, NUM_DIRECTIONS
end

function HexGrid:ijk_from_real_index(real_index)
...
function HexGrid:ijk_from_position(position)
...
function HexGrid:position_from_ijk(i, j, k)
	local root = self._root_position:unbox()
...
function HexGrid:real_index_from_ijk(i, j, k)
	local row_size = self._row_size
...
function HexGrid:real_index_from_position(position)
...
function HexGrid:is_out_of_bounds(i, j, k)
	local row_size = self._row_size
