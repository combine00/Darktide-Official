...
function PlayerManagerFixedTestify.reset_grenade_charges(_, _, data)
	local player = data.player
	local player_unit = player.player_unit
	local unit_data_extension = ScriptUnit.extension(player_unit, "unit_data_system")
...
function PlayerManagerFixedTestify.reset_magazine_ammo(_, _, data)
	local player = data.player
	local player_unit = player.player_unit
	local unit_data_extension = ScriptUnit.extension(player_unit, "unit_data_system")
	local components = unit_data_extension._components
...
function PlayerManagerFixedTestify.trigger_animation_event(_, _, player_unit, animation_event)
...
function PlayerManagerFixedTestify.wait_for_action_completed(_, _, data)
	local player = data.player
	local player_unit = player.player_unit
	local unit_data_extension = ScriptUnit.extension(player_unit, "unit_data_system")
	local weapon_action_component = unit_data_extension:read_component("weapon_action")
...
function PlayerManagerFixedTestify.wield_slot(_, t, data)
