...
function PlayerManagerTestify.local_player_archetype_name(player_manager, id)
	local local_player_id = id
	local local_player = player_manager:local_player(local_player_id)
...
function PlayerManagerTestify.make_players_invulnerable(player_manager)
...
function PlayerManagerTestify.make_players_unkillable(player_manager)
	for _, player in pairs(player_manager._players) do
		if player:unit_is_alive() then
			local player_unit = player.player_unit
			local health_extension = ScriptUnit.extension(player_unit, "health_system")
...
function PlayerManagerTestify.move_bots_to_position(player_manager, position)
...
function PlayerManagerTestify.num_bots(player_manager)
	local num_bots = player_manager:num_players() - player_manager:num_human_players()

	return num_bots
end

function PlayerManagerTestify.player_current_position(player_manager)
	local _, player = next(player_manager._human_players)

	return POSITION_LOOKUP[player.player_unit]
end

function PlayerManagerTestify.player_unit(player_manager)
	local players = player_manager:players()
...
function PlayerManagerTestify.players_are_alive(player_manager)
...
function PlayerManagerTestify.teleport_bots_forward_on_main_path_if_blocked(player_manager, bots_data)
...
function PlayerManagerTestify.teleport_players_to_main_path_point(player_manager, main_path_point)
	local players = player_manager:players()
	local human_players = table.filter(players, function (player)
		return player:is_human_controlled()
	end)
...
function PlayerManagerTestify.teleport_player_to_position(player_manager, position)
	local players = player_manager:players()
	local human_players = table.filter(players, function (player)
		return player:is_human_controlled()
	end)
...
function PlayerManagerTestify.wait_for_bots_to_reach_position(player_manager, wait_for_bots_position_data)
...
function PlayerManagerTestify.wait_for_item_equipped(_, data, time, timeout)
