...
function SessionLocalStateMachine:init(network_delegate, engine_lobby, engine_gamesession, gameobject_callback_object, clock_handler_client)
	local shared_state = {
...
function SessionLocalStateMachine:destroy()
	self:leave()
...
function SessionLocalStateMachine:update(dt)
	self._state_machine:update(dt)
end

function SessionLocalStateMachine:host()
	return self._shared_state.peer_id
end

function SessionLocalStateMachine:host_channel()
	return self._shared_state.channel_id
end

function SessionLocalStateMachine:next_event()
...
function SessionLocalStateMachine:leave()
